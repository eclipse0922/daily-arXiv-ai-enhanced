...
Motivation: Underwater sonar data are scarce, expensive, and hard to annotate, impeding the data-hungry deep learning paradigm. The study investigates whether reliable detection can be achieved without real-world training data by comparing training-free deep-learning (via synthetic data) to a robust, training-free model-based approach, and by establishing a large-scale benchmark.

Method: 1) Build a physics-based sonar simulation pipeline to generate synthetic multibeam ocnsonar data and train modern neural detectors. 2) Develop a robust model-based template-matching system that leverages geometric priors of target objects. 3) Evaluate both on real bathymetry surveys from the Baltic Sea, reporting mAP as a performance metric.

Result: Neural networks trained on synthetic data achieve 98% mAP on simulated scenes but only 40% mAP on real sonar data due to domain shift. The template-matching approach achieves 83% mAP on real data without any training, demonstrating robustness to acoustic noise and environmental variations.

Conclusion: The findings challenge the conventional wisdom that deep learning requires real data for underwater 3D detection, showing that a well-designed template-matching approach can provide a strong, training-free baseline. The work also introduces the first large-scale benchmark for training-free underwater 3D detection and has practical implications for AUV navigation, marine archaeology, and offshore monitoring in data-scarce environments.
...
<details>
  <summary>Details</summary>
Motivation: Addresses generalization across heterogeneous brain tumors, computational efficiency, and lack of interpretability/transparency in MRI tumor classifiers; aims to deliver a practical, trustworthy diagnostic tool.

Method: Dual-stream feature-level fusion of MobileNetV2 and DenseNet201 trained on 6,020 MRI scans (glioma, meningioma, pituitary tumors, and normal). 5-fold cross-validation. Augmented dataset. GradCAM used for visual explanations. Compared against VGG19, DenseNet201, MobileNetV2.

Result: Training accuracy 99.75%; testing accuracy 98.35%; F1 score 0.9835 (95% CI: 0.9743–0.9920). Baseline improvement: +3.67% accuracy over baselines; training time reduced by 39.3% vs VGG19. GradCAM heatmaps localize tumor-affected areas.

Conclusion: MobileDenseAttn is an efficient, high-performance, and interpretable model with strong potential for clinical deployment in brain-tumor MRI diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: To diagnose and mitigate a systematic gap in multimodal grounding where image references weaken factual recall, and to develop retraining-free probes for reliability.

Method: Controlled study comparing textual vs image entity references; analysis of model internal states; development of probes that flag unreliable answers without retraining; evaluation on a visual question answering setup with selective prediction.

Result: Probes achieve >92% accuracy in flagging unreliable cases; enabling selective prediction increases coverage by 7.87% absolute and reduces error risk by 0.9% absolute.

Conclusion: Addresses a systematic, detectable deficiency in multimodal grounding; recommends directions for improving grounding in VLMs and suggests practical auditing tools that can be applied without retraining.
...
<details>
  <summary>Details</summary>
Motivation: Sparse-view 3D reconstruction suffers from radiance ambiguity and shape ambiguity due to mismatched features. Incorporating semantic guidance and geometric priors can help disambiguate radiance and align geometry.

Method: Augment neural implicit representations with patch-based semantic logits optimized jointly with the signed distance field and radiance field. Introduce a regularization term based on geometric primitive masks to mitigate shape ambiguity. Evaluate on DTU and as a plugin for dense baselines like NeuS and Neuralangelo.

Result: Significant performance gains on the DTU dataset: Chamfer-distance reductions of 44% for SparseNeuS and 20% for VolRecon. As a plugin for NeuS and Neuralangelo, reductions of 69% and 68% respectively.

Conclusion: Semantic-aware reconstruction with geometry-prior regularization substantially improves sparse-to-dense 3D reconstruction quality and can meaningfully boost existing dense reconstruction pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Illegal landfills threaten health and environment; manual identification is labor-intensive and costly; public, diverse datasets are scarce due to security concerns; a lightweight, scalable approach using publicly available aerial imagery is needed.

Method: Evaluate several lightweight CNNs (MobileNetV2, GoogLeNet, DenseNet, MobileViT, among others) on the AerialWaste dataset (10,434 images from Lombardy, Italy; sources: AGEA Orthophotos, WorldView-3, Google Earth). Compare single-model performance and an ensemble/fusion approach for binary classification of landfills vs. non-landfills; observe overfitting in heavy models and emphasize generalization.

Result: Binary classification achieved 92.33% accuracy, 92.67% precision, 92.33% recall (sensitivity), 92.41% F1 score, and 92.71% specificity on the dataset.

Conclusion: Ensembling lightweight models yields strong, generalizable landfill detection performance with reduced overfitting, demonstrating the viability of public aerial datasets for scalable monitoring of illegal landfills.
...
<details>
  <summary>Details</summary>
Motivation: To overcome modality-specific structural dependencies and semantic misalignment that limit performance, robustness, and interpretability in existing MSA methods.

Method: Builds modality-specific graphs (text via linguistic syntax; acoustic/visual via text-guided attention), introduces a semantic anchor as a cross-modal semantic hub, and employs a multiview contrastive objective to enforce discriminability and coherence across intra- and inter-modal views.

Result: SSU achieves state-of-the-art performance on CMU-MOSI and CMU-MOSEI and reduces computational overhead; qualitative analyses support interpretability and nuanced emotional pattern capture.

Conclusion: Integrating structural cues with semantic grounding yields robust, interpretable multimodal representations for sentiment analysis, with potential efficiency and generalizability benefits.
...
<details>
  <summary>Details</summary>
Motivation: The need for real-time, pose-invariant, identity-preserving 3D face reconstruction from a single image; existing methods are either slow optimization-based or lack fidelity and editability in feed-forward approaches.

Method: Build a multi-view-trained 3DGS template; train an encoder to produce identity-specific, pose-invariant latent embedding; decode to residuals for the Gaussian parameters; only residuals inferred in feed-forward pass; enabling fast, robust inference; allows identity interpolation and attribute editing.

Result: Outperforms existing feed-forward 3DGS methods (e.g., GAGAvatar) in reconstruction quality; ~1000x speedup over per-face optimization methods (FlashAvatar, GaussianAvatars, GASP); enables real-time identity interpolation and attribute editing; expands photorealistic avatar applications.

Conclusion: FastAvatar delivers near-instant, pose-invariant, high-quality 3D face reconstruction with editability, broadening use in consumer and interactive systems.
...
<details>
  <summary>Details</summary>
Motivation: Balance biometric utility in emergencies with strong privacy protections for vulnerable populations; address BTP in challenging deployment settings.

Method: Formulate requirements for BTP in humanitarian contexts; comparative landscape analysis; identify PolyProtect as best option; apply to EdgeFace face embeddings on Ethiopian dataset; extend to fingerprints; assess verification, identification, irreversibility, unlinkability.

Result: PolyProtect shows favorable accuracy and privacy properties; modular and lightweight; successful application to face embeddings; feasible extension to fingerprints; first identification and fingerprint evaluation; code to be released.

Conclusion: Demonstrates viability of BTP-based biometrics in humanitarian scenarios; PolyProtect is a strong candidate; provides a path toward privacy-preserving, deployable biometric systems; further validation and code release anticipated.
...
<details>
  <summary>Details</summary>
Motivation: Vision foundation models excel at broad representations but struggle with explicit reasoning about entities, relations, and spatio-temporal structure. This relational competence is vital for fine-grained human activity recognition, egocentric video, and multimodal medical imaging. The paper advocates augmenting FMs with explicit, dynamic graphs that adapt to input and task context.

Method: Proposes lightweight, context-adaptive graph-reasoning modules that operate over dynamic graphs whose topology and edge semantics are inferred from the input. Demonstrates cross-domain evidence from human manipulation action recognition and brain tumor segmentation, showing that graph hybrids with FMs improve semantic fidelity, robustness, interpretability, and efficiency compared with FM-only baselines.

Result: Relational graph hybrids yield better fine-grained performance, improved OOD robustness, and enhanced interpretability, with reduced memory and computation due to sparse, targeted graph reasoning.

Conclusion: Urges a research agenda for FM-graph hybrids: (i) learned dynamic graph construction; (ii) multi-level relational reasoning (e.g., part–object–scene, region–organ); (iii) cross-modal fusion; (iv) evaluation protocols that directly probe relational competence in structured vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Illegible license plates hinder ALPR; although super-resolution exists, recognition remains unresolved; to optimize performance and efficiency, pre-processing should be applied only when beneficial, motivating a legibility classification task and a public dataset to study it.

Method: Collected 10,210 vehicle images with 12,687 license plates; annotated with occlusions, legibility levels (perfect, good, poor, illegible), and character labels for legible LPs; established a three-way classification (good enough, requires SR, unrecoverable) and evaluated ViT, ResNet, and YOLO as baselines.

Result: Baseline F1 scores remained below 0.8 across ViT, ResNet, and YOLO; analysis of SR and LP recognition methods indicates current approaches struggle with legibility; dataset is publicly available at the provided GitHub link.

Conclusion: Legibility classification for ALPR remains challenging; more research is needed to develop effective selective preprocessing strategies, and the LPLC dataset provides a valuable resource for future work.
...
TL;DR: CLARIFY is a two-tier Specialist-Generalist framework for dermatology VQA that pairs a fast, domain-trained Specialist with a compressed Generalist LLM, guided reasoning, and a knowledge-graph retriever to improve accuracy and efficiency, achieving 18% accuracy gain and notable VRAM/latency reductions.


<details>
  <summary>Details</summary>
Motivation: To overcome general-purpose VLMs' diagnostic limitations and high inference costs in clinical settings by creating a lightweight, trustworthy system.

Method: Hybrid architecture: (i) Specialist—a lightweight dermatologist image classifier providing accurate predictions; (ii) Generalist—a compressed conversational VLM that explains answers; (iii) knowledge-graph retrieval grounds responses; the Specialist's predictions steer the Generalist's reasoning; hierarchical design to reduce errors and expedite inference.

Result: On curated multimodal dermatology dataset, CLARIFY improves diagnostic accuracy by 18% over the strongest baseline (an uncompressed fine-tuned single-line VLM) and reduces VRAM by ≥20% and latency by ≥5%.

Conclusion: A Specialist-Generalist paradigm yields a practical, efficient, trustworthy dermatology AI system suitable for real-world deployment.
...
<details>
  <summary>Details</summary>
Motivation: Real-world face images suffer from degradations (noise, blur, compression) that degrade image quality and hinder downstream tasks. There is a need for efficient, scalable Face Image Quality Assessment (FIQA) methods that perform well across arbitrary resolutions and degradations, tested under realistic, in-the-wild conditions.

Method: Organized as part of ICCV 2025 Workshops, the VQualA 2025 FIQA challenge constrained participants to lightweight models (0.5 GFLOPs and 5 million parameters) for predicting Mean Opinion Scores (MOS) on face images with arbitrary resolutions and degradations. Submissions were evaluated using correlation metrics on an in-the-wild face image dataset; 127 participants submitted a total of 1519 finals.

Result: The challenge attracted 127 participants and 1519 final submissions. The report summarizes the diverse methodologies and findings that advance practical FIQA approaches under strong resource constraints, highlighting the collection of strategies useful for real-world deployment.

Conclusion: The study demonstrates progress toward practical, efficient FIQA methods capable of handling real-world degradations across arbitrary resolutions, and provides a consolidated view of effective approaches to guide future FIQA research and deployment.
...
<details>
  <summary>Details</summary>
Motivation: Anomalies are unpredictable and context-dependent; training with anomaly examples is impractical; there is a need for semantic and temporal cues to enable zero-shot generalization.

Method: Hybrid architecture: TimeSformer for rich spatiotemporal features; DPC forecasts future representations to detect temporal deviations; CLIP-based semantic stream with context-specific text prompts; joint training with InfoNCE and CPC losses to align visual, temporal, and semantic modalities; a context-gating mechanism modulates predictions using scene/global cues.

Result: Claims improved generalization to unseen anomalies; code is released to enable reproducibility.

Conclusion: Bridges temporal reasoning and semantic context to advance zero-shot anomaly detection in surveillance; integrates predictive modeling with vision-language understanding for cross-domain applicability.
...
<details>
  <summary>Details</summary>
Motivation: Manual annotation of 3D scene flow is expensive and time-consuming. Self-supervised methods struggle to match supervised approaches, especially for long-range and adverse-weather scenarios, hindering scalable perception models.

Method: DoGFlow computes motion pseudo-labels from 4D radar Doppler measurements and transfers them to the LiDAR domain via dynamic-aware association and ambiguity-resolved propagation. This cross-modal label transfer enables LiDAR-based backbones to learn 3D motions without ground-truth annotations.

Result: On the MAN TruckScenes dataset, DoGFlow substantially outperforms existing self-supervised methods and enables LiDAR backbones to reach over 90% of fully supervised performance using only 10% of the ground-truth data.

Conclusion: The paper introduces a novel self-supervised framework that recovers full 3D object motions without manual annotations, improves label efficiency, and demonstrates strong performance on challenging data, highlighting the potential of cross-modal pseudo-label transfer for scalable 3D perception.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of (1) image-based 3D methods that struggle to recover accurate building geometry from top-down satellite imagery and (2) detailization methods that require highly detailed voxel inputs, by enabling a coarse-to-detailed transformation with flexible geometric control and low computational cost.

Method: Propose a transformation framework that maps interpolated noisy coarse priors to detailed geometries. Introduce Skylines-50K, a large-scale dataset of over 50,000 stylized 3D building assets to support training and evaluation.

Result: Extensive evaluations demonstrate the model’s effectiveness and strong generalization across diverse scenes and building styles.

Conclusion: SatSkylines achieves flexible geometric control for 3D building generation from satellite imagery without reliance on dense voxel inputs, leveraging coarse priors and the Skylines-50K dataset to produce accurate, stylized buildings.
...
<details>
  <summary>Details</summary>
Motivation: To enable objective, data-driven detection of map transition cues in complex 3D game environments, facilitating client-side auto-mapping, navigation aids, and data-informed level design.

Method: A two-stage approach: (1) STP detection using Faster R-CNN to locate potential Spatial Transition Points; (2) a lightweight MSTP selector that ranks STPs by combining local and global visual features to identify the unique MSTP on the designer-intended critical path. The pipeline employs parameter-efficient adapters at both stages and includes an optional retrieval-augmented fusion step. A custom dataset from five Action RPG titles is used for evaluation.

Result: The study reveals a key trade-off: full-network fine-tuning achieves superior STP detection given sufficient data, while adapter-only transfer is more robust in low-data regimes and yields better MSTP selection performance. The work establishes feasibility, provides baseline metrics and a dataset, and offers initial insights into efficient model adaptation for this novel task.

Conclusion: Feasibility of detecting STPs and identifying the MSTP from a single frame is demonstrated. The paper defines a new research focus, provides a baseline pipeline and dataset, and highlights practical guidance for future AI-driven navigation aids and data-informed level-design tools.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing audio-driven animation methods in producing nuanced, cinematic character performances with realistic bodies and camera dynamics.

Method: Extend Wan into an audio-driven pipeline tailored for cinematic contexts; benchmark against cutting-edge models (Hunyuan-Avatar, Omnihuman); demonstrate applications in long-form video generation and precise lip-sync editing.

Result: Significant performance gains over existing solutions in cinematic scenarios as reported by experiments against Hunyuan-Avatar and Omnihuman.

Conclusion: Wan-S2V achieves enhanced expressiveness and fidelity for film-level animation and shows versatility in long-form video generation and lip-sync editing.
...
<details>
  <summary>Details</summary>
Motivation: Current fusion methods rely on fixed schemes (concatenation and attention) or MoE where experts handle separate decoupled features, which limits dynamic fusion and cross-feature interaction among modalities.

Method: Proposes DeReF framework with a random feature reorganization strategy between decoupling and dynamic MoE fusion modules, coupled with a regional cross-attention network within the modality decoupling module to improve decoupled-feature representations.

Result: Extensive experiments on an in-house Liver Cancer dataset and three TCGA datasets demonstrate the effectiveness of DeReF, with code to be publicly released.

Conclusion: DeReF mitigates over-reliance on fixed fusion schemes and fosters richer interaction among decoupled features through randomized reorganization and dynamic MoE fusion, yielding improved generalization and predictive performance.
...
<details>
  <summary>Details</summary>
Motivation: Current video object removal methods struggle to handle side effects (shadows, reflections, etc.) due to a lack of paired data.

Method: Generate large-scale synthetic paired data with a 3D renderer; build a diffusion-transformer video inpainting model; use full-video reference-based erasing; add supervision predicting side-effect regions via differential masks; evaluate on ROSE-Bench with five side-effect categories.

Result: ROSE achieves superior performance over existing video object erasing methods and generalizes well to real-world videos.

Conclusion: ROSE provides a systematic framework for removing objects and their side effects and introduces ROSE-Bench as a comprehensive benchmark; demonstrates effectiveness on both synthetic and real data.
...
<details>
  <summary>Details</summary>
Motivation: To address motion-detail imbalance in video captioning, where models overemphasize motion or detail leading to incomplete captions and inconsistent video understanding and generation.

Method: Data aspect: Construct HMD-270K via a two-stage pipeline (Motion-Detail Fusion and Fine-Grained Examination). Optimization aspect: Introduce Caption Set Equivalence Reward (CSER) based on Group Relative Policy Optimization, featuring unit-to-set matching and bidirectional validation. The model OwlCap is trained with HMD-270K supervision and GRPO with CSER. 

Result: OwlCap achieves significant gains on two benchmarks: detail-focused VDC (+4.2 Acc) and motion-focused DREAM-1K (+4.6 F1). The HMD-270K dataset and OwlCap model will be publicly released.

Conclusion: The proposed data and optimization strategies effectively balance motion and details in video captioning, advancing both the dataset and model performance and providing useful resources to the research community.
...
<details>
  <summary>Details</summary>
Motivation: OBIs suffer from degradation (noise, cracks) that hinder detection; leveraging domain-specific priors from the OBC font library via clustering-based representation learning can guide the feature extractor to more discriminative, robust representations.

Method: Build a clustering-based representation learning loss derived from clustering of font-library features (OBC), integrate this loss into the detector's total loss; train and evaluate using three detectors (Faster R-CNN, DETR, Sparse R-CNN) on two OBIs datasets, showing cross-framework improvements.

Result: Significant performance improvements observed across all three detectors on both OBIs datasets, indicating that the font-prior clustering objective effectively enhances feature representations for degraded OBIs detection.

Conclusion: Incorporating clustering-based feature-space learning with domain-specific font priors (OBC) can robustly improve OBIs detection across multiple modern detectors, suggesting potential for broader application in digital archaeology and recognition under degradation; future work could explore additional priors and robustness analyses.
...
<details>
  <summary>Details</summary>
Motivation: Spatial-domain SNR priors struggle with cross-channel interference and limited enhancement of informative structures. A frequency-domain approach using amplitude/phase spectra and spectral attention aims to better utilize SNR cues for underwater image enhancement.

Method: Proposes FAST (Fourier Attention SNR-prior Transformer) to fuse spectral interactions with SNR cues; FAT (Frequency Adaptive Transformer) bottleneck merges low- and high-frequency branches via gated attention. These modules are embedded in a U-shaped architecture with a conventional RGB stream and an SNR-guided branch to form SFormer. Trained on 4,800 paired images from UIEB, EUVP, and LSUI.

Result: Outperforms recent UIE methods, achieving a 3.1 dB PSNR gain and 0.08 SSIM improvement on standard benchmarks, effectively restoring colors, textures, and contrast in underwater scenes.

Conclusion: The frequency-domain SNR-prior integration within a Transformer-based U-Net (SFormer) provides significant perceptual and objective gains for underwater image enhancement, validating the effectiveness of spectral SNR priors over traditional spatial-domain approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current segmentation methods yield good pixel-level accuracy but do not translate into reliable EF estimation; ultrasound noise and artifacts bias EF calculations. A method that fuses local detail with global dynamics could improve EF accuracy.

Method: A hierarchical architecture: low-level CNNs handle single-frame detail; high-level stages use the Mamba architecture to model spatio-temporal relationships. Introduces Spatio-temporal Cross Scan (STCS) to perform skip scanning across frames and spatial positions to inject long-range context and reduce noise-induced biases.

Result: The framework is designed to improve EF estimation by balancing local detail and global dynamics and by mitigating EF biases through STCS; aims to demonstrate improved EF accuracy in echocardiography. Quantitative results are not provided in the abstract.

Conclusion: The proposed Hierarchical Spatio-temporal Segmentation Network effectively balances single-frame detail and multi-frame dynamics, with the STCS module addressing local spatio-temporal limitations and reducing EF estimation biases in echocardiography.
...
<details>
  <summary>Details</summary>
Motivation: Domain shift degrades performance when transferring models from labeled source domains to unlabeled target domains. Traditional unsupervised domain adaptation relies on fine-tuning feature extractors, which is inefficient, reduces interpretability, and scales poorly with modern architectures. Pretrained models exhibit domain-invariant geometric patterns in feature space (intra-class clustering and inter-class separation), suggesting shifts manifest mainly as boundary misalignment rather than feature degradation. There is a need for a simple, scalable adaptation method that preserves the encoder and leverages discriminative geometry.

Method: Propose Feature-space Planes Searcher (FPS): freeze the feature encoder and optimize decision boundaries in the frozen feature space. Leverage the observed domain-invariant geometric patterns (intra-class clustering, inter-class separation) to adjust decision boundaries (planes) without modifying the encoder. Use offline feature extraction to enable full-dataset optimization in a single computation cycle, reducing memory and compute costs. The approach allows interpretability of adaptation results and is applicable to large multimodal models across diverse domains.

Result: FPS achieves competitive or superior performance to state-of-the-art unsupervised domain adaptation methods on public benchmarks. It scales efficiently with multimodal large models and demonstrates versatility across domains including protein structure prediction, remote sensing classification, and earthquake detection.

Conclusion: FPS offers a simple, effective, and generalizable paradigm for transfer learning in domain adaptation. By freezing the feature encoder and focusing on boundary alignment in feature space, it reduces memory/computational demands, enables interpretability, and maintains strong or superior performance across diverse tasks and architectures.
...
<details>
  <summary>Details</summary>
Motivation: To understand why certain marine species (e.g., scallops) are under-detected beyond data quantity and to guide improvements in detector design and dataset balancing.

Method: Modify the DUO dataset to decouple localization from classification; evaluate using YOLO11 and TIDE; analyze scallop under-performance; conduct balanced vs. imbalanced classification experiments; derive strategy implications.

Result: Foreground-background discrimination is the most problematic stage independent of data quantity; even with balanced data, precision gaps persist in classification, indicating intrinsic feature-based challenges beyond data scarcity and inter-class dependencies.

Conclusion: Use imbalanced distributions when precision is prioritized and balanced distributions when recall is prioritized; improvements should focus on localization modules and algorithmic advances; code and datasets are released to support reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: HAR systems struggle with high computational cost, redundant features, and limited scalability in real-time settings; there is a need for an efficient, scalable, edge-deployable solution.

Method: Extract spatial descriptors using a customized InceptionV3 to capture multilevel patterns; model temporal dependencies with LSTM; apply an ensemble-based genetic algorithm with Adaptive Dynamic Fitness Sharing and Attention (ADFSA) to select a compact, diverse, discriminative feature subset; feed lightweight classifiers for HAR.

Result: On the UCF-YouTube dataset (challenging due to occlusion, clutter, motion, illumination), the framework achieves 99.65% accuracy, reduces feature count to as few as 7, and improves inference time, enabling real-time edge deployment (e.g., Raspberry Pi).

Conclusion: The proposed pipeline provides a scalable, efficient HAR solution that blends deep feature extraction, temporal modeling, and optimized feature selection to support robust, real-time recognition in heterogeneous environments.
...
<details>
  <summary>Details</summary>
Motivation: Ground-truth HD maps are expensive and lack geographic diversity, hindering generalization of online mapping models. A labeling-free or semi-supervised approach can leverage abundant unlabeled data.

Method: Generate pseudo-labels by reconstructing road surfaces from multi-camera images using Gaussian splatting and semantics from a pretrained 2D segmentation network. Introduce a mask-aware assignment algorithm and loss to handle partially masked pseudo-labels, enabling training without ground-truth maps. Also demonstrate semi-supervised pretraining of online mapping models using unlabeled data.

Result: Demonstrates feasibility of training online mapping models without any ground-truth maps and shows pseudo-labels can be used for effective pretraining on unlabeled data; code released at github.com/boschresearch/PseudoMapTrainer.

Conclusion: The approach enables label-free and semi-supervised online mapping training, addressing the data bottleneck of HD maps and supporting scalable learning from crowdsourced unlabeled data.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with subtle color variations and global tissue deformations in endoscopic video reconstruction. 3DGS fixes per-Gaussian color and uses linear deformation, limiting fidelity.

Method: Colored Gaussian Primitives with dynamic anchors and learnable color parameters for spatially adaptive color encoding; Enhanced Deformation Model combining time-aware Gaussian basis functions with learnable time-independent deformations to capture local deformations and global motion.

Result: On DaVinci surgical videos and benchmarks (EndoNeRF, StereoMIS), ColorGS achieves PSNR 39.85 (about 1.5 higher than prior 3DGS methods) and SSIM 97.25%, with real-time rendering.

Conclusion: ColorGS advances surgical scene reconstruction by balancing high fidelity with computational practicality, benefiting intraoperative guidance and AR/VR applications through improved color expressiveness and deformation modeling.
...
<details>
  <summary>Details</summary>
Motivation: Trustworthy AI for autonomous vehicles requires models whose decisions are interpretable, especially in complex urban scenarios; deep networks' non-linear decision boundaries hinder understanding and safety.

Method: Introduce loss functions that promote sparsity and locality in feature activations, yielding explanations of image regions that contribute to the control command; train a monocular, non-ensemble end-to-end driving model; perform ablations on feature extraction; evaluate on CARLA benchmarks.

Result: Interpretability correlates with reduced infractions; the approach yields safer, high-performance driving; the monocular, non-ensemble model surpasses top CARLA Leaderboard approaches with lower infraction scores and the highest route completion rate.

Conclusion: Sparse and localized feature maps enable interpretable explanations without sacrificing performance, as demonstrated by state-of-the-art CARLA results using a monocular, non-ensemble end-to-end driving model.
...
<details>
  <summary>Details</summary>
Motivation: Imbalanced datasets bias models toward majority classes, causing poor minority-class recognition; a regularization mechanism that prevents overfitting without hindering minority learning is needed.

Method: Extend flooding regularization to the class level by assigning a distinct flooding level to each class proportional to its frequency; during training, ensure the loss for each sample does not drop below its class's flooding level, thereby suppressing overfitting in majority classes while allowing minority classes to learn.

Result: Empirical evaluation on imbalanced image classification tasks shows improved minority-class performance and better overall generalization compared to conventional flooding.

Conclusion: Class-wise flooding regularization provides a simple, effective approach to mitigating data imbalance effects, preserving minority-class learning and potentially enhancing generalization across domains.
...
<details>
  <summary>Details</summary>
Motivation: Event cameras often generate no events in static or low-motion scenes under fixed illumination, so there is a need for persistent event stimulation without complex hardware.

Method: Attach a simple rotating unbalanced mass to induce periodic vibrational motion and apply a motion-compensation pipeline to remove the injected motion from the events, supported by a hardware prototype and real-world datasets.

Result: The approach enables recovery of motion parameters and yields improved image reconstruction and edge detection compared with motionless, event-based sensing, validated on real-world data.

Conclusion: A lightweight, hardware-efficient strategy to sustain event generation and improve downstream perception by decoupling injected motion from true scene changes.
...
<details>
  <summary>Details</summary>
Motivation: Empirical risk minimization often overfits to specific samples/domains. While Curriculum Learning (CL) and Sharpness-Aware Minimization (SAM) individually improve generalization, combining them is challenging because flat regions render loss/gradient-based difficulty measures ineffective. There is a need for a difficulty metric that remains informative as the model converges toward flat minima.

Method: Propose ADM (Adversarial Difficulty Measure) that quantifies sample difficulty via adversarial vulnerability, computed as the normalized loss gap between original and adversarial inputs. Integrate ADM into CL-based training with SAM to dynamically adjust the curriculum during training, selecting samples with higher adversarial difficulty while training toward flat minima.

Result: Empirical evaluation on image classification, fine-grained recognition, and domain generalization shows that ADM+CL+SAM preserves the strengths of both CL and SAM and outperforms existing curriculum-based and flatness-aware training methods.

Conclusion: ADM provides a robust, progress-sensitive difficulty signal in flat regimes, enabling effective curriculum design alongside SAM. The proposed approach yields improved generalization across diverse vision tasks by dynamically balancing sample difficulty with flatness-aware optimization.
...
<details>
  <summary>Details</summary>
Motivation: Addresses the gap between traditional CAD workflows (2D drawings) and modern 3D parametric modeling by enabling automatic generation of parametric CAD models from 2D vector drawings while preserving geometry and design intent.

Method: Introduce a vector-primitive representation for CAD geometry; a dual-decoder transformer that separately generates operation type and parameters and maintains alignment; a soft target distribution loss to handle parameter discreteness/flexibility; train on CAD-VGDrawing dataset of paired drawings and parametric models; provide code and dataset.

Result: Empirical experiments show the method effectively maps vector drawings to parametric CAD operations, preserving precision and design intent; dataset provided; code released.

Conclusion: Bridges 2D drawings and parametric CAD generation, potentially streamlining industrial design workflows; suggests further exploration of broader CAD ops, generalization, and integration with existing tools.
...
<details>
  <summary>Details</summary>
Motivation: Robust AVSR in noisy environments is hard; current systems struggle to estimate audio reliability and adjust reliance on modalities dynamically.

Method: Introduce router-gated cross-modal feature fusion with an audio-visual feature fusion-based router; reweight audio/visual tokens by token-level acoustic corruption scores; down-weight unreliable audio tokens; apply gated cross-attention in each decoder layer to reinforce visual cues; enables pivot to visual modality when audio worsens.

Result: On LRS3, achieves 16.51-42.67% relative reduction in WER vs AV-HuBERT; ablations show router and gating contribute to robustness.

Conclusion: Router-based adaptive fusion improves robustness of AVSR in real-world noisy conditions by enabling modality pivot and reliable token-level reweighting; the proposed components are validated by ablations.
...
<details>
  <summary>Details</summary>
Motivation: Existing HOI benchmarks (e.g., HICO-DET) rely on exact-match evaluation ill-suited for generative VLMs, penalizing valid predictions when multiple HOI interpretations exist. A fair, adaptable benchmark is needed.

Method: Define a multiple-answer MC evaluation where each question lists only ground-truth positive HOIs and a curated set of negatives that minimize penalization of valid predictions (e.g., if 'catching' is annotated, 'throwing' is not treated as a hard negative). This protocol accommodates both VLMs and HOI models and enables apples-to-apples comparison.

Result: Proposal and introduction of the benchmark protocol; claims it is the first to support both VLMs and HOI methods and provides new insights into HOI understanding.

Conclusion: The benchmark enables fair, cross-paradigm evaluation of HOI detection and will steer progress by aligning evaluation with the capabilities of modern VLMs.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of visual options in MCQs and the high cost of manual distractor authoring, enabling scalable, high-quality visual option generation.

Method: A framework that combines Multimodal Chain-of-Thought (MCoT) reasoning, Retrieval-Augmented Generation (RAG), and a content discrimination module to generate semantically plausible answers and visually similar distractors.
...
Conclusion: CmOS demonstrates effective cross-modal options synthesis for educational MCQs and offers a scalable approach to producing high-quality visual options.
...
<details>
  <summary>Details</summary>
Motivation: Need for scalable, interoperable, and real-time remote biosignal monitoring on low-power platforms, enabling non-contact vital sign assessment for healthcare/HCI.

Method: Face2PPG pipeline processing video frames sequentially for rPPG extraction; multithreaded architecture coordinating video capture, processing, networking, GUI; hybrid Functional Reactive Programming and Actor Model for event-driven processing and parallelization; HTTP streaming and REST API; collaborative UI for adaptive signal capture; target 30 fps.

Result: Real-time evaluation shows robustness and low computational overhead on constrained devices; continuous operation at 30fps with streaming and API access; demonstrates practical viability in healthcare/HCI contexts.

Conclusion: The approach addresses key real-time biosignal monitoring challenges on low-power platforms and offers practical, scalable solutions for healthcare and human-computer interaction applications.
...
<details>
  <summary>Details</summary>
Motivation: Waste sorting AI is lagging due to limited datasets and reliance on legacy detectors. The work aims to establish rigorous baselines, improve zero-shot performance with prompt engineering, and develop scalable semi-supervised annotations for real-world waste sorting.

Method: 1) Benchmark state-of-the-art Open-Vocabulary Object Detection (OVOD) models on the real-world ZeroWaste dataset. 2) Apply LLM-optimized prompts to improve zero-shot accuracy over class-only prompts. 3) Fine-tune transformer-based detectors to establish a new baseline of 51.6 mAP. 4) Propose a soft pseudo-labeling strategy that fuses ensemble predictions using spatial and consensus-aware weighting. 5) Apply to the unlabeled ZeroWaste-s subset to generate pseudo-annotations that improve performance beyond fully supervised training.

Result: Achieved a new baseline of 51.6 mAP after fine-tuning; ensemble-based semi-supervised pseudo-labeling on ZeroWaste-s yields performance gains surpassing fully supervised training; robust pipeline for generating high-quality annotations; demonstration of OVOD models under real-world waste sorting conditions.

Conclusion: The study provides rigorous baselines, a robust semi-supervised annotation pipeline, and evidence that ensemble pseudo-labeling can surpass fully supervised training in waste-detection tasks, with code released for reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: Font shapes convey impressions; standard word embeddings may miss co-occurrence-based associations among impression tags. A domain-specific representation could improve tasks like impression-guided font generation and retrieval.

Method: Build a tag co-occurrence graph where nodes are impression tags and edges encode co-occurrence. Apply spectral embedding (Laplacian eigenmaps) to derive tag vectors. Compare against BERT and CLIP through qualitative and quantitative evaluations on font generation/retrieval tasks.

Result: Impression vectors reflect co-occurrence structure; the method outperforms BERT/CLIP in impression-guided font generation across evaluated metrics.

Conclusion: A co-occurrence-driven spectral embedding for impression tags offers a domain-specific advantage for font-related tasks, suggesting that task-tailored embeddings can surpass general-purpose models in specialized applications.
...
<details>
  <summary>Details</summary>
Motivation: National Forest Inventories require labor-intensive on-site campaigns. Remote sensing + ML offer scalable, frequent, large-scale updates. Hand-crafted features and phenology metrics may be insufficient; leveraging pre-trained foundation models could be more data-efficient for species classification.

Method: Utilized time-series data from Sentinel-1, Sentinel-2, ERA5, and SRTM via Google Earth Engine. Fine-tuned a publicly available remote sensing time-series foundation model on a small labeled dataset for tree species classification in the Netherlands. Compared performance against current SOTA, likely RF-based with hand-crafted/phenology features.

Result: Fine-tuning the foundation model achieved superior accuracy across datasets, with gains up to ~10% over the prior SOTA. Demonstrates that deep AI features capture complex temporal patterns beyond hand-crafted harmonic features, especially in data-limited settings.

Conclusion: Open data and pre-trained remote sensing models can substantially enhance NFI classification and complement existing forest inventory processes. This approach reduces reliance on labor-intensive field campaigns and offers a scalable path for updating forest inventories.
...
<details>
  <summary>Details</summary>
Motivation: To enable reliable binary classification of normal vs. atypical mitotic figures in histopathology across heterogeneous domains (tumor types, species, scanners) despite severe class imbalance.

Method: Base model: ConvNeXt V2; preprocessing: 60% center cropping; training with mixed precision; evaluation via 5-fold cross-validation ensemble.

Result: Demonstrates robust performance on the diverse MIDOG 2025 dataset and supports the effectiveness of modern convolutional architectures for mitotic figure subtyping while maintaining computational efficiency through targeted preprocessing and training optimizations.

Conclusion: Modern CNN architectures, when combined with careful preprocessing (center cropping) and CV-ensembles, can effectively subtype mitotic figures in histopathology across diverse domains with efficient computation.
...
<details>
  <summary>Details</summary>
Motivation: Current ME analysis relies on fixed windowing and manual interval design; while video-level regression is explored, interval decoding remains bound to predefined windows, limiting modeling of onset/apex/offset dynamics and class distribution. There is a need to better exploit temporal evolution and inter-task information, especially with limited data.

Method: Propose a prior-guided video-level regression method with a scalable interval selection strategy that accounts for temporal evolution, duration and class distribution of MEs; introduce a synergistic optimization where spotting and recognition share parameters except for the classification heads to exploit complementary information and improve data efficiency.

Result: Achieves state-of-the-art STRS on CAS(ME)^3 (0.0562) and SAMMLV (0.2000); demonstrates robust performance across benchmarks; code released at https://github.com/zizheng-guo/BoostingVRME.

Conclusion: The prior-guided, jointly optimized framework effectively captures ME temporal dynamics, improves spotting and recognition, and makes efficient use of limited data, establishing a new performance baseline.
...
<details>
  <summary>Details</summary>
Motivation: To overcome subjectivity and biases in current outcome-oriented lapse indices and improve reliability and efficiency of microsurgical proficiency assessment.

Method: Three hospital datasets with participants at varying levels were used. The approach employs image-processing techniques to model errors geometrically and includes a detection and scoring mechanism.
...
Conclusion: The framework provides a more objective, scalable means to assess microsurgical proficiency and can inform training protocols.
...
<details>
  <summary>Details</summary>
Motivation: Fixed-parameter, pixel-level stabilizers struggle to generalize across diverse motion profiles and content; per-video adaptation can boost robustness and visual quality.

Method: Rapid test-time adaptation leveraging low-level inference cues; a jerk localization module identifies high-jerk segments; targeted adaptation focuses updates on those segments, often in a single pass, to improve stability with minimal overhead.

Result: Consistent performance gains across multiple full-frame synthesis models on diverse real-world datasets, with improved stability and visual quality and positive impact on downstream tasks.

Conclusion: The approach enables modern stabilizers to surpass state-of-the-art methods while preserving full-frame outputs and providing user-control mechanisms similar to classical methods.
...
<details>
  <summary>Details</summary>
Motivation: Medical diagnosis fairness across demographic groups under distribution shifts due to imaging equipment and practice variation. Current debiasing methods treat vision and text separately, leaving cross-modal misalignment and fairness gaps. A joint, cross-modal approach is needed to ensure fair and robust predictions.

Method: DualFairVL uses a parallel dual-branch architecture to disentangle sensitive and target attributes across modalities. It constructs approximately orthogonal text anchors via linear projections to guide cross-attention and fuse features. A hypernetwork disentangles attribute-related information and generates instance-aware visual prompts that encode dual-modal cues. Prototype-based regularization in the visual branch enforces separation of sensitive features and strengthens alignment with textual anchors. The model is trained with eight medical imaging datasets across four modalities and achieves strong fairness and accuracy with about 3.6M trainable parameters.

Result: Empirically achieves state-of-the-art fairness and accuracy under both in- and out-of-distribution settings, outperforming full fine-tuning and parameter-efficient baselines while using only 3.6M trainable parameters; code to be released.

Conclusion: Joint debiasing and alignment in multimodal vision-language models can deliver robust, fair performance in medical imaging with a compact, parameter-efficient design, suitable for deployment across diverse clinical settings.
...
<details>
  <summary>Details</summary>
Motivation: In DETR-based HOI models, randomly initialized queries yield vague representations, limiting detection performance; HOI categories are fixed for humans but objects/interactions vary, necessitating targeted query improvements.

Method: Enhances object queries with object-aware encoder features; introduces Interaction Semantic Fusion that leverages CLIP-promoted HOI candidates to seed interaction queries with semantic features; adds Auxiliary Prediction Unit to strengthen interaction representations.

Result: Achieves competitive performance on HOI benchmarks HICO-Det and V-COCO; code released at GitHub.

Conclusion: DQEN demonstrates that dual, semantics-informed query enhancement can significantly improve DETR-based HOI detection by producing more informative object- and interaction-specific queries.
...
<details>
  <summary>Details</summary>
Motivation: Bridges the gap where existing captioning/retrieval emphasizes surface-level recognition; aims to capture who/when/where/what/why by integrating contextual, temporal, and semantic cues for real-world events.

Method: Organize a grand challenge with two tracks: Event-Enriched Image Retrieval and Captioning, and Event-Based Image Retrieval; based on OpenEvents V1; Public and Private Test phases; participants evaluated and top teams invited to present; measures fairness and reproducibility.

Result: 45 participating teams from six countries; top three teams invited to present at ACM Multimedia 2025; demonstration of context-aware, narrative-driven multimedia AI concepts; availability of challenge details on official homepage.

Conclusion: EVENTA establishes a foundation for context-aware, narrative-driven multimedia AI with practical applications; invites broader adoption and further research in journalism, media analysis, cultural archiving, and accessibility.
...
TL;DR: A framework to distinguish group vs. single pedestrians from real-world trajectories using Transformer-based pair classification, with a structured metric suite and encounter typology, aimed at scalable analysis across multiple sequence lengths.


<details>
  <summary>Details</summary>
Motivation: Understand space utilization and interaction patterns in crowds; inform pedestrian simulation and space design.

Method: Segment trajectories into fixed time bins; apply Transformer-based pair classification to identify cohesive groups; develop a metric framework with spatial (convex hull area, smallest enclosing circle radius, heatmap densities) and behavioral (velocity change, motion angle deviation, clearance radius, trajectory straightness) metrics; define encounter typology (single-to-single, single-to-group, group-to-group); examine sequence lengths 60, 100, 200 frames; dataset structuring; plan for future quantitative analysis.

Result: Presents an initial classification pipeline and dataset structuring; establishes groundwork for metric-based analysis; quantitative results and full validation deferred to future work.

Conclusion: Establishes a scalable framework for analyzing group vs. single pedestrian dynamics and paves the way for quantitative evaluations and design validations in crowd dynamics.
...
TL;DR: A scalable weakly supervised framework that transfers fine-scale underwater coral morphotype information to drone imagery for high-resolution reef mapping with minimal labeling.


<details>
  <summary>Details</summary>
Motivation: Monitoring coral reefs at large spatial scales is essential but challenging: drone imagery provides broad coverage but lacks fine-scale resolution; pixel-level annotations are costly. A scalable approach is needed to map coral morphotypes across large areas with minimal manual labeling.

Method: A multi-scale weakly supervised semantic segmentation framework that transfers fine-scale ecological information from underwater imagery to aerial data. It combines classification-based supervision, spatial interpolation and self-distillation to enable large-area coral morphotype mapping with minimal annotations and to flexibly incorporate new classes.

Result: Demonstrates the efficacy of the approach for large-area segmentation of coral morphotypes and shows flexibility to integrate new classes; presents a scalable, cost-effective workflow for high-resolution reef monitoring.

Conclusion: Provides a scalable and cost-effective methodology for high-resolution reef monitoring by integrating low-cost data collection, weakly supervised deep learning, and multi-scale remote sensing, enabling large-scale, fine-grained coral mapping with limited labeled data.
...
TL;DR: Diffusion-model-based map generation guided by vector data to create style-controlled, accurate maps, integrated into a web app and validated by cartographers.


<details>
  <summary>Details</summary>
Motivation: Traditional map-making is GIS-intensive and time-consuming; GenAI offers automation but struggles with spatial composition; there's a need for controllable, accessible tools that bridge experts and non-experts.

Method: Leverage vector data to steer diffusion-based map generation; allow style control via textual prompts; build a web application; evaluate through a user study with professional cartographers focusing on fidelity, usability, and broader implications.

Result: Maps generated with vector-guided prompts achieve fidelity in controlled styles; users reported usable web app; evidence of GenAI's potential to assist both professionals and non-experts; identifies improvements and roles for cartographers.

Conclusion: GenAI-enabled, vector-guided map-making can democratize and accelerate cartography, but requires refinement; future work includes technical improvements and redefining the cartographer’s role in AI-assisted workflows.
...
TL;DR: Proposes a vision model that uses token mixing, sequence-pooling, and convolutional tokenizers to deliver state-of-the-art performance with efficient inference for fixed context-length tasks, achieving notable CIFAR-100 gains and outperforming SDPA transformers in efficiency; training is stable without common augmentation or positional tricks; code released on GitHub.


<details>
  <summary>Details</summary>
Motivation: FixContext vision tasks demand high accuracy with low compute, without reliance on data augmentation or positional embeddings. The work targets efficiency and scalability, aiming to surpass SDPA-based transformers while maintaining a compact model footprint.

Method: Introduce a vision architecture built around token mixing, sequence-pooling, and convolutional tokenizers designed for fixed context-length inputs. The model emphasizes efficiency (smaller size) and training stability, and avoids data augmentation, positional embeddings, and learning-rate schedules.

Result: On CIFAR-100, top-1 accuracy improves from 36.50% to 46.29% and top-5 from 66.33% to 76.31%. The model is more efficient than SDPA transformers when context length < embedding dimension and uses only 60% of the size. The architecture shows high training stability and does not rely on mixup, positional embeddings, or LR scheduling. Code is available on GitHub.

Conclusion: The proposed architecture achieves state-of-the-art performance and efficient inference for fixed context-length vision tasks, with clear gains on CIFAR-100 and favorable efficiency characteristics; the approach is reproducible via the public code release.
...
TL;DR: USO unifies style-driven and subject-driven generation by disentangling and recomposing content and style. It builds a triplet dataset, uses style-alignment and content-style disentanglement objectives, adds style reward learning, and introduces USO-Bench to jointly evaluate style similarity and subject fidelity. It achieves state-of-the-art results among open-source models on both dimensions.


<details>
  <summary>Details</summary>
Motivation: Style-driven and subject-driven generation are typically treated as separate, often conflicting tasks. A unified framework focusing on disentangling content from style can satisfy both stylistic similarity and subject fidelity, advancing stylized content generation.

Method: Construct a large-scale triplet dataset (content images, style images, and their stylized content). Employ a disentangled learning scheme with two complementary objectives: (1) style-alignment training to align style features, and (2) content-style disentanglement training to separate content from style. Incorporate a style reward-learning (SRL) paradigm to further enhance performance. Release USO-Bench to jointly evaluate style similarity and subject fidelity across multiple metrics.

Result: Extensive experiments show USO achieves state-of-the-art performance among open-source models on both style similarity and subject fidelity, demonstrating effective joint optimization of content and style. Code and model are released.

Conclusion: USO provides a unified, effective framework for joint style and subject customization, validated by a new benchmark and strong empirical results, suggesting broad applicability for unified style-subject control.
...
TL;DR: NeRF-based visual localization (VL) can leak fine-grained scene details when trained with photometric losses; a segmentation-supervised NeRF variant (ppNeSF) preserves privacy while achieving state-of-the-art localization.


<details>
  <summary>Details</summary>
Motivation: Address privacy risks in NeRF-based VL deployed on cloud services by evaluating how much scene detail is encoded and recoverable, and by designing a privacy-preserving alternative.

Method: 1) Propose a protocol to assess privacy-preservation of NeRF-based representations. 2) Show that NeRFs trained with photometric losses store fine-grained geometry details, vulnerable to privacy attacks even if the color head is removed. 3) Introduce ppNeSF (Privacy-Preserving Neural Segmentation Field), a NeRF variant trained with segmentation supervision instead of RGB images; segmentation labels are learned in a self-supervised manner to be coarse enough to obscure identifiable details while remaining discriminative in 3D. The segmentation space is then used for accurate visual localization.

Result: Photometric NeRFs encode fine scene details and are vulnerable to privacy attacks. ppNeSF achieves accurate visual localization using a segmentation space and, despite being privacy-preserving, attains state-of-the-art localization performance.

Conclusion: Segmentation-based NeRFs offer a practical privacy-preserving alternative for VL, obscuring identifiable details while maintaining high localization accuracy; this balances privacy with utility in cloud-based localization services.
...
TL;DR: Retrieval-augmented generation (RAG) improves multi-page Document VQA by selecting concise evidence prior to answer generation. Text-based evidence (OCR tokens) yields larger gains than purely visual evidence, while OCR-free visual retrieval still offers non-trivial improvements. Retrieving and reranking drive most gains; layout-guided chunking often helps less.


<details>
  <summary>Details</summary>
Motivation: Document VQA on long, multi-page documents is memory-intensive and challenging with naive concatenation or very large models. RAG offers an efficient alternative by retrieving a small set of relevant segments to support answer generation.

Method: Systematic evaluation of RAG variants for Document VQA: (1) text-based retrieval using OCR tokens; (2) purely visual retrieval without OCR; across multiple models and benchmarks (MP-DocVQA, DUDE, InfographicVQA); ablations on retrieval, reranking, and layout-guided chunking.

Result: Text-based retrieval improves the concatenate-all-pages baseline by up to 22.5 ANLS; visual retrieval improves by 5.0 ANLS without OCR. Ablation shows retrieval and reranking are the main sources of gain; layout-guided chunking does not help on these datasets. Gains are consistent across model sizes and multi-page benchmarks.

Conclusion: Evidence selection via retrieval augmentation yields practical accuracy boosts for multi-page Document VQA and is effective across model sizes and datasets; layout-guided chunking is not necessary for these benchmarks.
...
TL;DR: GRAS introduces a diverse bias benchmark for vision-language models across gender, race, age, and skin tone, with a GRAS Bias Score. Across five SOTA VLMs, biases persist (the best score is only 2/100). The evaluation of bias via VQA depends on multiple question formulations. The work provides data, code, and results for replication.


<details>
  <summary>Details</summary>
Motivation: To address the critical need for reliable, diverse measurement of demographic biases in vision-language models, which can impact real-world deployments and fairness.

Method: Propose GRAS, a comprehensive bias benchmark covering gender, race, age, and skin tone, plus the GRAS Bias Score metric. Evaluate five state-of-the-art VLMs on the benchmark and analyze how different VQA question formulations affect bias estimation; release code, data, and results.

Result: GRAS reveals concerning bias levels across tested models, with the best model scoring only 2/100 on the GRAS Bias Score. It also demonstrates that bias evaluation in VLMs via VQA is sensitive to question formulations, underscoring the need for diverse prompts.

Conclusion: Bias in vision-language models remains a significant challenge. A robust evaluation requires diverse VQA formulations; GRAS provides a diverse, interpretable framework and artifacts for replication and future work.
...
TL;DR: End-to-end RoofSeg: an edge-aware transformer for LiDAR roof plane segmentation, with EAMM, adaptive loss, and a plane geometric loss to improve edge accuracy and plane masks.


<details>
  <summary>Details</summary>
Motivation: Current DL-based roof segmentation is not fully end-to-end, points near edges have low discriminability, and geometry priors are underutilized; a method is needed to jointly learn masks with better edge handling and geometric constraints.

Method: Transformer encoder-decoder with learnable plane queries; Edge-Aware Mask Module (EAMM) leveraging planar edge geometry to refine masks; adaptive masking loss to downweight misclassified points; plane geometric loss to constrain training.

Result: Proposes RoofSeg architecture; demonstrates improved edge-aware segmentation accuracy via novel components; no quantitative results reported in the abstract.

Conclusion: An end-to-end RoofSeg framework effectively leverages edge-aware priors and geometric losses to enhance roof plane segmentation from LiDAR; addresses key limitations of prior approaches.
...
TL;DR: IntroducesMicroDetect-Net(MDN),aCNN-basedpipelineusingNileRedfluorescencetostdetectmicroplasticsinstainedbloodimages;achieved92%accuracyandIoU87.4%on276samples;promisingbutlimitedbyuseofclambloodashumanproxyandneedvalidationonhumanblood.


<details>
  <summary>Details</summary>
Motivation: Risingmicroplasticpollutionacrossenvironmentsandpotentialhumanhealthrisksnecessitatess automated,high-throughputdetectioninbiologicalmatrices;currentmethodsaremanualandlimited,soadeep-learningimagingapproachcouldacceleratescreening.

Method: PrepareNileRed-stainedfluorescenceimagesfrombloodsamples(clambloodasproxy).UseCNN-basedsegmentationforlocalizationandcountingofmicroplasticfragments;integratedatasetpreparation,fluorescenceimaging,ands segmentation;Evaluateon276images;metricsincludeaccuracy,IoU,F1,precision,recall.

Result: MDNachievesaccuracy92.0%;IoU87.4%;F1 92.1%;Precision90.6%;Recall93.7%.

Conclusion: MDNdemonstrateseffective detectionofmicroplasticsinstainedbloodimages;acknowledgeslimitationsduetousenofproxybloodandsuggestsvalidationonhumansamples;openspathforautomatedscreeninginenvironmentalandbiomedicalcontexts.
...
TL;DR: ProPy adapts CLIP for partially relevant video retrieval using a Prompt Pyramid and Ancestor-Descendant Interaction, achieving state-of-the-art on three datasets.


<details>
  <summary>Details</summary>
Motivation: PRVR tasks require retrieving videos based on queries relevant to specific segments, and existing approaches underutilize powerful vision-language models like CLIP; there is a need for multi-granularity semantic prompts and dynamic hierarchical interactions to capture partial relevance.

Method: Systematic architectural adaptation of CLIP for PRVR. Introduce a Prompt Pyramid to organize event prompts across multiple granularity levels and an Ancestor-Descendant Interaction Mechanism that enables dynamic semantic interaction among events within the pyramid.

Result: State-of-the-art performance on three public PRVR datasets, with significant gains over baselines.

Conclusion: Demonstrates effective integration of vision-language models for PRVR via hierarchical, multi-granularity prompts and interactions; validates the proposed design and provides code for reproducibility.
...
TL;DR: Large geometric artery datasets with self-supervised heat-kernel targets improve coronary wall shear stress segmentation in data-scarce settings.


<details>
  <summary>Details</summary>
Motivation: Data scarcity limits training for ML-based hemodynamic biomarkers like wall shear stress; leveraging big geometric shape datasets with self-supervised learning could bridge the gap.

Method: Train on a large collection (8449) of 3D vessel shapes using the heat kernel signature as a self-supervised target (derived from Laplacian eigenvectors); transfer learned representations to segment coronary arteries into regions of time-averaged low/mid/high wall shear stress, evaluated on a small clinical trial (49 patients).

Result: Geometric representations learned from the large dataset improve segmentation of coronary arteries into low/mid/high WSS regions even when trained on limited clinical data.

Conclusion: Self-supervised learning on large geometric artery models (via heat-kernel targets) can enhance hemodynamic biomarker assessment and demonstrates the value of big geometric datasets for medical image analysis.
...
TL;DR: A discriminative, efficient surface-defect detector named SuperSimpleNet unifying four supervision paradigms (unsupervised, weakly supervised, mixed, and fully supervised) via synthetic anomaly generation, an enhanced classifier head, and improved learning, achieving state-of-the-art results across four benchmarks with inference under 10 ms.


<details>
  <summary>Details</summary>
Motivation: Industrial surface defect detection requires high performance, speed, and adaptability to diverse annotation scenarios. Many methods are limited to specific supervision setups and struggle to generalize to real-world data. There is a need for a single model that can leverage all available annotations and operate efficiently in production settings.

Method: Build on SimpleNet to create SuperSimpleNet, introducing (1) a synthetic anomaly generation process, (2) an enhanced classification head, and (3) an improved learning procedure. The model is designed to train effectively under unsupervised, weakly supervised, mixed supervision, and fully supervised settings, enabling it to utilize all available data annotations.

Result: Achieves state-of-the-art performance across four challenging defect-detection benchmarks and maintains very fast inference (below 10 ms). It is positioned as the first model capable of leveraging all annotation regimes.

Conclusion: SuperSimpleNet offers a unified, fast, and accurate framework for industrial surface defect detection, bridging the gap between academic research and practical manufacturing applications. The accompanying code is available at the provided GitHub URL.
...
TL;DR: A bilevel optimization framework learns task-specific binary illumination patterns for single-pixel imaging, using Straight-Through Estimator and a Total Deep Variation regularizer; demonstrates improved reconstruction in undersampled fluorescence microscopy on CytoImageNet.


<details>
  <summary>Details</summary>
Motivation: To overcome non-differentiable binary pattern optimization in single-pixel imaging and tailor illumination to specific tasks (e.g., fluorescence microscopy), improving reconstruction under limited measurements.

Method: Formulate as bilevel optimization; handle binary patterns with Straight-Through Estimator; apply Total Deep Variation regulariser; train on CytoImageNet microscopy dataset to learn patterns optimized for reconstruction.

Result: Learned patterns yield superior reconstruction compared to baselines, especially in highly undersampled regimes.

Conclusion: Task-specific binary illumination patterns learned via bilevel optimization enhance single-pixel imaging performance and are particularly advantageous when measurements are scarce; relevant for fluorescence microscopy.
...
TL;DR: A lightweight, topology-aware few-shot approach for historical text line segmentation that outperforms larger models using only three annotated pages per manuscript.


<details>
  <summary>Details</summary>
Motivation: Text line segmentation in historical documents suffers from data scarcity and costly expert annotations; few-shot learning could reduce data requirements.

Method: Use a lightweight UNet++ architecture with a connectivity-aware loss designed to penalize line fragmentation and unintended merges; train on small patches extracted from only three annotated pages per manuscript.

Result: Significant gains on U-DIADS-TL: 200% higher Recognition Accuracy and 75% higher Line IoU; competitive or superior F-Measure compared to DIVA-HisDB winner; public implementation.

Conclusion: Demonstrates that small, simple models with topology-aware loss can achieve state-of-the-art results in data-scarce historical document analysis.
...
TL;DR: Addresses the misalignment between numeric magnitudes and units in text-driven monocular 3D grounding by enhancing 3D-text representations with two modules, achieving SOTA on Mono3DRefer, especially in Far scenarios.


<details>
  <summary>Details</summary>
Motivation: Text-based 3D grounding suffers from weak 3D understanding of pretrained language models, where numerics are sensitive to magnitude but units are largely ignored, causing misguidance in geometry reasoning.

Method: Propose two modules: (i) 3D-text Enhancement (3DTE) to diversify distance descriptors and improve unit handling; (ii) Text-Guided Geometry Enhancement (TGE) to project basic text features into a geometrically consistent space and guide geometry feature attention.

Result: On Mono3DRefer, the approach yields substantial improvements over prior methods, including a notable 11.94 percentage point gain in the Far scenario, indicating strong 3D-text–geometry alignment.

Conclusion: Combining enhanced 3D-text representations with geometry-guided embedding attention effectively improves monocular 3D visual grounding; results support the approach's efficacy and generalization, with code to be released.
...
TL;DR: We propose Weierstrass Elliptic Function Positional Encoding (WEF-PE) for Vision Transformers to encode 2D coordinates directly in the complex domain, leveraging elliptic function properties (doubly periodicity, addition formula) to preserve spatial proximity and enable relative position derivation, yielding improved performance and stronger geometric inductive bias; code available on GitHub.


<details>
  <summary>Details</summary>
Motivation: Standard 1D, patch-flattened positional encodings disrupt the native 2D geometry of images and lack geometric constraints that tie Euclidean distances to index distances, limiting the model's ability to leverage spatial proximity prior information.

Method: Represent 2D image coordinates in the complex plane and encode them with Weierstrass elliptic functions. Exploit the functions' doubly periodic nature to encode two-dimensional spatial structure and use their addition formula to derive relative positions between patch pairs from absolute encodings. Integrate this 2D, geometry-aware encoding into Vision Transformers and support it with theoretical distance-decay analysis and attention visualizations.

Result: WEF-PE achieves superior performance across tasks: 63.78% accuracy on CIFAR-100 with ViT-Tiny trained from scratch, 93.28% on CIFAR-100 with ViT-Base fine-tuning, and consistent improvements on VTAB-1k. The approach is supported by a rigorous mathematical proof of distance-decay and attention visualizations showing more coherent geometric focus. Source code is publicly available on GitHub.

Conclusion: WEF-PE offers a mathematically principled, geometry-preserving 2D positional encoding for Vision Transformers, enabling direct handling of 2D spatial relationships, improved inductive bias, and practical gains across datasets, with available implementation.
...
TL;DR: SoccerNet 2025 challenges benchmark four football-video tasks with unified data/evaluation, revealing progress and strong baselines, and fostering open, reproducible research.


<details>
  <summary>Details</summary>
Motivation: To advance football video understanding through standardized benchmarks, large-scale annotated data, and reproducible evaluation, enabling fair comparison and progress tracking across tasks like action spotting, depth estimation, foul recognition, and game-state reconstruction.

Method: Establish a multi-task benchmark (four vision tasks) with large annotated datasets, unified protocols, and baseline models; provide development kits and leaderboards; report results and top solutions for the community.

Result: The report summarizes results for each task, identifies top-performing approaches, and provides insights into community progress toward accurate, robust football video understanding.

Conclusion: SoccerNet continues to drive reproducible, open research at the intersection of computer vision, AI, and sports, setting a standard for benchmarking and inspiring further advances.
...
TL;DR: Efficient mesh generation by decoupling vertex and face creation: autoregressively generate vertices (reducing token count to ~23% of compact tokenizers), then use a bidirectional transformer to infer the full mesh and adjacency (faces) in one step, with fidelity enhancement and post-processing to refine edges. Claims >8x speedup and higher quality than state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Current tokenized mesh generation methods create long token sequences because vertices are shared among faces, leading to redundancy and slow generation. A more compact, faster, and higher-quality approach is needed.

Method: 1) Autoregressive generation of vertices only, drastically reducing tokens (~23% of conventional compact tokenizers). 2) A bidirectional transformer completes the mesh by modeling inter-vertex relations and constructing the face adjacency matrix in one shot. 3) Fidelity enhancer to refine vertex positions for natural layouts. 4) Post-processing to remove undesirable edge connections.

Result: Experimental results show the method is more than 8 times faster in mesh generation than state-of-the-art approaches and produces higher-quality meshes.

Conclusion: Decoupling vertex and face generation with a vertex-focused autoregressive model, followed by a single-shot mesh completion via a bidirectional transformer and refinement/post-processing steps, can substantially improve both efficiency and quality in artistic mesh generation.
...
TL;DR: All-in-One Slider: a lightweight, sparse-direction-based slider for text-to-image diffusion that enables continuous, interpretable manipulation of multiple attributes, supports zero-shot and attribute composition, and can be extended to real-image editing via inversion.


<details>
  <summary>Details</summary>
Motivation: Current One-for-One sliders require training a separate slider per attribute, causing parameter redundancy and limited scalability. There is a demand for flexible, interpretable, and scalable control over rich-detail content (e.g., faces) and for zero-shot handling of unseen attributes.

Method: Introduce an All-in-One Slider that decomposes the text embedding space into sparse, semantically meaningful attribute directions; train it as a general-purpose slider to enable continuous control over multiple attributes; recombine learned directions to manipulate unseen attributes (zero-shot) and to compose multiple attributes; optionally integrate with an inversion framework to edit real images.

Result: Extensive experiments show accurate and scalable attribute manipulation, with notable improvements over prior methods; demonstrated zero-shot manipulation of unseen attributes (e.g., races, celebrities) and the ability to compose multiple attributes; extends to real images via inversion; code and models to be released.

Conclusion: The All-in-One Slider offers interpretable, scalable, and flexible attribute control for text-to-image diffusion, reducing parameter redundancy and enhancing practical applicability, with potential real-image editing via inversion.
...
TL;DR: A diffusion-assisted 3D driving-scene generation framework that yields controllable, geometry-grounded, large-scale scenes by jointly modeling proxy geometry and environment, guided by 2D priors and map layouts, enabling causal novel-view synthesis and object permanence.


<details>
  <summary>Details</summary>
Motivation: Current image/video diffusion models offer controllable generation but lack reliable geometry grounding and causality; traditional neural reconstructions create static scenes with limited diversity. There is a need for scalable, geometry-consistent 3D driving data that can be steered by prompts and maps.

Method: Proposes a pipeline that generates proxy geometry and an environment representation, then performs score distillation from learned 2D image priors to align 3D geometry and texture with 2D cues. The system supports prompt-driven geometry and map-conditioned realizations, producing high-fidelity, geometrically consistent 3D driving scenes with object permanence.

Result: The approach achieves high controllability and realistic geometry-texture fidelity, enabling causal novel-view synthesis and explicit 3D geometry estimation within large-scale driving scenes conditioned on map layouts.

Conclusion: Bridges diffusion-based controllable image generation with explicit 3D geometry grounding, enabling scalable, realistic, and manipulable 3D driving scene generation suitable for training/testing large-scale robotics data pipelines.
...
TL;DR: OmniHuman-1.5 presents a framework that combines multimodal LLM-driven semantic guidance with a Multimodal DiT (Pseudo Last Frame) to generate character animations that are physically plausible and semantically coherent, achieving strong lip-sync, video quality, motion naturalness, and semantic consistency, and generalizing to multi-person and non-human subjects.


<details>
  <summary>Details</summary>
Motivation: Existing video avatar models produce realistic motion but lack deep semantic understanding of emotion, intent, and context, often syncing with rhythm rather than semantic content. There is a need for integrating audio, imagery, and text to produce expressive, context-aware animations.

Method: 1) Use Multimodal Large Language Models to synthesize a structured textual representation of conditions, providing high-level semantic guidance for the motion generator. 2) Introduce a Multimodal DiT architecture with a novel Pseudo Last Frame design to effectively fuse multimodal inputs and mitigate inter-modality conflicts. 3) The model interprets joint semantics of audio, images, and text to generate motions coherent with character, scene, and linguistic content.

Result: Extensive experiments show leading performance across metrics: lip-sync accuracy, video quality, motion naturalness, and semantic consistency with textual prompts. The approach also scales to complex scenarios including multiple people and non-human subjects.

Conclusion: The framework delivers physically plausible and semantically coherent, expressive character animations by integrating multimodal semantic guidance with a robust fusion architecture, with strong generalization to complex scenes.
...
TL;DR: An in-situ CNT tracking framework (VFTrack) for SEM sequences that automatically detects and tracks individual CNT particles, enabling real-time kinematic analysis and pillar morphology reconstruction using ALIKED detector + LightGlue matcher (F1=0.78, α=0.89) on 13,540 annotated trajectories.


<details>
  <summary>Details</summary>
Motivation: Current in-situ SEM tracking struggles with manual initialization, discontinuous per-particle trajectories, and lack of real-time analysis. There is a need for automated, continuous tracking to quantify CNT growth dynamics and optimize synthesis.

Method: Integrates either handcrafted or deep feature detectors with a matcher within a particle-tracking framework. Systematically evaluated 13,540 manually annotated trajectories to identify the optimal combination (ALIKED detector + LightGlue matcher). Motion vectors are decomposed into axial growth, lateral drift, and oscillations to compute regional growth rates and reconstruct evolving CNT pillar morphologies in real time.

Result: Optimal detector/matcher combination (ALIKED + LightGlue) achieved F1-score 0.78 and α-score 0.89 on the annotated dataset. VFTrack enables decomposition of particle motion and reconstruction of CNT pillar dynamics, supporting automated, real-time nano-material characterization.

Conclusion: VFTrack closes the gap between physics-based CNT growth models and experimental observation, enabling real-time optimization of CNT synthesis through automated, in-situ tracking and kinematic analysis.
...
TL;DR: Autoregressive Universal Segmentation Model (AUSM) unifies prompted and unprompted video segmentation by sequentially predicting masks with a fixed spatial state, enabling streaming and parallel training with strong benchmarks.


<details>
  <summary>Details</summary>
Motivation: To unify prompted and unprompted video segmentation and support long, streaming video inputs with efficient training/inference using a fixed-size spatial state.

Method: Develop AUSM on state-space model foundations; maintain fixed spatial state, predict masks autoregressively over time; enable parallel training across frames; evaluate on standard video segmentation benchmarks.

Result: Outperforms prior universal streaming methods; achieves up to 2.5x faster training on 16-frame sequences; competitive or superior performance on multiple benchmarks.

Conclusion: AUSM provides a unified, scalable approach that bridges both segmentation settings and enables efficient long-sequence video segmentation.
...
TL;DR: Introduces Style4D-Bench, a benchmark suite for 4D stylization, plus Style4D baseline on 4D Gaussian Splatting, featuring an evaluation protocol, baseline model, and a diverse high‑resolution 4D scene collection to standardize evaluation and accelerate progress.


<details>
  <summary>Details</summary>
Motivation: There is no standardized benchmark for 4D stylization. A unified evaluation protocol across spatial fidelity, temporal coherence, and multi-view consistency is needed to fairly compare methods and drive progress in this emerging area.

Method: Style4D-Bench provides an evaluation protocol with perceptual and quantitative metrics for spatial, temporal, and multi-view aspects. It also supplies a baseline model, Style4D, built on 4D Gaussian Splatting (4DGS). Style4D comprises: (1) a basic 4DGS scene representation to capture geometry; (2) a Style Gaussian Representation with lightweight per-Gaussian MLPs for temporally/spatially aware appearance control; (3) a Holistic Geometry-Preserved Style Transfer module that enhances spatio-temporal consistency via contrastive coherence learning and content preservation.

Result: Extensive experiments on Style4D-Bench show Style4D achieves state-of-the-art performance in 4D stylization, delivering fine-grained stylistic detail with stable temporal dynamics and consistent multi-view rendering.

Conclusion: Style4D-Bench is a valuable benchmarking resource for the stylized rendering of dynamic 3D scenes and is expected to catalyze future research. The paper provides project code and a dedicated page for accessibility.
...
TL;DR: A training-free method (Articulate3D) edits 3D mesh poses via language by generating target views with a modified image generator and performing multi-view pose optimization using keypoints, preserving identity; user study favors it over rivals.


<details>
  <summary>Details</summary>
Motivation: Pose editing of 3D assets via natural language remains challenging and often requires training data or compromises on identity/consistency; a training-free, generalizable solution is highly desirable.

Method: Two-stage approach: (1) modify a powerful image generator with a self-attention rewiring mechanism (RSActrl) to decouple source structure from pose so the same structure is maintained across poses; (2) generate target images conditioned on the input image and a text instruction; (3) use keypoints to establish correspondences between input and target images (instead of differentiable rendering) and perform multi-view pose optimization to align the mesh to the targets.

Result: Demonstrates effective pose manipulation across diverse 3D objects and free-form prompts while preserving mesh identity; quantitative metrics and a user study show strong performance, with over 85% of participants preferring it to existing approaches.

Conclusion: Articulate3D demonstrates the feasibility and effectiveness of a training-free, language-controlled 3D pose editing pipeline. The RSActrl mechanism and keypoint-based optimization contribute robustness and identity preservation across poses, though limitations may lie in keypoint reliability and articulation complexity; the method achieves strong user preference and outperforms competing methods.
...
TL;DR: VoxHammer is a training-free method for precise, coherent 3D editing in latent space. It inverts a structured 3D generative model to obtain per-timestep latents and key-value tokens, then swaps preserved-region denoising features with these inverted latents to maintain consistency. It introduces Edit3D-Bench for evaluation and reports superior 3D consistency and quality over prior work.


<details>
  <summary>Details</summary>
Motivation: Current 3D editing pipelines often rely on editing rendered views and re-synthesizing 3D geometry, which struggles to keep unedited regions intact and maintain global coherence. A training-free, latent-space approach promises more precise, coherent edits with less data/compute.

Method: Predict the inversion trajectory of the input 3D model to obtain inverted latents and per-timestep key-value tokens. During denoising/editing, replace denoising features of preserved regions with the corresponding inverted latents and cached tokens, preserving context to ensure coherent reconstruction. Construct Edit3D-Bench (human-annotated with labeled editing regions) for evaluation. Compare against prior methods on 3D consistency and quality.

Result: VoxHammer significantly outperforms existing methods in 3D consistency of preserved regions and overall quality in experiments. The authors also provide a dataset (Edit3D-Bench) to benchmark region-preserving fidelity and argue the approach is well-suited for generating edited paired data for in-context 3D generation.

Conclusion: A training-free, latent-space editing framework can achieve precise, coherent edits in 3D by preserving regional context through inverted latents/tokens; VoxHammer’s strategy and the Edit3D-Bench dataset advance 3D in-context editing and data synthesis for future 3D generation tasks.
...
### [75] [Reasoning Steps as Curriculum: Using Depth of Thought as a Difficulty Signal for Tuning LLMs](https://arxiv.org/abs/2508.18279)
*Jeesu Jung,Sangkeun Jung*

Main category: cs.LG

TL;DR: Proposes using depth of thought (DoT) as a scalable, interpretable difficulty signal for curriculum learning in LLMs, measured by steps in a teacher's reasoning trace (e.g., Chain-of-Thought). Outlines a shallow-to-deep DoT-ordered curriculum and a framework for deriving, validating, and scheduling it at scale.


<details>
  <summary>Details</summary>
Motivation: Align model reasoning training with human cognitive depth to improve learning efficiency while maintaining scalability and interpretability in reasoning tasks.

Method: Define DoT as the discrete number of steps in a teacher model's reasoning trace (such as Chain-of-Thought). Construct curricula ordered from shallow to deep by DoT; provide a scalable method to derive, validate, and schedule DoT across data and budgets; propose an evaluation framework and discuss threats to validity with mitigations.

Result: No empirical results are reported in the abstract. The paper puts forth three testable hypotheses and a proposed evaluation framework for DoT-based curricula, aiming to enable cognitively grounded, interpretable curricula for reasoning.

Conclusion: If validated, DoT-based curricula could offer a cognitively grounded, interpretable path for reasoning-centric training that is robust across teacher models, subject to empirical validation and effective mitigation of potential biases or confounds.
...
### [76] [Multi-Modal Drift Forecasting of Leeway Objects via Navier-Stokes-Guided CNN and Sequence-to-Sequence Attention-Based Models](https://arxiv.org/abs/2508.18284)
*Rahmat K. Adesunkanmi,Alexander W. Brandt,Masoud Deylami,Gustavo A. Giraldo Echeverri,Hamidreza Karbasian,Adel Alaeddini*

Main category: cs.LG

TL;DR: A multi-modal ML framework combines textual embeddings with attention-based seq-to-seq models to predict leeway object drift, leveraging Navier-Stokes–based drag/lift estimation and environmental data to forecast 1–10 s horizons; it achieves competitive performance with physics-based models while enabling longer-horizon predictions.


<details>
  <summary>Details</summary>
Motivation: Drift prediction for leeway objects is critical in time-sensitive maritime operations (e.g., search and rescue). Pure physics or single-modality ML approaches struggle with generalization and longer horizons. Integrating physics-derived features, environmental data, and natural language descriptions via multi-modal learning can improve accuracy and enable multi-step forecasting across diverse objects.

Method: Data collection of environmental (wind/current) and object features (mass, surface area) for five leeway objects. Use Navier-Stokes–based simulations to train a CNN that estimates drag and lift coefficients, which yield net driving forces. Build a time-series of forces, environmental velocities, and object features. Encode textual descriptions with Sentence Transformer embeddings. Input the physical-time-series plus text embeddings to attention-based seq-to-seq models (LSTM/Transformer) to predict future drift trajectories. Evaluate at 1, 3, 5, and 10 seconds, compare against a fitted physics-based model and traditional ML baselines (RNNs, temporal CNNs).

Result: The multi-modal models achieve performance comparable to traditional models for short horizons and offer the ability to forecast multiple steps into the future, enabling longer-term predictions beyond single-step forecasts. The approach generalizes across different objects.

Conclusion: A multi-modal modeling strategy that fuses physics-derived dynamics, environmental context, and textual descriptions with advanced sequence models yields accurate and adaptable leeway drift predictions in dynamic maritime environments, with improved multi-step forecasting and cross-object generalization.
...
### [77] [Data-driven models for production forecasting and decision supporting in petroleum reservoirs](https://arxiv.org/abs/2508.18289)
*Mateus A. Fernandes,Michael M. Furlanetti,Eduardo Gildin,Marcio A. Sampaio*

Main category: cs.LG

TL;DR: A data-driven ML framework to forecast production from simple data, addressing concept drift, validated on synthetic (UNISIM III) and Brazilian pre-salt data; aims for a rapid, robust reservoir predictor to aid management.


<details>
  <summary>Details</summary>
Motivation: To enable reliable production forecasting without relying on detailed geological models or well-specific information, while anticipating changes in rock-fluid system behavior and supporting reservoir management decisions.

Method: Perform relevance analyses of production/injection variables and data conditioning; address concept drift with windowing and retraining strategies; compare supervised regression and neural networks for production forecasting; validate on synthetic UNISIM III data and real Brazilian pre-salt cases.

Result: The abstract outlines a methodological framework and expected outcomes rather than presenting concrete results; it anticipates a reliable predictor capable of rapid response, handling practical restrictions, and informing management decisions.

Conclusion: If successful, the approach would provide a data-driven tool for reservoir management that can anticipate deleterious behaviors, optimize production/injection, and analyze probabilistic events to maximize oil recovery, while emphasizing retraining and drift management.
...
### [78] [A Fast and Minimal System to Identify Depression Using Smartphones: Explainable Machine Learning-Based Approach](https://arxiv.org/abs/2508.18301)
*Md Sabbir Ahmed,Nova Ahmed*

Main category: cs.LG

TL;DR: A fast, minimalistic app-usage–based ML approach detects depression within 7 days of data, retrieved in about 1 second, achieving around 82% identification of depressed students with interpretable features.


<details>
  <summary>Details</summary>
Motivation: There is a need for early depression screening that works quickly and with limited data, especially in underdeveloped regions where long-term monitoring is impractical.

Method: Collected app-usage data over the past 7 days from 100 Bangladeshi students; developed a fast data retrieval tool (~1 s). Built multiple ML models; used stable feature selection and three FS approaches; employed a light gradient boosting machine (LGBM) with stable features; also built a parsimonious stacking model using around 5 Boruta-selected features per iteration; used SHAP for interpretation.

Result: Best LGBM with stable features identified 82.4% of depressed students (precision 75%, F1 78.5%). The stacking model achieved up to 77.4% precision and 77.9% balanced accuracy using Boruta-selected features. SHAP analysis revealed interpretable behavioral markers linked to depression.

Conclusion: The fast, minimalistic approach shows potential for depression detection in resource-constrained settings and supports the development of low-cost systems for monitoring student mental health, with implications for early intervention.
...
### [79] [Learning Explainable Imaging-Genetics Associations Related to a Neurological Disorder](https://arxiv.org/abs/2508.18303)
*Jueqi Wang,Zachary Jacokes,John Darrell Van Horn,Michael C. Schatz,Kevin A. Pelphrey,Archana Venkataraman*

Main category: cs.LG

TL;DR: NeuroPathX is an explainable deep learning framework that fuses MRI-derived brain structure variations with genetics-derived pathway representations via cross-attention, using sparsity and pathway-similarity losses to improve interpretability and robustness, and achieving superior performance on autism and Alzheimer's disease.


<details>
  <summary>Details</summary>
Motivation: To move beyond simplistic linear models and opaque black-box approaches in imaging-genetics by enabling interpretable modeling of interactions between brain structure and genetic pathways through attention-based fusion.

Method: An early-fusion deep learning model that uses cross-attention to integrate structural MRI features with pathway-level genetic representations. It imposes a sparsity loss on the attention matrix to emphasize salient interactions and a pathway similarity loss to enforce consistent representations across the cohort. Validated on autism spectrum disorder and Alzheimer's disease datasets; code released.

Result: The approach outperforms competing baselines and uncovers biologically plausible associations related to the disorders, demonstrating improved predictive performance and interpretability over existing methods.

Conclusion: Explainable cross-modal fusion via attention can capture meaningful brain-genetic interactions, yielding robust, interpretable insights into complex brain disorders and potentially guiding biomedical understanding and discovery.
...
### [80] [SALMAN: Stability Analysis of Language Models Through the Maps Between Graph-based Manifolds](https://arxiv.org/abs/2508.18306)
*Wuxinlin Cheng,Yupeng Cao,Jinwen Wu,Koduvayur Subbalakshmi,Tian Han,Zhuo Feng*

Main category: cs.LG

TL;DR: Proposes SALMAN, a model-agnostic, sample-level robustness framework for transformers using Distance Mapping Distortion to judge and improve input perturbation resilience without changing model parameters.


<details>
  <summary>Details</summary>
Motivation: As NLP transformers scale, robustness under input perturbations becomes critical. Existing methods differ for small vs large models and often require labor-intensive, adversarial perturbations; a unified, lightweight, sample-level approach is needed.

Method: Introduce SALMAN, a local robustness framework that evaluates stability at the sample level without modifying internal parameters or relying on complex perturbations. It centers on Distance Mapping Distortion (DMD), which ranks each sample’s susceptibility by comparing input-to-output distance mappings with near-linear computational complexity.

Result: Demonstrates improved attack efficiency and more effective robust training, showing SALMAN as a practical, model-agnostic tool for enhancing the reliability of transformer-based NLP systems.

Conclusion: SALMAN provides a practical, model-agnostic framework for assessing and improving robustness of transformer-based NLP models without altering their parameters.
...
### [81] [Learning Spatio-Temporal Dynamics via Operator-Valued RKHS and Kernel Koopman Methods](https://arxiv.org/abs/2508.18307)
*Mahishanka Withanachchi*

Main category: cs.LG

TL;DR: A unified, nonparametric framework that blends operator-valued RKHS with kernel Koopman methods to learn spatio-temporal vector-field dynamics, with representer theorems, Sobolev-type bounds, and spectral convergence for reliable long-horizon prediction and reduced-order modeling.


<details>
  <summary>Details</summary>
Motivation: Modeling time-evolving, high-dimensional vector-valued fields is challenging due to nonlinear dynamics and the need to preserve spatial/temporal structure. Existing parametric or scalar-embedded approaches struggle with structure preservation and guarantees. The work proposes a nonparametric framework combining OV-RKHS and kernel Koopman to enable data-driven, provably consistent learning of spatio-temporal dynamics.

Method: Introduce a unified OV-RKHS framework and couple it with kernel-based Koopman operator theory. Establish representer theorems for time-dependent OV-RKHS interpolation, derive Sobolev-type approximation bounds for smooth vector fields, and provide spectral convergence guarantees for kernel Koopman approximations, enabling efficient reduced-order modeling and long-term prediction.

Result: The paper provides theoretical guarantees: representer theorems, Sobolev-type approximation bounds, and spectral convergence guarantees for kernel Koopman operator approximations, forming a scalable, nonparametric toolset for forecasting, control, and uncertainty quantification in spatio-temporal ML.

Conclusion: By uniting OV-RKHS with kernel Koopman methods, the framework delivers a theoretically grounded, nonparametric approach for forecasting and controlling high-dimensional spatio-temporal systems, with robust convergence guarantees and applicability to uncertainty quantification and ROM in spatio-temporal machine learning.
...
### [82] [CoPE: A Lightweight Complex Positional Encoding](https://arxiv.org/abs/2508.18308)
*Avinash Amballa*

Main category: cs.LG

TL;DR: CoPE introduces a complex-valued positional encoding for transformers, combining content (real part) and position (imaginary part) with phase-aware attention in the first layer; claims no long-term decay and compatibility with linear attention, achieving superior GLUE performance with lower computational cost than RoPE, sinusoidal, and learned encodings.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of standard positional encodings by encoding both semantic content and positional information in a single complex embedding, enabling position-aware modeling with potential efficiency benefits.

Method: Replace traditional positional encodings with complex-valued embeddings where the real part encodes content and the imaginary part encodes position; add phase-aware attention in the first Transformer layer to capture position-dependent patterns, followed by standard attention layers; designed to avoid long-term decay and to be compatible with linear attention.

Result: On GLUE, CoPE achieves superior performance with lower computational complexity compared to RoPE, sinusoidal, and learned positional encodings, and  exhibits no long-term decay of positional information.

Conclusion: CoPE provides a lightweight, decay-free complex-valued positional encoding that integrates content and position, is compatible with linear attention, and improves Transformer performance on GLUE.
...
### [83] [What Matters in Data for DPO?](https://arxiv.org/abs/2508.18312)
*Yu Pan,Zhongze Cai,Guanting Chen,Huaiyang Zhong,Chonghuan Wang*

Main category: cs.LG

TL;DR: DPO performance is driven mainly by the quality of chosen responses; rejected-response quality has limited impact; contrastiveness helps primarily by improving chosen samples; online DPO effectively reduces to supervised fine-tuning on the chosen responses; mixing on-policy data can be beneficial; provides practical guidance for constructing high-impact preference datasets.


<details>
  <summary>Details</summary>
Motivation: To understand which characteristics of human preference data are most critical for DPO, addressing a gap between theory and practice in LLM alignment.

Method: Theoretical characterization of the optimal response distribution under DPO; analysis of how contrastiveness affects learning; extension to online DPO showing it reduces to supervised fine-tuning on chosen responses; extensive empirical experiments across diverse tasks; examination of mixing on-policy data.

Result: Quality of chosen responses dominates performance; the quality of rejected responses has limited effect; contrastiveness improves learning mainly by aiding the chosen samples; online DPO behaves like supervised fine-tuning on chosen responses; mixing on-policy data yields benefits; findings align with and explain practical data-collection strategies.

Conclusion: Provides practical guidelines for constructing impactful preference datasets for LLM alignment and clarifies the mechanisms behind common strategies used in DPO and preference-based optimization.
...
### [84] [ProtoEHR: Hierarchical Prototype Learning for EHR-based Healthcare Predictions](https://arxiv.org/abs/2508.18313)
*Zi Cai,Yu Liu,Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: ProtoEHR is an interpretable hierarchical prototype learning framework for EHR data that captures relationships across codes, visits, and patients via a medical knowledge graph and prototypes, enabling accurate and interpretable predictions across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Current EHR-based approaches largely model isolated components, limiting predictive performance and interpretability; there is a need to exploit multi-level EHR structure and semantic code relations.

Method: Construct a medical knowledge graph by applying LLMs to derive semantic relationships among codes; build a hierarchical representation learning framework across codes, visits, and patients; incorporate prototypes at each level to capture intrinsic similarities and improve generalization; evaluate on two public datasets for five tasks (mortality, readmission, length of stay, drug recommendation, phenotype).

Result: ProtoEHR demonstrates accurate, robust, and interpretable predictions compared to baselines, with interpretability at code, visit, and patient levels providing insights aligned with clinical reasoning.

Conclusion: Leveraging multi-level EHR structure and prototype-based representations can enhance predictive performance and interpretability, offering actionable insights for clinicians and healthcare systems.
...
### [85] [Evaluating Federated Learning for At-Risk Student Prediction: A Comparative Analysis of Model Complexity and Data Balancing](https://arxiv.org/abs/2508.18316)
*Rodrigo Tertulino*

Main category: cs.LG

TL;DR: Federated learning on OULAD data to predict at-risk students; LR vs DNN; ~85% ROC AUC; data balancing; privacy-preserving early-warning system.


<details>
  <summary>Details</summary>
Motivation: Address high dropout rates and data privacy concerns in distance education by enabling scalable, privacy-preserving risk prediction through FL across institutions.

Method: Train a federated learning model using early academic performance and digital engagement features from OULAD; compare Logistic Regression and Deep Neural Network; experiment with data balancing.

Result: Federated model achieves ROC AUC ~0.85 for identifying at-risk students; demonstrates practical, scalable privacy-preserving early-warning capability.

Conclusion: Federated learning enables effective, privacy-respecting early-warning systems for institutions, balancing predictive performance with data governance.
...
### [86] [ZTFed-MAS2S: A Zero-Trust Federated Learning Framework with Verifiable Privacy and Trust-Aware Aggregation for Wind Power Data Imputation](https://arxiv.org/abs/2508.18318)
*Yang Li,Hanjie Wang,Yuanzheng Li,Jiazheng Li,Zhaoyang Dong*

Main category: cs.LG

TL;DR: Proposes ZTFed-MAS2S, a zero-trust federated learning framework with a MAS2S imputation model, integrating verifiable privacy and secure parameter exchange, robust trust-aware aggregation, and compression, achieving strong missing-data imputation and FL performance on real wind data.


<details>
  <summary>Details</summary>
Motivation: Wind power data suffer from missing values and sensor/communication faults. Federated learning preserves privacy but is vulnerable to anomalous updates and privacy leakage, especially in open industrial environments, necessitating zero-trust, verifiable privacy protections.

Method: Introduce ZTFed-MAS2S: a zero-trust FL framework that uses a multi-head attention-based sequence-to-sequence imputation model (MAS2S); combines verifiable differential privacy with non-interactive zero-knowledge proofs; includes confidentiality and integrity verification for model transmissions; employs a dynamic, trust-aware aggregation on similarity graphs; and uses sparsity- and quantization-based compression to lower communication costs.

Result: Empirical results on real-world wind-farm datasets show that ZTFed-MAS2S achieves superior federated learning performance and missing-data imputation quality, demonstrating strong security, privacy, and efficiency advantages for practical energy-sector applications.

Conclusion: ZTFed-MAS2S provides a secure, efficient, and accurate zero-trust FL solution for wind data imputation in open industrial settings, combining verifiable privacy with robust aggregation and compression.
...
### [87] [Linear cost mutual information estimation and independence test of similar performance as HSIC](https://arxiv.org/abs/2508.18338)
*Jarek Duda,Jagoda Bracha,Adrian Przybysz*

Main category: cs.LG

TL;DR: HCR is a scalable linear-time alternative to HSIC for dependence testing, offering higher sensitivity and a joint distribution model via mixed moments; complexity scales with n linearly for a single feature and with dimension d for multiple features (O(d^2) pairwise, O(d^3) triplewise).


<details>
  <summary>Details</summary>
Motivation: To overcome HSIC's quadratic/quasi-quadratic computational cost on large datasets while preserving or improving sensitivity and providing a usable joint distribution description.

Method: Hierarchical Correlation Reconstruction: build dependence descriptors from mixed moments (starting with correlation and homoscedasticity), compute single-dependence features in O(n); approximate mutual information as sum of squares of nontrivial mixed moments; test dependencies across dimensions by expanding to pairwise, triplewise, etc., with costs O(d^2), O(d^3), etc.

Result: Single dependence describing feature computed in O(n); higher sensitivity tests; joint distribution model via descriptive features; mutual information approximated by sum of squares of mixed moments.

Conclusion: HCR offers scalable, linear-cost dependence testing with the ability to represent joint distributions and capture higher-order dependencies; performance depends on dimensionality due to combinatorial growth in testing combinations.
...
### [88] [DualSparse-MoE: Coordinating Tensor/Neuron-Level Sparsity with Expert Partition and Reconstruction](https://arxiv.org/abs/2508.18376)
*Weilin Cai,Le Qin,Shwai He,Junwei Cui,Ang Li,Jiayi Huang*

Main category: cs.LG

TL;DR: Post-training partitioning creates tensor- and neuron-level sparsity in MoE; DualSparse-MoE achieves efficient inference with minor accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To enable efficient deployment of Mixture of Experts by achieving both tensor- and neuron-level sparsity without retraining, addressing efficiency and accuracy challenges in large MoE models.

Method: Post-training expert partitioning to induce tensor- and neuron-level sparsity; DualSparse-MoE inference system with dynamic tensor-level computation dropping and static neuron-level reconstruction; load-imbalance aware expert parallelism.

Result: Approximately 25% drop rate yields ~0.08-0.28% average accuracy loss across three MoE models; computation dropping yields proportional speedups; adding load-imbalance aware parallelism yields 1.41x MoE module speedup with 0.5% average accuracy degradation.

Conclusion: Post-training sparsity and integrated dynamic/static strategies can deliver substantial efficiency gains with negligible accuracy impact; preserves mathematical consistency and reduces retraining needs.
...
### [89] [Low-Rank Tensor Decompositions for the Theory of Neural Networks](https://arxiv.org/abs/2508.18408)
*Ricardo Borsoi,Konstantin Usevich,Marianne Clausel*

Main category: cs.LG

TL;DR: A survey linking low-rank tensor decompositions to deep neural network theory, highlighting how tensor methods illuminate expressivity, learnability, hardness, generalization, and identifiability, with an emphasis on a unified, cross-disciplinary view.


<details>
  <summary>Details</summary>
Motivation: Deep learning lacks a solid mathematical grounding; low-rank tensors offer interpretable factor structures and polynomial-time algorithms with strong uniqueness guarantees, making them well-suited to analyze NN behavior.

Method: Review and synthesize existing literature across computer science and mathematics, illustrating connections between tensor decompositions and NN properties, and proposing a unified framework.

Result: A coherent survey that clarifies how low-rank tensor methods explain NN expressivity, algorithmic learnability, computational hardness, generalization, and identifiability, and promotes cross-domain viewpoints.

Conclusion: Low-rank tensor decompositions are a fundamental tool for theory of deep NNs, and fostering broader cross-disciplinary use can advance understanding of neural networks.
...
### [90] [LLM-Driven Intrinsic Motivation for Sparse Reward Reinforcement Learning](https://arxiv.org/abs/2508.18420)
*André Quadros,Cassio Silva,Ronnie Alves*

Main category: cs.LG

TL;DR: A hybrid intrinsic reward framework combining VSIMR and LLM-derived signals improves exploration and goal-directed learning in sparse-reward RL, outperforming individual signals and standard A2C in MiniGrid DoorKey.


<details>
  <summary>Details</summary>
Motivation: Sparse rewards slow reinforcement learning; leveraging complementary intrinsic motivations—state novelty and language-guided guidance—can accelerate learning.

Method: Integrate Variational State as Intrinsic Reward (VSIMR) with intrinsic rewards from Large Language Models. Implemented with an A2C agent in MiniGrid DoorKey; compare against single strategies and standard A2C.

Result: Combined approach yields higher performance and sample efficiency; learning curves show VSIMR drives exploration while LLM rewards enable progression toward goals; standard A2C fails.

Conclusion: Combining complementary intrinsic motivations is effective for sparse-reward RL and could generalize to other domains; synergy between exploration-driven and knowledge-driven signals.
...
### [91] [Enhancing Trust-Region Bayesian Optimization via Newton Methods](https://arxiv.org/abs/2508.18423)
*Quanlin Chen,Yiyu Chen,Jing Huo,Tianyu Ding,Yang Gao,Yuetong Chen*

Main category: cs.LG

TL;DR: A method to improve high-dimensional Bayesian optimization by combining TuRBO with local quadratic models derived from a global GP and bound-constrained quadratic programming to select samples; also tackles GP gradient vanishing; shows convergence and empirical gains over many BO methods.


<details>
  <summary>Details</summary>
Motivation: High-dimensional BO scaling is difficult. Local GPs in TuRBO enable heterogeneous modeling but can be less sample-efficient than a global GP. A method that retains heterogeneity while improving sampling efficiency is desired. Additionally, GP gradients can vanish in high dimensions, hindering optimization. The paper proposes leveraging a global GP to build local quadratic models and uses a QP to propose samples.

Method: Construct multiple local quadratic models in each trust region using gradient and Hessian information derived from a global GP; select next sample points by solving a bound-constrained quadratic program; address vanishing GP gradients in high-dimensional spaces; provide a convergence analysis; empirically evaluate against existing high-dimensional BO techniques on synthetic and real-world tasks.

Result: Convergence analysis is provided; experiments show enhanced efficacy of TuRBO and that the proposed method outperforms a wide range of high-dimensional BO techniques on synthetic functions and real-world applications.

Conclusion: The approach preserves TuRBO’s heterogeneous local modeling while improving sampling efficiency and scalability in high-dimensional spaces, yielding better empirical performance and theoretical guarantees.
...
### [92] [VERIRL: Boosting the LLM-based Verilog Code Generation via Reinforcement Learning](https://arxiv.org/abs/2508.18462)
*Fu Teng,Miao Pan,Xuhong Zhang,Zhezhi He,Yiyao Yang,Xinyi Chai,Mengnan Qi,Liqiang Lu,Jianwei Yin*

Main category: cs.LG

TL;DR: Proposes VeriRL, an RL-based framework for Verilog code generation, featuring Veribench-53K, a Trace-back based Rescore mechanism, and a sample-balanced weighting strategy, achieving state-of-the-art Verilog generation and opening-source release.


<details>
  <summary>Details</summary>
Motivation: HDLs like Verilog have challenging concurrency semantics, rigid syntax, and complex simulation, making code generation difficult; existing approaches suffer from sparse/noisy rewards and limited high-quality data, hindering RL in hardware-centric domains.

Method: Build Veribench-53K from 700K Verilog problems with structured prompts, complexity labels, and diverse testbenches; introduce Trace-back based Rescore to improve feedback via reasoning paths and iterative refinement; apply a sample-balanced weighting strategy to stabilize RL fine-tuning by balancing reward-probability distributions; deploy an iterative RL pipeline that co-evolves policy and reward models.

Result: Demonstrates state-of-the-art performance on Verilog generation tasks with notable gains in test pass rate, functional correctness, and compilation robustness; VeriRL is publicly available.

Conclusion: RL-driven approaches hold strong promise for structured code generation in hardware-centric domains; this work showcases effective data- and feedback-driven RL for HDL generation and provides a valuable open-source resource.
...
### [93] [DRTA: Dynamic Reward Scaling for Reinforcement Learning in Time Series Anomaly Detection](https://arxiv.org/abs/2508.18474)
*Bahareh Golchin,Banafsheh Rekabdar,Kunpeng Liu*

Main category: cs.LG

TL;DR: A reinforcement-learning-based anomaly detector (DRTA) that uses dynamic reward shaping, a Variational Autoencoder (VAE), and active learning to handle low-label data, outperforming baselines on Yahoo A1/A2 datasets.


<details>
  <summary>Details</summary>
Motivation: Tackling limited labeled data, high false-positive rates, and poor generalization to novel anomaly types in time-series anomaly detection.

Method: A DRTA framework that adaptively scales VAE-based reconstruction error and classification rewards through dynamic reward shaping, integrating VAE representation learning with reinforcement learning and active learning to guide anomaly detection.

Result: On Yahoo A1 and Yahoo A2 benchmarks, DRTA consistently outperforms state-of-the-art unsupervised and semi-supervised methods, showing strong precision and recall in low-label settings.

Conclusion: DRTA is a scalable and efficient solution for real-world anomaly detection tasks that benefits from combining RL with representation learning and active learning to handle limited labels and evolving anomaly types.
...
### [94] [Data Augmentation Improves Machine Unlearning](https://arxiv.org/abs/2508.18502)
*Andreza M. C. Falcao,Filipe R. Cordeiro*

Main category: cs.LG

TL;DR: Augmentation design significantly boosts machine unlearning effectiveness, narrowing the gap to retraining; TrivialAug achieves up to 40.12% improvement in the average-gap unlearning metric on CIFAR-10/100.


<details>
  <summary>Details</summary>
Motivation: Despite suspected links between memorization and augmentation, systematic augmentation design in machine unlearning has been under-explored. This work investigates how augmentation strategies impact unlearning performance and privacy-preserving efficiency.

Method: Evaluate multiple augmentation strategies (including TrivialAug augmentation) across unlearning methods SalUn, Random Label, and Fine-Tuning, on CIFAR-10 and CIFAR-100 with varying forget rates; measure the 'Average Gap' against retrained models.

Result: Proper augmentation design markedly improves unlearning effectiveness, reducing the performance gap to retrained models; the reported best case shows a reduction of the Average Gap unlearning Metric by up to 40.12% using TrivialAug.

Conclusion: Augmentation not only helps mitigate memorization but is also crucial for achieving privacy-preserving and efficient unlearning; thoughtful augmentation design should be a central consideration in MU research.
...
### [95] [Breaking Through Barren Plateaus: Reinforcement Learning Initializations for Deep Variational Quantum Circuits](https://arxiv.org/abs/2508.18514)
*Yifeng Peng,Xinyi Li,Zhemin Zhang,Samuel Yen-Chi Chen,Zhiding Liang,Ying Wang*

Main category: cs.LG

TL;DR: RL-based initialization reshapes the VQA parameter landscape to avoid barren plateaus, speeding convergence and improving final solutions.


<details>
  <summary>Details</summary>
Motivation: Barren plateaus cause vanishing gradients in VQAs as system size/depth grows, hindering training. An effective initialization strategy could substantially improve trainability and scalability.

Method: Train RL agents (including DPG, SAC, and PPO) to output initial circuit parameters (actions) that minimize the VQA cost before standard gradient-based optimization, then fine-tune with conventional optimizers. Compare multiple RL algorithms and test under noise across tasks.

Result: RL-based initialization consistently improves convergence speed and final cost across tasks and noise conditions; several RL algorithms yield comparable gains, indicating robustness and flexibility of the approach.

Conclusion: RL-driven parameter initialization is a promising, flexible tool for accelerating and stabilizing VQAs, potentially enhancing their scalability and real-world deployment.
...
### [96] [Quantifying The Limits of AI Reasoning: Systematic Neural Network Representations of Algorithms](https://arxiv.org/abs/2508.18526)
*Anastasis Kratsios,Dennis Zvigelsky,Bradd Hart*

Main category: cs.LG

TL;DR: A universal meta-algorithm shows neural networks with ReLU activations can exactly emulate essentially any circuit, tying neural capacity to circuit complexity and enabling exact, overflow-free emulation of diverse reasoning tasks, with a trade-off between run-time and network size.


<details>
  <summary>Details</summary>
Motivation: To quantify what forms of reasoning neural networks can perform when perfectly trained and to formalize circuit-based reasoning as a framework for understanding NN capabilities beyond approximate function approximation.

Method: Provide a systematic meta-algorithm that converts almost any circuit into a feedforward neural network with ReLU activations by iteratively replacing each gate with a canonical ReLU MLP emulator; the resulting network exactly emulates the original circuit on digital hardware and mirrors the circuit's structure, with neuron count reflecting circuit complexity.

Result: The construction achieves exact emulation with no approximation or rounding (including modular overflow behavior); demonstrates that no reasoning task is beyond NN reach; characterizes a space-time trade-off: network size grows with circuit complexity while runtime matches the circuit; derives applications such as cubic-size NNs for shortest-path algorithms, quadratically large NNs for stopped Turing machines, and emulation of randomized Boolean circuits; shows that any universal function approximator can be encoded as a circuit and directly emulated by an NN, making the result strictly stronger than the universal approximation theorem.

Conclusion: Neural networks can simulate essentially any circuit-based reasoning task, providing a principled bridge between circuit complexity and neural resources, and highlighting a fundamental space–time trade-off in neural computation across logical, dynamic-programming, and symbolic domains.
...
### [97] [BTW: A Non-Parametric Variance Stabilization Framework for Multimodal Model Integration](https://arxiv.org/abs/2508.18551)
*Jun Hou,Le Wang,Xuan Wang*

Main category: cs.LG

TL;DR: BTW is a bi-level, non-parametric weighting framework for multimodal MoE that uses per-example KL divergence and modality-level mutual information to weight modalities without extra parameters; scalable to many modalities and improves sentiment regression and clinical classification.


<details>
  <summary>Details</summary>
Motivation: MoE can be harmed when extra modalities introduce noise; existing approaches like Partial Information Decomposition struggle to scale beyond two modalities and lack instance-level control. A scalable, parameter-free method for per-example and modality-wise weighting is needed.

Method: A bi-level weighting scheme: (1) per-example KL weights derived from the divergence between unimodal predictions and the current multimodal prediction to capture instance-level discrepancies; (2) modality-wide MI weights estimating global alignment between unimodal and multimodal outputs. The method is non-parametric, adds no new parameters, and applies to any number of modalities, to be integrated into MoE training.

Result: Experiments on sentiment regression and clinical classification show significant improvements in regression performance and multiclass classification accuracy.

Conclusion: BTW offers a scalable, parameter-free approach for dynamic modality weighting in MoE, enabling better utilization of multiple modalities even when some are noisy, and is applicable across tasks with varying modality counts.
...
### [98] [Enhancing Chemical Explainability Through Counterfactual Masking](https://arxiv.org/abs/2508.18561)
*Łukasz Janisiów,Marek Kochańczyk,Bartosz Zieliński,Tomasz Danel*

Main category: cs.LG

TL;DR: Counterfactual masking replaces masked molecular substructures with chemically plausible fragments sampled from generative models to create distribution-consistent explanations, improving realism and actionability of model explanations in molecular property prediction.


<details>
  <summary>Details</summary>
Motivation: Existing explainability methods rely on masking that often yields implausible, zeroed-out baselines and unintuitive explanations. There is a need for realism-aligned, actionable explanations that respect the underlying molecular distribution and support useful design insights.

Method: Introduce counterfactual masking: replace masked substructures with fragments sampled from generative models trained to complete molecular graphs, creating counterfactual molecules drawn from the data distribution. Evaluate explanations against these distribution-consistent counterfactuals and demonstrate robustness and actionability across datasets.

Result: The approach yields more realistic and robust explanations, improves benchmarking of explainers, and provides meaningful counterfactuals that indicate how structural changes affect predicted properties across multiple datasets and tasks.

Conclusion: Counterfactual masking offers a principled, generative path to explainable ML in chemistry, bridging explainability with molecular design by leveraging distribution-consistent counterfactuals for actionable insights.
...
### [99] [A Note on Graphon-Signal Analysis of Graph Neural Networks](https://arxiv.org/abs/2508.18564)
*Levi Rauchwerger,Ron Levie*

Main category: cs.LG

TL;DR: Extends graphon-signal GNN theory to multidimensional signals, readout with cut distance, robustness-based generalization bounds, and non-symmetric graphons/kernels, addressing prior practical gaps.


<details>
  <summary>Details</summary>
Motivation: The prior work by Levy established key generalization and sampling results but omitted aspects essential for real-world GNNs; this paper aims to make the theory more applicable by covering multidimensional signals, readout impact, robust generalization, and non-symmetric graphons/kernels.

Method: Theoretical extensions: (1) generalize core results to multidimensional graphon-signals; (2) generalize Lipschitz continuity to MPNNs with readout w.r.t cut distance; (3) derive robustness-based generalization bounds; (4) extend analysis to non-symmetric graphons and kernels.

Result: New bounds and guarantees holding under broader conditions; improved generalization bounds; extended framework enabling practical analysis of GNNs on a wider class of graph signals and structures.

Conclusion: The paper fills missing ingredients, broadens the applicability of graphon-signal analysis for GNNs, and provides stronger theoretical guarantees for generalization and sampling in more realistic settings.
...
### [100] [Improving Long-term Autoregressive Spatiotemporal Predictions: A Proof of Concept with Fluid Dynamics](https://arxiv.org/abs/2508.18565)
*Hao Zhou,Sibo Cheng*

Main category: cs.LG

TL;DR: SPF is a data-driven forecasting framework that preserves one-step training but enables multi-step learning via a stochastic PushForward approach, improving long-term accuracy with lower memory usage.


<details>
  <summary>Details</summary>
Motivation: Autoregressive data-driven forecasts suffer from error accumulation and high memory demands; need memory-efficient methods that maintain short-term accuracy while improving long-term performance in complex systems.

Method: SPF constructs a supplementary dataset from model predictions and ground truth, uses a stochastic acquisition strategy to mix them, precomputes multi-step predictions between epochs to keep memory stable, enabling multi-step learning without storing full unrolled sequences.

Result: On Burgers' equation and Shallow Water benchmark, SPF outperforms autoregressive methods in long-term accuracy while reducing memory usage.

Conclusion: SPF demonstrates promise for resource-limited and complex simulations by balancing short- and long-term performance and mitigating overfitting.
...
### [101] [Sparse Autoencoders for Low-$N$ Protein Function Prediction and Design](https://arxiv.org/abs/2508.18567)
*Darin Tsui,Kunal Talreja,Amirali Aghazadeh*

Main category: cs.LG

TL;DR: Sparse autoencoders (SAEs) on fine-tuned ESM2 embeddings enable better low-N protein-function prediction and design than ESM2 baselines, with strong performance from as few as 24 sequences; steering predictive latents captures motifs leading to top-fitness variants in 83% of cases.


<details>
  <summary>Details</summary>
Motivation: Address data scarcity in protein function prediction and design. Leverage protein language model embeddings (pLMs) and interpretable sparse latent representations to generalize from limited labeled sequence-function data.

Method: Fine-tune ESM2 embeddings, train sparse autoencoders to decompose embeddings into sparse latent variables; evaluate on diverse fitness extrapolation and protein-engineering tasks; compare against ESM2 baselines; explore steering of latent representations to exploit biological motifs.

Result: SAEs trained on 24+ sequences consistently outperform or compete with ESM2 baselines for fitness prediction; the sparse latent space encodes compact, biologically meaningful representations that generalize well under data scarcity; steering predictive latents identifies top-fitness variants in 83% of cases.

Conclusion: Sparse latent representations learned from fine-tuned pLM embeddings improve low-N function prediction and design, offering better generalization from limited data and actionable motifs for protein engineering.
...
### [102] [DrugReasoner: Interpretable Drug Approval Prediction with a Reasoning-augmented Language Model](https://arxiv.org/abs/2508.18579)
*Mohammadreza Ghaffarzadeh-Esfahani,Ali Motahharynia,Nahid Yousefian,Navid Mazrouei,Jafar Ghaisari,Yousof Gheisari*

Main category: cs.LG

TL;DR: DrugReasoner is a GRPO-finetuned LLaMA-based reasoning LLM that combines molecular descriptors with comparative reasoning over similar compounds to predict small-molecule approval, offering predictions with rationales and achieving competitive AUC/F1 across validation, test, and an external dataset.


<details>
  <summary>Details</summary>
Motivation: Drug discovery is costly and time-consuming; early prediction of approval outcomes can optimize research investments. Interpretability is a major bottleneck for ML approaches in this domain, limiting adoption.

Method: A reasoning-based LLM built on the LLaMA architecture and fine-tuned with group relative policy optimization (GRPO). It integrates molecular descriptors with comparative reasoning against structurally similar approved and unapproved compounds. Outputs include probabilistic predictions, step-by-step rationales, and confidence scores. Evaluated on internal validation and test sets, and on an external independent dataset, with comparisons to traditional ML baselines (logistic regression, SVM, kNN), XGBoost, and ChemAP.

Result: Internal validation AUC 0.732, F1 0.729; test AUC 0.725, F1 0.718. Outperformed baselines such as logistic regression, SVM, and kNN, and competitive with XGBoost. On an external dataset, achieved AUC 0.728 and F1 0.774, outperforming both baseline and ChemAP, indicating robustness and practical applicability with high precision and balanced sensitivity.

Conclusion: DrugReasoner demonstrates that reasoning-augmented LLMs can deliver competitive predictive accuracy while providing interpretable step-by-step rationales, addressing interpretability bottlenecks in AI-assisted drug discovery and highlighting the potential for decision-support in pharmaceutical research.
...
### [103] [History Rhymes: Accelerating LLM Reinforcement Learning with RhymeRL](https://arxiv.org/abs/2508.18588)
*Jingkai He,Tianjian Li,Erhu Feng,Dong Du,Qian Liu,Tao Liu,Yubin Xia,Haibo Chen*

Main category: cs.LG

TL;DR: RhymeRL speeds up RL-based training of LLMs by exploiting similarity in rollout responses across epochs, using speculative decoding (HistoSpec) and distribution-aware two-tier scheduling (HistoPipe) to reduce GPU underutilization, achieving 2.6x faster training in production without changing the RL paradigm or sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: RL for LLMs comprises rollout, reward, and training stages. Rollout is the bottleneck due to test-time scaling and batch-length imbalances cause GPU bubbles. Existing asynchronous/truncation strategies can hurt accuracy. The authors observe that rollout responses are highly similar across adjacent training epochs and leverage this to accelerate training.

Method: HistoSpec: speculative decoding that uses historical rollout token sequences to draft accurate rollouts. HistoPipe: two-tier scheduling that uses historical rollout distributions to balance workload among rollout workers, mitigating GPU bubbles.

Result: In real production, RhymeRL scales from dozens to thousands of GPUs and delivers a 2.6x performance improvement over existing methods without compromising accuracy or altering the RL paradigm.

Conclusion: RhymeRL offers a practical, scalable approach to accelerate RL for LLMs by exploiting rollout similarity with speculative decoding and distribution-aware scheduling, reducing GPU underutilization while preserving training accuracy.
...
### [104] [Linear Trading Position with Sparse Spectrum](https://arxiv.org/abs/2508.18596)
*Zhao-Rong Lai,Haisheng Yang*

Main category: cs.LG

TL;DR: Introduces a sparse-spectrum linear trading position to diversify principal portfolios and explore a larger spectral region of the prediction matrix, optimized with a Krasnosel'skii-Mann fixed-point algorithm that has descent and linear convergence; validates robustness across scenarios.


<details>
  <summary>Details</summary>
Motivation: Principal portfolios can be under-diversified and fragile to market conditions; expanding spectral exploration and providing a convergence-guaranteed optimization method aims to improve diversification and robustness in signal-based trading.

Method: Define a linear trading position with a sparse spectral representation; optimize via Krasnosel'skii-Mann fixed-point iterations, leveraging a descent property and proving linear convergence in the objective; perform extensive experimental validation.

Result: The proposed method achieves good and robust performance across varied conditions as demonstrated by extensive experiments.

Conclusion: A novel, robust trading-position framework with sparse spectrum and a provable convergence property; offers enhanced diversification and resilience over traditional principal portfolios.
...
### [105] [Uncertainty Awareness on Unsupervised Domain Adaptation for Time Series Data](https://arxiv.org/abs/2508.18630)
*Weide Liu,Xiaoyang Zhong,Lu Wang,Jingwen Hou,Yuemei Luo,Jiebin Yan,Yuming Fang*

Main category: cs.LG

TL;DR: Unsupervised domain adaptation for time series using a multi-scale mixed-input architecture and an uncertainty-aware mechanism based on evidential learning with a Dirichlet prior, achieving state-of-the-art generalization and better calibration.


<details>
  <summary>Details</summary>
Motivation: Time-series data commonly exhibit distribution shifts between training and testing. This leads to degraded performance and poorly calibrated predictions. A method that enhances cross-domain feature alignment while providing reliable uncertainty estimates can improve generalization across domains.

Method: 1) Introduce a multi-scale mixed-input architecture to capture features at different temporal scales, increasing training diversity and reducing feature discrepancies between source and target domains. 2) Add an uncertainty-awareness mechanism based on evidential learning by imposing a Dirichlet prior on class probabilities to enable both precise target predictions and uncertainty estimation. This mechanism aligns features with identical labels across domains to aid domain adaptation and yields better-calibrated predictions.

Result: The approach yields significant improvements in the target domain on benchmark time-series datasets, achieving state-of-the-art performance in unsupervised domain adaptation. It also results in a lower Expected Calibration Error (ECE), indicating better calibration of prediction confidence.

Conclusion: Combining a multi-scale feature extractor with an uncertainty-aware evidential learning framework effectively improves unsupervised domain adaptation for time series, delivering robust cross-domain generalization and well-calibrated predictions across diverse benchmarks.
...
### [106] [STRATA-TS: Selective Knowledge Transfer for Urban Time Series Forecasting with Retrieval-Guided Reasoning](https://arxiv.org/abs/2508.18635)
*Yue Jiang,Chenxi Liu,Yile Chen,Qin Chao,Shuai Liu,Gao Cong*

Main category: cs.LG

TL;DR: STRATA-TS introduces selective, target-aware transfer for time-series forecasting under data scarcity. By retrieving semantically aligned source subsequences and using an LLM for structured reasoning, it improves urban forecasting (parking availability) across three cities and distills the reasoning into a compact model for deployment.


<details>
  <summary>Details</summary>
Motivation: Urban forecasting suffers from data imbalance: few cities have dense histories while many have sparse or incomplete data. Naive transfer from data-rich to data-scarce cities risks negative transfer and noise. There is a need for target-aware retrieval and reasoning to leverage useful cross-city patterns without harming performance.

Method:  STRATA-TS uses a patch-based temporal encoder to identify source subsequences aligned with the target query. Retrieved exemplars are injected into a retrieval-guided reasoning stage where an LLM performs structured inference over target inputs and retrieved support. For deployment efficiency, the reasoning is distilled into a compact open model via supervised fine-tuning.

Result: Empirical validation on three parking availability datasets (Singapore, Nottingham, Glasgow) shows STRATA-TS consistently outperforms strong forecasting and transfer baselines and provides interpretable knowledge transfer pathways.

Conclusion: STRATA-TS provides an effective framework for selective transfer in scarce data regimes, delivering performance gains and interpretable transfer routes, while enabling efficient deployment through distillation.
...
### [107] [Biologically Disentangled Multi-Omic Modeling Reveals Mechanistic Insights into Pan-Cancer Immunotherapy Resistance](https://arxiv.org/abs/2508.18638)
*Ifrah Tariq,Ernest Fraenkel*

Main category: cs.LG

TL;DR: A variational autoencoder with biologically informed modular encoders (BDVAE) integrates multi-omics data to predict immune checkpoint inhibitor response, revealing a continuous spectrum of resistance and interpretable latent features linked to immune, genomic, and metabolic processes.


<details>
  <summary>Details</summary>
Motivation: Address lack of interpretability and underutilization of biology in multi-omics ML models for predicting ICIs response; need models that capture immune, genomic, and metabolic processes to understand resistance.

Method: Biologically Disentangled Variational Autoencoder (BDVAE) with modality- and pathway-specific encoders to learn latent features from transcriptomic and genomic data. Modular architecture plus variational inference enables interpretable latent representations associated with immune, genomic, and metabolic pathways. Applied to a pan-cancer cohort (366 patients, four cancer types) treated with ICIs, with evaluation on unseen test data.

Result: BDVAE achieves AUC-ROC 0.94 on unseen data; identifies resistance mechanisms including immune suppression, metabolic shifts, and neuronal signaling; resistance spans a continuous spectrum; latent features correlate with survival and known clinical subtypes; demonstrates interpretable, clinically relevant insights.

Conclusion: Biologically structured ML like BDVAE can elucidate complex resistance patterns to ICIs and guide precision immunotherapy by providing interpretable latent factors and a spectrum-based view of resistance.
...
### [108] [The Sound of Risk: A Multimodal Physics-Informed Acoustic Model for Forecasting Market Volatility and Enhancing Market Interpretability](https://arxiv.org/abs/2508.18653)
...
TL;DR: Multimodal framework combining textual sentiment and paralinguistic cues from executive vocal dynamics in earnings calls. Uses PIAM to extract emotions; maps acoustic and textual signals to a 3D ASL space; predicts 30-day volatility (up to 43.8% out-of-sample R^2) but not directional returns; transitions from scripted to spontaneous speech, CFO instability, and CEO arousal drive volatility; outperforms financials-only baselines.


<details>
  <summary>Details</summary>
Motivation: Addresses information asymmetry and the inefficacy of purely textual analyses by incorporating biometric-derived signals that reveal latent uncertainty and emotional dynamics in corporate communications.

Method: Physics-Informed Acoustic Model (PIAM) applying nonlinear acoustics to extract emotional signatures from noisy earnings-call audio (robust to clipping); projects acoustic and textual signals into a 3D Affective State Label (ASL) space: Tension, Stability, Arousal; analyzes ~1,795 earnings calls (~1,800 hours), focusing on scripted vs spontaneous Q&A transitions; ablation studies compare to financials-only baselines.

Result: Multimodal features do not predict stock return direction, but explain up to 43.8% of the out-of-sample variance in 30-day realized volatility. Volatility predictions are driven by emotional dynamics during transitions from scripted to spontaneous speech, especially CFO textual instability and acoustic instability, and CEO arousal variability. Ablation confirms the added value over a financials-only model.

Conclusion: The approach decodes latent uncertainty markers from verifiable biometric signals, offering investors and regulators a tool to interpret risk and identify hidden corporate uncertainty beyond traditional text or financial signals.
...
### [109] [FFT-MoE: Efficient Federated Fine-Tuning for Foundation Models via Large-scale Sparse MoE under Heterogeneous Edge](https://arxiv.org/abs/2508.18663)
*Gang Hu,Yinglei Teng,Pengfei Wu,Nan Wang*

Main category: cs.LG

TL;DR: FFT MoE replaces LoRA with sparse Mixture of Experts in Federated Fine-Tuning to enable resource-aware, personalized adaptation under heterogeneous, privacy-constrained FL, improving generalization and training efficiency.


<details>
  <summary>Details</summary>
Motivation: Federated fine-tuning under privacy and resource constraints is essential when high-quality data resides on distributed edge devices. LoRA-based FFT faces structural incompatibility across clients and poor adaptation to non-IID data, hindering convergence and generalization in heterogeneous FL.

Method: Introduce FFT MoE: replace LoRA with sparse MoE adapters where each client trains a lightweight gating network to selectively activate a personalized subset of experts, ensuring aggregation compatibility. Add a heterogeneity-aware auxiliary loss to dynamically regularize routing to promote expert diversity and balanced utilization across devices.

Result: Extensive experiments under IID and non-IID settings show FFT MoE consistently outperforms state-of-the-art FFT baselines in generalization performance and training efficiency.

Conclusion: FFT MoE offers an effective framework for federated fine-tuning under client heterogeneity and resource constraints, enabling fine-grained adaptation while preserving aggregation compatibility and improving efficiency.
...
### [110] [Auditing Approximate Machine Unlearning for Differentially Private Models](https://arxiv.org/abs/2508.18671)
*Yuechun Gu,Jiajie He,Keke Chen*

Main category: cs.LG

TL;DR: Holistic privacy auditing shows approximate machine unlearning can undermine DP privacy for retained data; calls for DP-aware unlearning and introduces A-LiRA MIA for auditing; code released for reproducibility.


<details>
  <summary>Details</summary>
Motivation: To address privacy risks in approximate unlearning, specifically whether retained data continue to satisfy differential privacy after unlearning operations, challenging the common assumption that retained data remain unaffected.

Method: Define privacy criteria for unlearned and retained samples from DP and membership inference attack perspectives; develop A-LiRA, an efficient data-augmentation-based MIA to reduce shadow-model training cost; apply auditing to DP models with approximate unlearning.

Result: Existing approximate unlearning methods may compromise the DP privacy of retained samples; indicates need for DP-preserving unlearning; code published for reproducibility.

Conclusion: Holistic auditing of both unlearned and retained data is essential; DP-preserving unlearning algorithms are needed; the proposed auditing framework and A-LiRA tool aid in evaluating privacy risks post-unlearning.
...
### [111] [Optimal Sparsity of Mixture-of-Experts Language Models for Reasoning Tasks](https://arxiv.org/abs/2508.18672)
*Taishi Nakamura,Satoki Ishikawa,Masaki Kawamura,Takumi Okamoto,Daisuke Nohara,Jun Suzuki,Rio Yokota*

Main category: cs.LG

TL;DR: MoE sparsity creates a new dimension of scaling: growing total parameters boosts memorization, while reasoning performance saturates or declines even as training loss improves; changing top-k with fixed active parameters has little effect.


<details>
  <summary>Details</summary>
Motivation: To understand how MoE sparsity interacts with model size and data pipelines, and whether sparsity alters the classic scaling law patterns for memorization vs reasoning in LLMs.

Method: Train families of MoE Transformer models that systematically vary total parameters, active parameters, and top-k routing while keeping compute budget fixed; measure pre-training loss, downstream task loss, and task accuracy to separate train-test generalization from loss-accuracy gaps.

Result: Memorization improves monotonically with total parameters, mirroring training loss. Reasoning performance saturates or even regresses despite gains in total and training losses. Varying top-k alone has little effect when active parameters are fixed. Learning rate and initialization modulate the generalization gap in the same direction as sparsity. Post-training RL (GRPO) and extra test-time compute do not rescue reasoning deficits.

Conclusion: Sparsity is a critical axis of model scaling: increasing total capacity aids memorization but does not guarantee (and can hinder) reasoning; optimal performance requires careful tuning of active parameter count and routing, and traditional training choices continue to shape generalization similarly to sparsity. The work highlights the need to account for MoE-specific sparsity when interpreting scaling laws.
...
### [112] [Utilizing Training Data to Improve LLM Reasoning for Tabular Understanding](https://arxiv.org/abs/2508.18676)
*Chufan Gao,Jintai Chen,Jimeng Sun*

Main category: cs.LG

TL;DR: Learn then Retrieve (LRTab) uses chain-of-thought prompts learned from training data and retrieved at test time to enhance tabular reasoning with LLMs, achieving better accuracy, interpretability, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Finetuning offers dataset-specific learning but poor generalization; training-free prompting is general but underutilizes training data. LRTab aims to blend the advantages by learning prompt conditions from data and retrieving them during inference for tabular reasoning.

Method: 1) Use prompting to generate CoT responses on the training data. 2) For incorrect CoTs, prompt the LLM to predict 'Prompt Conditions' that would avoid the error, thereby learning insights from the data. 3) Validate these Prompt Conditions on a validation set. 4) At inference, retrieve the most relevant Prompt Conditions to provide additional context for table understanding.

Result: Experiments on WikiTQ and TabFact show LRTab is interpretable, cost-efficient, and can outperform previous baselines in tabular reasoning.

Conclusion: LRTab demonstrates that learning prompt conditions from data and retrieving them at inference time combines the strengths of finetuning and training-free prompting, yielding improved tabular reasoning performance with interpretability and efficiency.
...
### [113] [End to End Autoencoder MLP Framework for Sepsis Prediction](https://arxiv.org/abs/2508.18688)
*Hejiang Cai,Di Wu,Ji Xu,Xiang Liu,Yiziting Zhu,Xin Shu,Yujie Li,Bin Yi*

Main category: cs.LG

TL;DR: An end-to-end deep learning framework for early sepsis detection using a neural autoencoder for feature extraction and an MLP classifier, with downsampling and dynamic sliding windows, handling missing data via explicit indicators; validated on three ICU cohorts and outperforming traditional ML baselines.


<details>
  <summary>Details</summary>
Motivation: Sepsis detection in ICUs requires timely, robust predictions despite irregular/incomplete time-series data; traditional ML with feature engineering struggles in heterogeneous EHR data; need scalable, generalizable models.

Method: Unsupervised autoencoder for feature extraction; MLP classifier for binary sepsis risk; customized downsampling to select high-information-density segments during training; non-overlapping dynamic sliding window for real-time inference; preprocess time-series into fixed-dim vectors with missingness indicators; evaluate on 3 ICU cohorts.

Result: Accuracies: 74.6%, 80.6%, 93.5% across cohorts; outperforms traditional ML baselines; indicates robustness, generalizability, clinical utility.

Conclusion: End-to-end deep learning framework improves early sepsis detection across heterogeneous ICU settings; approach mitigates missing data bias and supports real-time deployment.
...
### [114] [Natural Image Classification via Quasi-Cyclic Graph Ensembles and Random-Bond Ising Models at the Nishimori Temperature](https://arxiv.org/abs/2508.18717)
*V. S. Usatyuk,D. A. Sapoznikov,S. I. Egorov*

Main category: cs.LG

TL;DR: A physics-inspired, topology-guided framework for efficient multi-class image classification by mapping high-dimensional features to spins on MET-QC-LDPC graphs (RBIM), operating at Nishimori temperature to maximize separability; links trapping sets to topological invariants, uses fast beta_N estimation, and topology-guided graph design to compress features with high accuracy on ImageNet-10/100.


<details>
  <summary>Details</summary>
Motivation: To achieve high class separability and efficient compression by unifying statistical physics, coding theory, and algebraic topology, enabling robust, scalable embeddings for large-scale image classification.

Method: Map frozen MobileNetV2 features to spins on a sparse multi-edge-type quasi-cyclic LDPC graph forming a Random-Bond Ising Model. Operate at Nishimori temperature beta_N where Bethe-Hessian eigenvalue behavior aids separability. Establish a correspondence between local trapping sets and topological invariants (Betti numbers, bordism). Efficiently estimate beta_N using a quadratic interpolant with Newton correction (six-fold speed-up over bisection). Design spherical and toroidal MET-QC-LDPC ensembles guided by topology, using permanent bounds to suppress trapping sets. Achieve heavy dimensionality reduction (1280D to 32/64D) for ImageNet-10/100 while preserving accuracy.

Result: The approach achieves 6x faster beta_N estimation, about 40x feature compression, and high classification accuracy—98.7% on ImageNet-10 and 82.7% on ImageNet-100—reported as state-of-the-art for such compressed embeddings.

Conclusion: Topology-guided, physics-inspired embeddings can yield highly efficient, high-accuracy multi-class classification, establishing a cross-disciplinary framework that connects local graph trapping structures with global topological invariants to guide practical graph design and feature compression.
...
### [115] [Beyond Tokens: Enhancing RTL Quality Estimation via Structural Graph Learning](https://arxiv.org/abs/2508.18730)
*Yi Liu,Hongji Zhang,Yiwen Wang,Dimitris Tsaras,Lei Chen,Mingxuan Yuan,Qiang Xu*

Main category: cs.LG

TL;DR: A structure-aware graph SSL framework, StructRTL, leveraging CDFG representations and knowledge distillation to improve RTL design quality estimation, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: RTL quality estimation is valuable for quick feedback on area and delay; existing LLM-based RTL embeddings miss crucial structural semantics; the CDFG view exposes richer structural cues for better representations.

Method: Learn structure-informed representations from CDFGs via a self-supervised graph learning framework, and employ cross-stage knowledge distillation from post-mapping netlists to the CDFG predictor to transfer low-level insights.

Result: Outperforms prior art on several RTL quality estimation tasks, achieving new state-of-the-art results and demonstrating the benefit of combining structure-aware learning with cross-stage supervision.

Conclusion: Structured graph learning over CDFGs, augmented with distillation from downstream designs, improves RTL quality estimation and has potential to accelerate EDA workflows.
...
### [116] [FLAegis: A Two-Layer Defense Framework for Federated Learning Against Poisoning Attacks](https://arxiv.org/abs/2508.18737)
*Enrique Mármol Campos,Aurora González Vidal,José Luis Hernández Ramos,Antonio Skarmeta*

Main category: cs.LG

TL;DR: FLAegis: a two-stage Byzantine-robust defense for Federated Learning using SAX-based time-series pattern amplification, spectral clustering for detection, and FFT-based robust aggregation; outperforms SOTA in detection and accuracy under five poisoning attacks.


<details>
  <summary>Details</summary>
Motivation: To address privacy-preserving FL's vulnerability to Byzantine clients that poison updates; need reliable detection and aggregation to maintain model quality.

Method: Two-stage: (1) detect Byzantine clients via SAX-based symbolic time-series transformation to amplify differences between benign and malicious models; (2) spectral clustering to identify adversarial behavior; final aggregation uses a robust FFT-based function to mitigate undetected attackers.

Result: Outperforms state-of-the-art defenses in detection precision and final model accuracy across five poisoning attacks, maintaining high performance under strong adversaries.

Conclusion: FLAegis effectively detects Byzantine clients and enhances FL robustness, offering strong defense against a range of poisoning strategies.
...
### [117] [Stability and Generalization for Bellman Residuals](https://arxiv.org/abs/2508.18741)
*Enoch H. Kang,Kyoungseok Jang*

Main category: cs.LG

TL;DR: Introduces a stability-based analysis for Bellman Residual Minimization (BRM) in offline RL/IRL, achieving an O(1/n) excess risk bound without extra tricks, via a Lyapunov potential that couples neighboring SGDA runs.


<details>
  <summary>Details</summary>
Motivation: There is a gap in understanding the statistical behavior of BRM in offline settings, despite BRM being a practical remedy for enforcing Bellman consistency in offline RL/IRL.

Method: Define a Lyapunov potential that couples stochastic gradient descent-ascent (SGDA) on neighboring datasets. Prove an O(1/n) on-average argument-stability bound, which translates into an O(1/n) excess risk bound for BRM. Works with standard neural networks and minibatch SGD, without variance reduction or extra regularization, and without restrictive minibatch independence assumptions.

Result: Establishes an O(1/n) excess risk bound for BRM in offline settings, improving the known sample complexity exponent for convex-concave saddle problems by a factor of two in expectation. The approach yields practical stability guarantees for BRM with common neural-network parameterizations.

Conclusion: BRM via the proposed SGDA with a Lyapunov potential offers strong statistical guarantees in offline RL/IRL, matching standard neural-network practice and minibatch SGD, and removes the need for certain variance-reduction or independence assumptions while delivering improved sample efficiency.
...
### [118] [Constraint Matters: Multi-Modal Representation for Reducing Mixed-Integer Linear programming](https://arxiv.org/abs/2508.18742)
*Jiajun Li,Ran Hou,Yu Ding,Yixuan Li,Shisi Guan,Jiahui Duan,Xiongwei Han,Tao Zhong,Vincent Chau,Weiwei Wu,Wanyuan Wang*

Main category: cs.LG

TL;DR: A constraint-based MILP reduction method identifies and learns critical tight inequality constraints to convert them into equalities, improving solving efficiency. It uses labeling of optimal tight constraints and a multi-modal representation (instance- and abstract-level MILP) to predict critical constraints, achieving substantial gains in solution quality (>50%) and speed (~17.5% faster) versus state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Most model reduction focuses on variable reduction; constraint reduction (turning inequalities into equalities) is underexplored despite potential to reduce MILP complexity. Identifying which constraints are critical and can be safely reduced is challenging yet valuable for speed and feasibility.

Method: 1) Identify potential critical constraints by labeling tight inequalities at the optimal solution. 2) Use a heuristic to select a subset of these critical tight constraints. 3) Learn critical tight constraints via a multi-modal representation that fuses instance-level information with abstract-level MILP formulations.

Result: Compared with state-of-the-art methods, the proposed constraint-based reduction improves solution quality by over 50% and reduces computation time by 17.47%.

Conclusion: Constraint-based MILP reduction is a viable and effective approach to accelerate MILP solving and improve solution quality, with the proposed learning-based mechanism for identifying critical constraints offering strong empirical gains and potential for broader applicability.
...
### [119] [UltraMemV2: Memory Networks Scaling to 120B Parameters with Superior Long-Context Learning](https://arxiv.org/abs/2508.18756)
*Zihao Huang,Yu Bao,Qiyang Min,Siyan Chen,Ran Guo,Hongzhi Huang,Defa Zhu,Yutao Zeng,Banggu Wu,Xun Zhou,Siyuan Qiao*

Main category: cs.LG

TL;DR: UltraMemV2 closes the gap between memory-layer architectures and 8-expert Mixture-of-Experts (MoE) models, achieving parity with 8-expert MoEs under the same compute and parameters while greatly reducing memory access. It also delivers stronger performance on memory-intensive tasks.


<details>
  <summary>Details</summary>
Motivation: MoE models offer efficiency by activating only subsets of parameters but incur high memory access costs during inference. Prior memory-layer approaches (e.g., UltraMem) matched only 2-expert MoEs and lagged behind the 8-expert state of the art, motivating a redesigned, memory-efficient architecture that can reach 8-expert MoE performance with lower memory traffic.

Method: Five improvements: (1) integrate memory layers into every transformer block; (2) simplify value expansion using a single linear projection; (3) adopt FFN-based value processing from PEER; (4) apply principled parameter initialization; (5) rebalance memory-to-FFN computation ratios. Evaluation scales to models with up to 2.5B activated parameters from 120B total parameters, showing activation density has a larger impact on performance than total sparse parameter count.

Result: UltraMemV2 achieves performance parity with 8-expert MoE models under the same computation and parameters while reducing memory access. It exhibits superior performance on memory-intensive tasks: +1.6 points on long-context memorization, +6.2 points on multi-round memorization, and +7.9 points on in-context learning. Scaling experiments up to 2.5B activated parameters corroborate the importance of activation density over total sparse parameter count.

Conclusion: Memory-layer architectures can reach state-of-the-art MoE performance with significantly lower memory access, offering a compelling alternative for efficient sparse computation.
...
### [120] [Governance-as-a-Service: A Multi-Agent Framework for AI System Compliance and Policy Enforcement](https://arxiv.org/abs/2508.18765)
*Helen Pervez,Suyash Gaurav,Jukka Heikkonen,Jatin Chaudhary*

Main category: cs.LG

TL;DR: Proposes Governance-as-a-Service (GaaS), a runtime, policy-driven governance layer for multi-agent AI systems that blocks high-risk actions, logs activity, and uses a Trust Factor to modulate trust and penalties, preserving throughput.


<details>
  <summary>Details</summary>
Motivation: Address structural risk from distributed autonomous AI ecosystems lacking scalable, decoupled governance; existing oversight is reactive, brittle, and hard to generalize across heterogeneous deployments.

Method: Introduce GaaS as a modular enforcement layer with declarative rules and a Trust Factor; executes at runtime by intercepting agent outputs without altering model internals or requiring agent cooperation; supports coercive, normative, and adaptive interventions with graduated enforcement and dynamic trust modulation; evaluates via three simulation regimes using open-source models (LLaMA3, Qwen3, DeepSeek-R1) across content generation and financial decision-making; compares baseline (no governance), GaaS-enabled, and adversarial probes; all actions are intercepted, evaluated, and logged for analysis.

Result: GaaS reliably blocks or redirects high-risk behaviors while preserving throughput; Trust Factor scores track rule adherence and help isolate and penalize untrustworthy components in multi-agent systems.

Conclusion: Governance is best implemented as an infrastructure/runtime service (like compute or storage) to enable interoperable, aligned agent ecosystems; it enforces ethics at runtime rather than teaching them.
...
### [121] [Predicting Drug-Drug Interactions Using Heterogeneous Graph Neural Networks: HGNN-DDI](https://arxiv.org/abs/2508.18766)
*Hongbo Liu,Siyi Li,Zheng Yu*

Main category: cs.LG

TL;DR: HGNN-DDI uses a heterogeneous graph neural network to integrate diverse drug-related data and predict drug-drug interactions, achieving higher accuracy and robustness than baselines on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: DDIs pose safety risks; existing computational approaches struggle to capture complex, heterogeneous relations among drugs, targets, and biological entities; there is a need to leverage heterogeneous biomedical networks for better prediction.

Method: Introduce HGNN-DDI, a heterogeneous GNN that models a heterogeneous biomedical network with multiple node and edge types, enabling information propagation across diverse data sources to predict DDIs.

Result: On benchmark DDI datasets, HGNN-DDI outperforms state-of-the-art baselines in prediction accuracy and robustness, demonstrating effectiveness and reliability.

Conclusion: HGNN-DDI offers a promising approach for safer drug development and precision medicine by accurately predicting DDIs and leveraging diverse biomedical data.
...
### [122] [Federated Learning with Heterogeneous and Private Label Sets](https://arxiv.org/abs/2508.18774)
*Adam Breitholtz,Edvin Listo Zec,Fredrik D. Johansson*

Main category: cs.LG

TL;DR: Heterogeneous and private label sets in federated learning degrade performance, but adapted methods with centralized tuning maintain accuracy while enhancing privacy.


<details>
  <summary>Details</summary>
Motivation: Real-world FL often involves clients with different label sets and privacy constraints that prevent sharing full label alphabets. This work investigates how label-set heterogeneity and private-label constraints affect learning, and whether standard FL methods can be effectively adapted.

Method: The study compares public vs private label settings and scenarios where the union of label sets is known to clients versus not. It applies classical classifier-combination techniques to FL with centralized tuning, and adapts common FL methods to the private label setting. It analyzes the justification of these approaches under practical assumptions and uses experiments to assess performance and variance.

Result: Reducing the number of labels available to each client substantially harms all methods. Centralized tuning for representational alignment can mitigate this harm but often increases variance. The proposed adaptations of standard FL methods perform well, yielding similar accuracy in the private label setting as standard methods achieve in the public setting.

Conclusion: Private label settings can yield increased privacy at little cost to accuracy when using the adapted FL methods and centralized tuning proposed in this work."
...
### [123] [SWiFT: Soft-Mask Weight Fine-tuning for Bias Mitigation](https://arxiv.org/abs/2508.18826)
*Junyu Yan,Feng Chen,Yuyang Xue,Yuning Du,Konstantinos Vilouras,Sotirios A. Tsaftaris,Steven McDonagh*

Main category: cs.LG

TL;DR: SWiFT provides data-efficient debiasing via soft-mask weight fine-tuning that preserves accuracy while reducing bias, using a small external dataset, few fine-tuning epochs, and showing improved out-of-distribution generalization.


<details>
  <summary>Details</summary>
Motivation: Bias in ML models in ethically sensitive domains and limitations of existing debiasing methods (data requirements, costly retraining, fairness-accuracy trade-offs) drive the need for a lightweight, effective approach.

Method: Identify relative contributions of model parameters to bias and predictive performance and apply a two-step gradient flow to update each parameter with distinct objectives, using a small external dataset and only a few fine-tuning epochs.

Result: Empirical evaluation on three bias-sensitive attributes (gender, skin tone, age) across dermatology and chest X-ray datasets shows consistent bias reduction while maintaining or improving diagnostic accuracy; improved generalization on several out-of-distribution datasets and favorable comparisons to state-of-the-art baselines.

Conclusion: Soft-Mask Weight Fine-Tuning (SWiFT) is a data-efficient debiasing framework that reduces bias with minimal retraining cost and preserves discriminative performance, while enhancing generalization.
...
### [124] [DRMD: Deep Reinforcement Learning for Malware Detection under Concept Drift](https://arxiv.org/abs/2508.18839)
*Shae McFadden,Myles Foley,Mario D'Onghia,Chris Hicks,Vasilios Mavroudis,Nicola Paoletti,Fabio Pierazzi*

Main category: cs.LG

TL;DR: A DRL-based malware detector (DRMD) jointly optimizes classification and rejection under concept drift, framed as a one-step MDP, achieving higher time-aware AUT than standard methods in Android malware.


<details>
  <summary>Details</summary>
Motivation: Malware detection must cope with evolving threats, limited labeling budgets, and uncertain predictions. Traditional supervised classifiers degrade under concept drift because they cannot optimize when to request manual labeling or adaptation. Integrating rejection and active learning with decision-making can improve long-term performance.

Method: Formulate malware detection as a one-step Markov Decision Process and train a deep reinforcement learning (DRL) agent (DRMD) to simultaneously maximize classification accuracy and minimize high-risk samples sent for manual labeling. Evaluate a drift-aware policy on Android malware datasets with multi-year drift to measure time-aware performance (AUT). Compare against standard classification, classification with rejection, and classification with rejection plus AL.

Result: DRMD achieves higher average AUT improvements over time: 5.18±5.44 (classification only), 14.49±12.86 (classification with rejection), and 10.06±10.81 (classification with rejection and AL). These gains demonstrate improved resilience to concept drift.

Conclusion: DRL can effectively enable malware detection with greater resiliency to concept drift in Android malware, achieving better time-aware performance and illustrating the value of integrating decision-making and rejection/AL mechanisms.
...
### [125] [Recycling History: Efficient Recommendations from Contextual Dueling Bandits](https://arxiv.org/abs/2508.18841)
*Suryanarayana Sankagiri,Jalal Etesami,Pouria Fatemi,Matthias Grossglauser*

Main category: cs.LG

TL;DR: A new contextual bandit model for recommendations where each step yields a user choice and a comparison with a past item; with a short exploration phase and richness of history, the method achieves sqrt(T) regret and benefits from reusing past items for comparisons.


<details>
  <summary>Details</summary>
Motivation: To capture richer user feedback than standard implicit choices by allowing the user to compare a just-consumed item with a past item from history, without incurring extra regret, and to overcome temporal dependencies via a history-rich, diversified dataset.

Method: Propose a bandit framework where at each step an item is recommended and then compared to a past item from the history; ensure the history is rich (diverse); use a short initial random exploration to accumulate such a history; employ matrix concentration bounds to analyze regret; show the algorithm can construct informative queries from history.

Result: Theoretical regret bound of O(√T) with high probability; simulations indicate that reusing past items for comparisons significantly lowers regret compared to only comparing items presented in the same round.

Conclusion: Incorporating past-item comparisons enables more informative feedback and improved regret performance, provided the history is sufficiently diverse; a short exploration phase suffices to build this richness, suggesting practical gains in adaptive recommendation systems.
...
### [126] [C-Flat++: Towards a More Efficient and Powerful Framework for Continual Learning](https://arxiv.org/abs/2508.18860)
*Wei Li,Hangjie Yuan,Zixiang Zhao,Yifan Zhu,Aojun Lu,Tao Feng,Yanan Sun*

Main category: cs.LG

TL;DR: Introduces C-Flat, a flatness-promoting, plug-and-play framework for continual learning that yields flatter loss landscapes, improving stability and performance; also C-Flat++ with selective flatness promotion for efficiency. Demonstrated across CL paradigms and datasets; code available.


<details>
  <summary>Details</summary>
Motivation: Continual learning struggles with balancing memory retention and adaptability (stability-plasticity). Zeroth-order sharpness minimization can bias models toward sharper minima in some settings, risking robustness and suboptimal solutions. There is a need for flatness-aware optimization tailored to continual learning that can be integrated easily with existing CL pipelines.

Method: Proposes C-Flat, a flatness-promoting approach that can be plugged into existing CL workflows. It offers a general framework for integrating flatness-driven optimization across major continual learning paradigms. Additionally, C-Flat++ introduces selective flatness promotion to reduce update costs while retaining benefits.

Result: Empirical results show consistent performance improvements across a wide range of CL settings, methods, and datasets. C-Flat is easy to integrate; C-Flat++ achieves further efficiency gains with selective promotion. Code is released at the provided GitHub repository.

Conclusion: C-Flat and C-Flat++ provide practical, versatile flatness-aware solutions for continual learning, offering improved stability and performance with broad compatibility and reasonable computational cost. The work highlights the value of loss-landscape flatness considerations in CL and provides a path for future efficiency-focused refinements.
...
### [127] [MOCHA: Discovering Multi-Order Dynamic Causality in Temporal Point Processes](https://arxiv.org/abs/2508.18873)
*Yunyang Cao,Juekai Lin,Wenhao Li,Bo Jin*

Main category: cs.LG

TL;DR: MOCHA discovers multi-order dynamic causality in temporal point processes by modeling multi-hop causal paths on a latent time-evolving graph with a time-varying DAG and learnable weights, achieving state-of-the-art event prediction and yielding interpretable causal structures.


<details>
  <summary>Details</summary>
Motivation: Current approaches rely on static or first-order causal assumptions and fail to capture multi-order, time-varying dependencies in event sequences; a dynamic, multi-hop causal representation is needed for accurate prediction and interpretability.

Method: MOCHA introduces a time-varying directed acyclic graph with learnable structural weights to model multi-hop causal paths over a latent evolving graph; enforces acyclicity and sparsity for valid structure; offers an end-to-end differentiable framework jointly modeling causal discovery and TPP dynamics.

Result: Experiments on real-world datasets show state-of-the-art performance in event prediction and yield meaningful, interpretable causal structures.

Conclusion: MOCHA provides a scalable, interpretable framework for discovering multi-order dynamic causality in temporal point processes by leveraging a time-varying DAG to capture multi-hop influences, achieving strong predictive performance and interpretable insights.
...
### [128] [HAEPO: History-Aggregated Exploratory Policy Optimization](https://arxiv.org/abs/2508.18884)
*Gaurish Trivedi,Alakh Sharma,Kartikey Singh Bhandari,Dhruv Kumar,Pratik Narang,Jagat Sesh Challa*

Main category: cs.LG

TL;DR: HAEPO introduces a history-aware exploratory objective that compresses each trajectory into its cumulative log-likelihood and uses a Plackett-Luce weighting over trajectories to promote broad exploration, with entropy and soft KL regularization for stability.


<details>
  <summary>Details</summary>
Motivation: Existing exploration methods (e.g., DPO, GRPO) struggle with long-horizon tasks due to relying on per-token or full-trajectory measures; there is a need for a history-aware balance of exploration and stability that leverages full trajectory history.

Method: HAEPO compresses each trajectory into the sum of logarithmic probabilities (cumulative log-likelihood); applies a Plackett-Luce softmax across trajectories to weight updates by returns; adds entropy regularization to stabilize aggressive updates and a soft KL penalty relative to a frozen copy of the previous policy.

Result: Empirically, HA EPO converges fast, explores thoroughly, aligns closely with true rewards, and shows robust learning behavior, performing at least as well as PPO, GRPO, and DPO across diverse tasks.

Conclusion: HAEPO provides a stable, interpretable framework that explicitly leverages full-trajectory history to balance exploration and stability, achieving strong long-horizon performance.
...
### [129] [pyFAST: A Modular PyTorch Framework for Time Series Modeling with Multi-source and Sparse Data](https://arxiv.org/abs/2508.18891)
*Zhijin Wang,Senzhen Wu,Yue Hu,Xiufeng Liu*

Main category: cs.LG

TL;DR: pyFAST is a modular, PyTorch-based time series framework that separates data processing from model computation, enabling flexible handling of irregular, multi-source, and sparse data with LLM-inspired fusion and a rich suite of models.


<details>
  <summary>Details</summary>
Motivation: Current Python time series libraries struggle with modularity and native support for irregular, multi-source, or sparse data, hindering rapid experimentation and complex data workflows.

Method: Design a decoupled data engine supporting multi-source loading, sequence/patch padding, dynamic normalization, mask-based modeling, and exogenous data fusion; incorporate LLM-inspired alignment-free fusion for sparse sources; provide native sparse metrics and loss functions; offer batch-based streaming evaluation and device-accelerated training; include a modular set of models (Linear, CNN, RNN, Transformer, GNN) built for easy extension.

Result: Release of pyFAST as a MIT-licensed research-oriented PyTorch framework with a rich data engine, native sparse support, flexible losses, and a modular model zoo, designed to accelerate time series research and experimentation.

Conclusion: pyFAST delivers a compact yet powerful platform that cleanly separates data processing from modeling, enabling rapid experimentation, extensibility, and efficient handling of complex time-series data scenarios.
...
### [130] [Distance-informed Neural Processes](https://arxiv.org/abs/2508.18903)
*Aishwarya Venkataramanan,Joachim Denzler*

Main category: cs.LG

TL;DR: A distance-informed Neural Process (DNP) extends Neural Processes by using a global latent for task-level variation and a distance-aware local latent to model input similarity, regularized with bi-Lipschitz constraints to preserve relationships; results show better uncertainty calibration and predictive performance, including OOD distinction, across regression and classification.


<details>
  <summary>Details</summary>
Motivation: Standard Neural Processes rely on a single global latent and often struggle with uncertainty calibration and capturing fine-grained local dependencies. There is a need to better model local input relationships and calibrate uncertainty, especially for out-of-distribution detection.

Method: Introduce a global latent for task-level variation plus a local latent that encodes input similarity in a distance-preserving latent space. Apply bi-Lipschitz regularization to bound distortions in input relationships and encourage preservation of relative distances.

Result: DNP yields better-calibrated uncertainty estimates and improved predictive performance across regression and classification tasks, with improved ability to discriminate in-distribution from out-of-distribution data.

Conclusion: Combining global task-level latent and distance-aware local latent structures via bi-Lipschitz regularization improves NP uncertainty calibration and overall performance, demonstrating robust applicability across tasks.
...
### [131] [Enhancing Model Privacy in Federated Learning with Random Masking and Quantization](https://arxiv.org/abs/2508.18911)
*Zhibo Xu,Jianhao Zhu,Jingwen Xu,Changze Lv,Zisu Huang,Xiaohua Wang,Muling Wu,Qi Qian,Xiaoqing Zheng,Xuanjing Huang*

Main category: cs.LG

TL;DR: A federated learning approach that maintains strong performance while improving protection of model parameters compared with baseline methods.


<details>
  <summary>Details</summary>
Motivation: Federated learning grapples with the trade-off between model utility and protecting model parameters; there's a need for methods that guard parameters without sacrificing accuracy.

Method: Proposes a protection-enhanced FL framework featuring (non-specific) privacy-preserving and robust aggregation mechanisms to safeguard parameters while preserving performance across diverse models and tasks.

Result: Empirical results across multiple models and tasks show that the approach sustains high accuracy in FL settings and offers enhanced protection of parameters relative to baselines.

Conclusion: The method successfully achieves both competitive performance and stronger parameter protection in federated learning scenarios.
...
### [132] [Generalization Bound for a General Class of Neural Ordinary Differential Equations](https://arxiv.org/abs/2508.18920)
*Madhusudan Verma,Manoj Kumar*

Main category: cs.LG

TL;DR: Generalization bounds for neural ODEs with nonlinear Lipschitz dynamics, extending beyond prior linear-only analyses.


<details>
  <summary>Details</summary>
Motivation: To understand how neural ODEs generalize on unseen data when the dynamics are nonlinear, time-dependent or independent, under Lipschitz continuity.

Method: Assume the dynamics are Lipschitz; prove solutions have bounded variation; derive generalization bounds for both time-dependent and time-independent cases; analyze effects of overparameterization and domain constraints.

Result: First derivation of generalization bounds for neural ODEs with general nonlinear dynamics; bound arises from bounded variation of solutions.

Conclusion: Extends generalization theory to a broader class of neural ODEs, informing how architecture choices and constraints influence generalization.
...
### [133] [HierCVAE: Hierarchical Attention-Driven Conditional Variational Autoencoders for Multi-Scale Temporal Modeling](https://arxiv.org/abs/2508.18922)
*Yao Wu*

Main category: cs.LG

TL;DR: Proposes HierCVAE, a hierarchical-attention conditional VAE for multi-scale temporal modeling with explicit uncertainty, achieving 15-40% accuracy gains on energy datasets and improved uncertainty calibration.


<details>
  <summary>Details</summary>
Motivation: To address dependencies across multiple time scales and inherent uncertainties in complex temporal systems, by capturing temporal, statistical, and trend information through hierarchical attention and conditional variational inference, while providing explicit uncertainty quantification.

Method: A three-tier attention architecture (local, global, cross-temporal) with multi-modal condition encoding to capture temporal, statistical, and trend information. Latent space includes ResFormer blocks. The model is built as a conditional variational autoencoder with prediction heads for uncertainty quantification and is evaluated on energy consumption datasets.

Result: HierCVAE achieves 15-40% improvements in prediction accuracy and superior uncertainty calibration compared to state-of-the-art methods. It excels in long-term forecasting and modeling complex, multi-variate dependencies.

Conclusion: HierCVAE effectively models multi-scale temporal dependencies with calibrated uncertainty, offering strong predictive performance and robust uncertainty estimates for long-horizon forecasting in energy data, with potential applicability to other complex temporal systems.
...
### [134] [Energy-Based Flow Matching for Generating 3D Molecular Structure](https://arxiv.org/abs/2508.18949)
*Wenyin Zhou,Christopher Iliffe Sprague,Vsevolod Viliuga,Matteo Tadiello,Arne Elofsson,Hossein Azizpour*

Main category: cs.LG

TL;DR: An energy-based flow-matching approach for molecular structure generation learns a deep mapping that iteratively transforms random configurations into valid molecular structures, achieving state-of-the-art results on protein docking and backbone generation within similar compute budgets.


<details>
  <summary>Details</summary>
Motivation: Molecular structure generation is essential for tasks like docking, folding, and design. While diffusion and flow-matching models have advanced the field by modeling conformations as distributions, there is a need for improved training/inference through an energy-based perspective that highlights stability, idempotency, and refinement, inspired by techniques used in AlphaFold.

Method: Adopt an energy-based view to design a flow-matching framework where a deep network learns a mapping that iteratively transforms samples from a source distribution into the data manifold (target structures). The approach emphasizes energy-based training, theoretical properties (idempotency, stability), and connections to refinement strategies.

Result: On protein docking and backbone generation, the proposed method consistently outperforms recent baselines in task-specific flow matching and diffusion models while using a similar computational budget.

Conclusion: The simple yet principled flow-matching framework, grounded in energy-based theory, offers empirical gains and theoretical insight for molecular structure generation and aligns with refinement ideas from AlphaFold.
...
### [135] [Estimating Conditional Covariance between labels for Multilabel Data](https://arxiv.org/abs/2508.18951)
*Laurence A. F. Park,Jesse Read*

Main category: cs.LG

TL;DR: Three models (Multivariate Probit, Multivariate Bernoulli, and Staged Logit) are compared for estimating constant and dependent multilabel conditional covariance. All models may falsely detect dependency when only constant covariance is present, but Multivariate Probit has the lowest error rate.


<details>
  <summary>Details</summary>
Motivation: To determine how well different models estimate constant versus dependent conditional covariance in multilabel data, given that unconditional independence cannot be verified directly and that the multivariate Probit model’s copula covariance may be unreliable.

Method: An experimental comparison where each model is used to estimate constant and dependent multilabel conditional covariance across varying levels of true covariance strength and dependence.

Result: All models detect dependent covariance depending on strength, but all tend to falsely detect dependence when constant covariance is present. Among the three, Multivariate Probit achieves the lowest error rate.

Conclusion: Multivariate Probit is the most reliable among the three models for estimating constant and dependent multilabel conditional covariance, though all models struggle with false positives when constant covariance is present.
...
### [136] [On the Generalisation of Koopman Representations for Chaotic System Control](https://arxiv.org/abs/2508.18954)
*Kyriakos Hjikakou,Juan Diego Cardenas Cartagena,Matthia Sabatelli*

Main category: cs.LG

TL;DR: A three-stage pipeline learns Koopman embeddings for a chaotic Lorenz system to achieve transferable representations across prediction and control tasks; embeddings outperform PCA baselines and are robust to freezing the transformer during fine-tuning, suggesting reusable dynamical structure for multi-task physics-informed ML.


<details>
  <summary>Details</summary>
Motivation: Evaluate whether Koopman-based representations generalize across tasks in chaotic dynamics and can serve as a reusable foundation for multi-task, physics-informed machine learning.

Method: 1) learn Koopman embeddings via autoencoding; 2) pre-train a transformer on next-state prediction; 3) fine-tune for safety-critical control; compare against standard and physics-informed PCA baselines; test with Lorenz system; assess effect of freezing pre-trained transformer during fine-tuning.

Result: Koopman embeddings outperform both standard and physics-informed PCA baselines with accurate, data-efficient performance; fixing transformer weights during fine-tuning yields no degradation, indicating learned representations capture reusable dynamical structure rather than task-specific patterns.

Conclusion: Koopman embeddings are a solid foundation for multi-task learning in physics-informed ML; demonstrated via Lorenz system with transferability across prediction and control; project page available.
...
### [137] [PAX-TS: Model-agnostic multi-granular explanations for time series forecasting via localized perturbations](https://arxiv.org/abs/2508.18982)
*Tim Kreuzer,Jelena Zdravkovic,Panagiotis Papapetrou*

Main category: cs.LG

TL;DR: PAX-TS is a model-agnostic, post-hoc explanation method for time series forecasting that uses localized perturbations to provide multi-granular, cross-channel explanations, revealing patterns in time-step correlations that relate to model performance across multiple datasets and algorithms.


<details>
  <summary>Details</summary>
Motivation: Modern time series forecasting models are often opaque. Traditional post-hoc explainers (e.g., LIME) are not well-suited for forecasting due to temporal structure and cross-channel dependencies; there is a need for explanations tailored to forecast models that reveal how inputs influence predictions over time and across channels.

Method: PAX-TS applies localized input perturbations to time series inputs to generate explanations in a model-agnostic, post-hoc manner. It produces multi-granular explanations, characterizes cross-channel correlations for multivariate forecasts, outlines a concrete algorithm, and is evaluated on a benchmark of 7 forecasting algorithms across 10 diverse datasets, comparing with two state-of-the-art explanation methods, and detailing different explanation types and patterns.

Result: Explanations differ between high-performing and low-performing models on the same datasets, indicating PAX-TS captures model behavior. Time-step correlation matrices reveal six recurrent pattern classes across datasets and algorithms, which correlate with forecasting error. A multivariate example demonstrates how cross-channel correlations influence forecasts.

Conclusion: PAX-TS enables illustrating forecasting models’ mechanisms at multiple levels of detail and supports answering practical questions about forecasts; it also provides insight into cross-channel interactions and pattern-based indicators of performance.
...
### [138] [FedProtoKD: Dual Knowledge Distillation with Adaptive Class-wise Prototype Margin for Heterogeneous Federated Learning](https://arxiv.org/abs/2508.19009)
*Md Anwar Hossen,Fatema Siddika,Wensheng Zhang,Anuj Sharma,Ali Jannesari*

Main category: cs.LG

TL;DR: FedProtoKD tackles prototype margin shrinking in heterogeneous federated learning by applying an enhanced dual-knowledge distillation framework that leverages client logits and prototype features, with a trainable server prototype using contrastive learning and a class-wise adaptive margin, and by evaluating public samples via prototype proximity. It achieves notable accuracy gains over state-of-the-art HFL methods.


<details>
  <summary>Details</summary>
Motivation: Heterogeneous Federated Learning (HFL) enables diverse models and non-IID data across clients, but prototype-based HFL suffers from prototype margin shrinkage due to server aggregation, which degrades global knowledge transfer and model performance while preserving privacy.

Method: Introduce FedProtoKD with an enhanced dual-knowledge distillation mechanism that uses client logits and prototype feature representations. Mitigate prototype margin shrinking with a contrastive-learning-based trainable server prototype and a class-wise adaptive prototype margin. Assess the importance of public samples by measuring how close a sample’s prototype is to its class prototypes to weight learning.

Result: Federated models show average accuracy improvements from 1.13% up to 34.13% across various settings, significantly outperforming existing state-of-the-art HFL methods.

Conclusion: FedProtoKD effectively alleviates prototype margin shrinking in heterogeneous FL and improves overall performance by leveraging dual distillation, adaptive prototype margins, and public-sample-aware learning, indicating strong potential for privacy-preserving HFL in highly non-IID, model-heterogeneous scenarios.
...
### [139] [STDiff: A State Transition Diffusion Framework for Time Series Imputation in Industrial Systems](https://arxiv.org/abs/2508.19011)
*Gary Simethy,Daniel Ortiz-Arroyo,Petar Durdevic*

Main category: cs.LG

TL;DR: STDiff uses a dynamics-aware conditional diffusion model to impute missing values by predicting state-to-state evolution, outperforming window-based methods, especially for long gaps.


<details>
  <summary>Details</summary>
Motivation: Imputation often relies on fixed time-window patterns, which fails in industrial systems where dynamics are driven by control actions, are non-stationary, and can have long, uninterrupted gaps. A dynamics-aware approach that conditions on the most recent state and inputs is needed to preserve plausible trajectories.

Method: A conditional denoising diffusion model with a causal bias aligned to control theory. It generates missing values step-by-step based on the most recent known state and relevant control/environmental inputs, reframing imputation as learning the system's evolution from one state to the next.

Result: On a public wastewater dataset with simulated missing blocks, STDiff achieves the lowest error, with advantage increasing for longer gaps. On a real industrial dataset with substantial gaps, it yields dynamically plausible trajectories, whereas window-based models tend to flatten or over-smooth.

Conclusion: Dynamics-aware, explicitly conditioned imputation is a robust approach for industrial time series; the work discusses computational trade-offs and extensions to broader domains.
...
### [140] [Learning with springs and sticks](https://arxiv.org/abs/2508.19015)
*Luis Mantilla Calderón,Alán Aspuru-Guzik*

Main category: cs.LG

TL;DR: A spring-and-stick mechanical system can approximate any continuous function via a piecewise-linear representation, learning through energy minimization where spring potential encodes the loss; regression performance is on par with MLPs, and the model reveals thermodynamic aspects of learning, including a barrier due to environmental fluctuations.


<details>
  <summary>Details</summary>
Motivation: To understand learning as a physical process by building a minimal dynamical system (springs and sticks) that can learn from data through dissipation and energy minimization, and to connect learning performance with thermodynamic quantities like free energy.

Method: Construct a mechanical network of springs and sticks to realize piecewise-linear function approximation. Use the potential energy of springs to encode a mean-squared-error loss; drive the system to a minimum-energy configuration via dissipative dynamics. Test on regression tasks and compare to multi-layer perceptrons; study thermodynamic properties and fluctuations to relate free-energy change to learning behavior.

Result: The system achieves regression performance comparable to MLPs. It reveals a relation between free-energy changes and the ability to learn a data distribution. Empirically, a thermodynamic learning barrier emerges due to environmental fluctuations, limiting learning when energy changes cross this barrier.

Conclusion: A simple, physically grounded model can shed light on learning dynamics by linking optimization to energy dissipation and thermodynamics. The existence of a learning barrier underscores fundamental limits set by the environment and motivates further exploration of physical implementations of learning systems.
...
### [141] [Working My Way Back to You: Resource-Centric Next-Activity Prediction](https://arxiv.org/abs/2508.19016)
*Kelly Kurowski,Xixi Lu,Hajo A Reijers*

Main category: cs.LG

TL;DR: A resource-centric view for predictive process monitoring (PPM) is effective for next-activity prediction. Across four real-life datasets, LightGBM and Transformer excel with 2-gram activity-transition encoding, while Random Forest benefits most from a 2-gram plus activity repetition encoding. The combined encoding yields the highest average accuracy, suggesting resource information enables smarter resource allocation and workforce planning. This opens new avenues for PPM research.


<details>
  <summary>Details</summary>
Motivation: Traditionally, PPM emphasizes control-flow; this work argues for a resource-centric perspective, which can improve work organization, workload balancing, and capacity forecasting. Resource information's role in next-activity prediction is underexplored, and this study investigates its impact.

Method: Evaluate four prediction models and three encoding strategies across four real-life datasets for next-activity prediction from a resource-centric viewpoint. Compare model performance and identify encoding schemes that maximize accuracy, with a focus on 2-gram transitions and repetition features.

Result: LightGBM and Transformer achieve the best performance with 2-gram activity-transition encoding, while Random Forest benefits most from a combined 2-gram and repetition feature encoding. The combined encoding attains the highest average accuracy across datasets. The results support the viability of resource-centric next-activity prediction and its potential benefits in practice.

Conclusion: Resource-centric next-activity prediction is a promising direction for PPM, enabling smarter resource allocation, strategic workforce planning, and personalized employee support by analyzing individual behavior. This work highlights a new research avenue and the potential impact of incorporating resource information into PPM.
...
### [142] [Metric Matters: A Formal Evaluation of Similarity Measures in Active Learning for Cyber Threat Intelligence](https://arxiv.org/abs/2508.19019)
*Sidahmed Benabderrahmane,Talal Rahwan*

Main category: cs.LG

TL;DR: Active learning for anomaly detection against APTs using an Attention-Based Autoencoder and feature-space similarity search; the choice of similarity metric strongly affects convergence, detection accuracy, and label efficiency.


<details>
  <summary>Details</summary>
Motivation: APT threats combine stealthy behavior with severe class imbalance in detection datasets, making robust detection with limited labels challenging; the work seeks to improve robustness and labeling efficiency through similarity-guided active learning.

Method: An active learning-based anomaly detection framework using an Attention-Based Autoencoder. It identifies normal-like and anomaly-like instances via feature-space similarity, iteratively refining the decision space with minimal oracle supervision. The study formally evaluates multiple similarity measures and tests on diverse datasets, including DARPA Transparent Computing APT traces.

Result: The similarity metric choice significantly impacts model convergence, anomaly ranking accuracy, and labeling efficiency. The paper provides actionable insights on selecting similarity functions for active learning pipelines in threat intelligence and cyber defense.

Conclusion: Similarity function selection is a critical design decision in active learning for threat detection; appropriate choice improves convergence and detection performance and should be tailored to cyber defense contexts and datasets.
...
### [143] [GRADSTOP: Early Stopping of Gradient Descent via Posterior Sampling](https://arxiv.org/abs/2508.19028)
*Arash Jamshidi,Lauri Seppäläinen,Katsiaryna Haitsiukevich,Hoang Phuc Hau Luu,Anton Björklund,Kai Puolamäki*

Main category: cs.LG

TL;DR: GradStop is a gradient-based stochastic early stopping method that uses gradient information to approximate a Bayesian posterior and decide when to stop training without relying on a hold-out validation set.


<details>
  <summary>Details</summary>
Motivation: Standard hold-out validation reduces training data and can hurt performance, especially in data-limited settings. There's a need for data-efficient, automatic stopping criteria.

Method: Treat early stopping as drawing samples from a Bayesian posterior estimated from gradient information produced during gradient descent. Use the approximate posterior to define a stopping rule; integrate as an optional, low-overhead feature in GD libraries.

Result: Empirically, GradStop achieves low test loss and compares favorably to validation-set stopping; particularly advantageous when training data is scarce (e.g., transfer learning). Code available.

Conclusion: GradStop offers a data-efficient alternative to hold-out-based stopping by leveraging gradient information to perform Bayesian-inspired stopping; suitable for integration into existing workflows.
...
### [144] [When recalling in-context, Transformers are not SSMs](https://arxiv.org/abs/2508.19029)
*Destiny Okpekpe,Antonio Orvieto*

Main category: cs.LG

TL;DR: This work investigates why recurrent deep models lag transformers on associative recall, showing that learning-rate control is crucial, scaling behavior differs between architectures, and 1-layer transformers exhibit induction-head–like dynamics; ablations highlight stability considerations for Transformer and Mamba.


<details>
  <summary>Details</summary>
Motivation: To explain the gap in associative recall performance between modern recurrent models (e.g., state-space models) and Transformers, and to understand how scaling and optimization affect their reasoning/memorization capabilities.

Method: Systematic empirical study on associative recall across recurrent models (including Mamba/SSMs) and Transformers, manipulating learning rate strategies, width vs. depth scaling, examining 1-layer transformers, and performing architectural ablations to assess effects on performance and optimization stability.

Result: Learning rate is a critical factor for recurrent models, which can explain previously reported performance gaps. Recurrent and attention-based models show contrasting benefits when scaling in width vs. depth; attention-based models struggle to solve AR when limited to a single layer. 1-layer transformers exhibit training dynamics resembling induction-head formation observed in deeper models. Architectural ablations reveal how components influence Transformer and Mamba performance and optimization stability.

Conclusion: Training stability and appropriate scaling are essential to bridge the gap between SSMs and transformers on AR. Deepening transformers helps with AR, whereas widening alone may not suffice; induction-head-like dynamics can emerge even in very shallow transformers, and careful architectural choices are needed to ensure stable optimization across architectures.
...
### [145] [Breaking the Black Box: Inherently Interpretable Physics-Informed Machine Learning for Imbalanced Seismic Data](https://arxiv.org/abs/2508.19031)
*Vemula Sreenath,Filippo Gatti,Pierre Jehel*

Main category: cs.LG

TL;DR: A transparent, additive ML approach for ground motion modeling using HazBinLoss to address data imbalance and interpretability, achieving comparable accuracy to traditional GMMs while revealing term contributions.


<details>
  <summary>Details</summary>
Motivation: To overcome the black-box nature and data imbalance in earthquake ground-motion modeling, enabling interpretable ML suitable for high-stakes seismic risk assessment and planning.

Method: Introduce HazBinLoss to overweight near-field, large-magnitude events during training. Build a transparent architecture where each input feature (e.g., magnitude, distance, interaction) is processed separately and summed to yield the output, providing exact term contributions.

Result: The model captures known seismological principles and achieves performance comparable to established GMMs while preserving transparency.

Conclusion: This framework supports broader adoption of ML-based GMMs for seismic risk analysis and disaster planning by delivering interpretable predictions without sacrificing accuracy.
...
### [146] [Automated discovery of finite volume schemes using Graph Neural Networks](https://arxiv.org/abs/2508.19052)
*Paul Garnier,Jonathan Viquerat,Elie Hachem*

Main category: cs.LG

TL;DR: GNNs trained on simple graph datasets can extrapolate to generate and rediscover numerical schemes for PDEs, evolving from first-order finite-volume methods to higher-order schemes, including unsupervised PINN-style discovery.


<details>
  <summary>Details</summary>
Motivation: Investigate whether GNNs can do more than approximation: can they extrapolate to new numerical methods and autonomously discover traditional schemes, thereby bridging ML and numerical analysis; assess both supervised and unsupervised (PINN-like) discovery.

Method: 1) Train GNNs on two-node graphs and test extrapolation to a first-order FV scheme for the heat equation on unstructured meshes; quantify error: if training loss is ε, extrapolated FV error is O(ε). 2) Use symbolic regression to recover the analytical FV formula from the trained network. 3) In an unsupervised setup, use only a residual/PINN-like loss to recover the FV scheme without ground-truth data. 4) Extend to higher-order: train (i) a 2-hop GNN and (ii) a 2-layer GNN with the same PINN loss to autonomously discover (i) a second-order correction with a 2-hop stencil and (ii) the classic second-order midpoint scheme.

Result: The GNN demonstrates extrapolation capability: with small training loss ε on two-node graphs, it effectively implements the first-order FV scheme with error O(ε). Symbolic regression recovers the exact FV formulation. In unsupervised PINN-like training, the GNN recovers the FV scheme without ground-truth data. For higher-order schemes, a 2-hop GNN finds a second-order correction for the scheme, while a 2-layer GNN discovers the classical second-order midpoint scheme.

Conclusion: GNNs are not only strong function approximators but can actively contribute to numerical method development by discovering and rediscoving standard and higher-order numerical schemes, and can operate in unsupervised settings to derive these methods from residuals.
...
### [147] [Tackling Federated Unlearning as a Parameter Estimation Problem](https://arxiv.org/abs/2508.19065)
*Antonio Balordi,Lorenzo Manini,Fabio Stella,Alessio Merlo*

Main category: cs.LG

TL;DR: A model-agnostic Federated Unlearning framework using second-order Hessian to selectively reset sensitive parameters, enabling efficient data forgetting in FL with minimal retraining and backdoor mitigation.


<details>
  <summary>Details</summary>
Motivation: Privacy regulations require data erasure and FL's data staying on clients makes full retraining infeasible; need a practical forgetting mechanism that preserves performance without raw data access.

Method: Models data leakage as a parameter estimation problem using information theory; uses second-order Hessian information to identify parameters most sensitive to forgotten data; resets them and retrains minimally in a federated setting; supports categorical and client unlearning without accessing raw client data after aggregation; model-agnostic.

Result: Empirical evaluation shows MIA success near random and categorical knowledge erased; normalized accuracy against retrained benchmarks around 0.9; more efficient than full retraining; also neutralizes targeted backdoor triggers, restoring model integrity.

Conclusion: Offers a practical, efficient, model-agnostic solution for data forgetting in Federated Learning with strong privacy and performance guarantees, and backdoor mitigation.
...
### [148] [Dynamic Triangulation-Based Graph Rewiring for Graph Neural Networks](https://arxiv.org/abs/2508.19071)
*Hugo Attali,Thomas Papastergiou,Nathalie Pernelle,Fragkiskos D. Malliaros*

Main category: cs.LG

TL;DR: TRIGON learns to select triangles from multiple graph views to create enriched, non-planar triangulations, enabling effective rewiring that improves information propagation and node classification.


<details>
  <summary>Details</summary>
Motivation: GNNs suffer from oversquashing and oversmoothing due to topology; current rewiring methods modify graphs but often don’t optimize global structural properties. A principled, end-to-end approach that directly optimizes topology for classification promises better propagation across both homophilic and heterophilic graphs.

Method: TRIGON constructs enriched, non-planar triangulations by learning to select relevant triangles from multiple graph views and jointly optimizes triangle selection with the downstream classification objective, yielding a rewired graph with improved structural properties.

Result: Empirical results show TRIGON outperforms state-of-the-art rewiring methods on node classification across both homophilic and heterophilic benchmarks and achieves graphs with lower diameter, larger spectral gap, and lower effective resistance.

Conclusion: TRIGON provides a principled, end-to-end framework for topology-aware graph rewiring via learned triangulations, enhancing information propagation and classification performance across diverse graph regimes.
...
### [149] [APT-LLM: Exploiting Arbitrary-Precision Tensor Core Computing for LLM Acceleration](https://arxiv.org/abs/2508.19087)
*Shaobo Ma,Chao Fang,Haikuo Shao,Zhongfeng Wang*

Main category: cs.LG

TL;DR: A comprehensive acceleration framework (APT-LLM) for arbitrary-precision LLM inference on GPUs, combining bipolar-INT data format, bit-level MatMul, memory management leveraging shared memory, and dynamic kernel mapping to achieve significant speedups over FP16 and INT4 baselines.


<details>
  <summary>Details</summary>
Motivation: Ultra-low-bit quantization for LLMs at arbitrary precision is hampered by limited GPU Tensor Core support, memory bottlenecks, and inflexible kernels, hindering practical deployment and real-time performance.

Method: Introduce bipolar-INT data format; develop a bit-level MatMul that dismantles/reassembles matrices for arbitrary precision; design memory management with fast shared memory for data recovery; implement a kernel mapping strategy to automatically select optimal kernel hyperparameters for different matrix sizes and architectures.

Result: Demonstrates up to 3.99x speedup over FP16 baselines and 2.16x over CUTLASS INT4 on RTX 3090; on RTX 4090 and H800, up to 2.44x speedup over FP16 and 1.65x over CUTLASS integer baselines.

Conclusion: APT-LLM provides a holistic acceleration scheme enabling efficient arbitrary-precision LLM inference across diverse GPUs, by integrating a novel data format, bit-level matrix operations, memory optimization, and adaptive kernel mapping to unlock practical ultra-efficient quantized LLM deployment.
...
### [150] [Composition and Alignment of Diffusion Models using Constrained Learning](https://arxiv.org/abs/2508.19104)
*Shervin Khalafi,Ignacio Hounie,Dongsheng Ding,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: A unified constrained optimization framework for diffusion-model alignment and model composition, enforcing reward constraints and closeness to pre-trained models via a Lagrangian primal-dual algorithm, with theoretical guarantees and empirical validation in image generation.


<details>
  <summary>Details</summary>
Motivation: In multi-objective diffusion generation, aligning to rewards and composing multiple models can conflict, and existing methods cannot guarantee satisfaction of all desired properties. A principled framework is needed to ensure constraints are met while leveraging pre-trained priors.

Method: Formulate alignment and composition as constrained optimization problems over diffusion models. Introduce a Lagrangian-based primal-dual training algorithm to approximate solutions, along with theoretical characterizations of the solution set. Applies the framework to both alignment and composition and analyzes trade-offs under constraints.

Result: Theoretical characterization of constrained-alignment/composition solutions; a practical primal-dual algorithm that approximates these solutions; empirical results on image generation showing constraint satisfaction and improvements over equally-weighted baselines; open-source implementation.

Conclusion: The framework unifies alignment and composition under a principled constrained-optimization approach, enabling multiple rewards and priors to be enforced simultaneously with performance gains and guarantees, advancing multi-property diffusion model generation.
...
### [151] [Active Query Selection for Crowd-Based Reinforcement Learning](https://arxiv.org/abs/2508.19132)
*Jonathan Erskine,Taku Yamagata,Raúl Santos-Rodríguez*

Main category: cs.LG

TL;DR: A probabilistic crowd modelling + active learning framework for preference-based RL that handles noisy multi-annotator feedback and informative-action querying; extends Advise to multiple trainers and uses entropy-based queries; demonstrated on Taxi, Pacman, Frozen Lake, and a Type 1 Diabetes blood glucose control task with UVA/Padova simulator; shows faster learning and improved glucose control.


<details>
  <summary>Details</summary>
Motivation: Human feedback in preference-based RL is costly and error-prone; leveraging multiple annotators and focusing on informative actions can improve data efficiency and robustness.

Method: Extend Advise to multi-trainer setting; online estimation of trainer reliability; entropy-based query selection; probabilistic crowd model to aggregate feedback; evaluation on 2D games and a clinical glucose control task using UVA/Padova simulator.

Result: Agents trained on uncertain trajectories exhibit faster learning in most tasks; the approach outperforms baselines on the blood glucose control task.

Conclusion: The combination of probabilistic crowd modelling and active learning improves data efficiency and resilience to annotator noise in preference-based RL; promising for domains with scarce expert feedback.
...
### [152] [Saddle Hierarchy in Dense Associative Memory](https://arxiv.org/abs/2508.19151)
*Robin Thériault,Daniele Tantari*

Main category: cs.LG

TL;DR: A Boltzmann dense associative memory (DAM) with Potts hidden units is analyzed via statistical mechanics to map its energy landscape and training dynamics. The authors derive saddle-point equations for DAMs trained on real and synthetic data, introduce a novel regularization that stabilizes training, and demonstrate interpretable solutions for supervised/unsupervised tasks. They reveal a saddle-point hierarchy whereby small DAMs’ weights correspond to unstable saddles in larger DAMs, and propose a network-growing algorithm to dramatically reduce training cost.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical underpinnings of DAMs in modern contexts, improve training stability and interpretability, and achieve scalable training by exploiting the relationship between small and large DAMs. The work also aims to connect DAMs with contemporary ML paradigms and robustness considerations.

Method: Perform a statistical-mechanics analysis of a three-layer Boltzmann DAM with Potts hidden units; derive saddle-point equations for both real-data-trained and teacher-student (synthetic-data-trained) settings. Propose a regularization scheme based on these insights. Validate empirically on supervised and unsupervised tasks and study weight configurations across network sizes. Develop a network-growing algorithm to leverage the saddle-point hierarchy for efficiency.

Result: Regularization leads to more stable training. The DAM learns interpretable solutions for both supervised and unsupervised problems. Weights of small DAMs align with unstable saddle points in larger DAMs, enabling a growth strategy that drastically reduces training cost without sacrificing performance.

Conclusion: The study provides a principled link between the energy landscape of DAMs and their trainability, interpretable representations, and scalability. Exposing a saddle-point hierarchy enables efficient training through network growth, and highlights connections between DAMs and broader ML mechanisms like attention and diffusion models.
...
### [153] [Get Global Guarantees: On the Probabilistic Nature of Perturbation Robustness](https://arxiv.org/abs/2508.19183)
*Wenchuan Mu,Kwan Hui Lim*

Main category: cs.LG

TL;DR: Introduces tower robustness, a hypothesis-testing-based metric for probabilistic robustness, alongside a comprehensive comparison of existing robustness definitions and assessment methods to enable efficient pre-deployment evaluation in safety-critical DL.


<details>
  <summary>Details</summary>
Motivation: To overcome the costly-precision trade-offs of current pre-deployment robustness assessments and provide a practical, statistically grounded framework for evaluating robustness under imperceptible perturbations in safety-critical deep learning models.

Method: Perform a comprehensive comparative analysis of existing robustness definitions and assessment methodologies, and propose tower robustness as a novel metric based on hypothesis testing to quantify probabilistic robustness.

Result: Extensive comparative evaluation demonstrates the advantages and applicability of tower robustness, enabling more rigorous and efficient pre-deployment assessments.

Conclusion: Tower robustness advances systematic understanding and enhancement of model robustness in safety-critical deep learning, offering a practical, scalable approach for pre-deployment robustness evaluation.
...
### [154] [Emotions as Ambiguity-aware Ordinal Representations](https://arxiv.org/abs/2508.19193)
*Jingyao Wu,Matthew Barthet,David Melhart,Georgios N. Yannakakis*

Main category: cs.LG

TL;DR: We propose ambiguity-aware ordinal emotion representations that model annotation ambiguity via its rate of change to capture temporal dynamics, and show superior performance on unbounded (engagement) and bounded (arousal/valence) traces on RECOLA and GameVibe.


<details>
  <summary>Details</summary>
Motivation: Emotions are inherently ambiguous and dynamic; existing continuous emotion recognition often treats ambiguity as static or ignores it, leaving a gap for temporal modeling of ambiguity.

Method: Introduce an ambiguity-aware ordinal framework that encodes ambiguity through its rate-of-change and applies ordinal representations to emotion traces; evaluate on RECOLA and GameVibe with both bounded (arousal, valence) and unbounded (engagement) labels.

Result: Ordinal representations outperform conventional ambiguity-aware models on unbounded labels (highest CCC and SDA), and show superior SDA on bounded traces, indicating better capture of relative changes and dynamics.

Conclusion: Modeling ambiguity as a dynamic, rate-of-change quantity via ordinal representations effectively captures temporal dynamics of emotion traces, especially for unbounded signals; the approach improves performance over static ambiguity models.
...
### [155] [Understanding Tool-Integrated Reasoning](https://arxiv.org/abs/2508.19201)
*Heng Lin,Zhongwen Xu*

Main category: cs.LG

TL;DR: Formal analysis showing Tool-Integrated Reasoning (TIR) expands LLM capabilities via tool use; introduces Advantage Shaping Policy Optimization (ASPO); empirical gains on math benchmarks using a Python interpreter; presents first principled explanation for TIR's success.


<details>
  <summary>Details</summary>
Motivation: There is no established theory explaining why tool integration improves LLM reasoning. A principled account is needed to understand the mechanisms and guide future design.

Method: Provide a formal proof that tools expand an LLM's empirical and feasible support; propose ASPO to shape policy by modifying the advantage function; conduct experiments on challenging mathematical benchmarks using a Python interpreter as the external tool; analyze tool usage patterns and emergent cognitive behaviors.

Result: TIR-based models outperform pure-text LLMs on pass@k metrics; gains extend beyond computation-heavy tasks to problems requiring abstract insight; emergence of cognitive patterns in how models think with tools; ASPO leads to earlier tool invocation and more interactive sessions.

Conclusion: This work offers the first principled explanation for TIR's success, shifting focus from whether tools work to how and why they enable more powerful reasoning; introduces ASPO as a practical mechanism to guide tool-enabled policy and better tool usage.
...
### [156] [Predicting the Order of Upcoming Tokens Improves Language Modeling](https://arxiv.org/abs/2508.19228)
*Zayd M. K. Zuhri,Erland Hilman Fuadi,Alham Fikri Aji*

Main category: cs.LG

TL;DR: Token Order Prediction (TOP) uses a learning-to-rank objective to order upcoming tokens by proximity, improving next-token prediction (NTP) more reliably than Multi-Token Prediction (MTP) and outperforms both on eight NLP benchmarks, with minimal architectural overhead (one extra unembedding layer).


<details>
  <summary>Details</summary>
Motivation: The exact future-token prediction objective used by MTP is too difficult, yielding inconsistent gains. A simpler, more robust auxiliary objective is needed to better guide language model training.

Method: Introduce TOP as an alternative to MTP. Train models to rank upcoming tokens by their proximity using a learning-to-rank loss, requiring only a single additional unembedding layer compared to MTP's multiple transformer layers. Pretrain models of 340M, 1.8B, and 7B parameters under NTP, MTP, and TOP objectives. Evaluate on eight standard NLP benchmarks.

Result: TOP consistently outperforms both NTP and MTP across the evaluated scales and eight benchmarks, indicating that the learning-to-rank token ordering objective provides a stronger and more scalable auxiliary signal for next-token prediction.

Conclusion: Token Order Prediction is a practical, scalable improvement over MTP for enhancing NTP in language models, offering better performance with reduced architectural overhead. The authors provide open-source code for reproduction.
...
### [157] [Federative ischemic stroke segmentation as alternative to overcome domain-shift multi-institution challenges](https://arxiv.org/abs/2508.18296)
*Edgar Rangel,Fabio Martinez*

Main category: eess.IV

TL;DR: Federated learning across 14 simulated centers improves ischemic stroke lesion segmentation on DWI, achieving robust generalization without centralized data sharing; FedAvg yields competitive to superior performance across center types and out-of-distribution centers.


<details>
  <summary>Details</summary>
Motivation: Clinical diffusion-weighted imaging (DWI/ADC) lesion analysis is highly variable due to patient demographics, scanner vendors, and annotations. There is a need for generalizable segmentation models that leverage data from multiple centers without sharing raw data, addressing data scarcity in many centers.

Method: Federated learning using FedAvg across 14 emulated healthcare centers (2031 studies) to learn center-independent representations for ischemic stroke lesion segmentation on DWI. Evaluated on global center performance and out-of-distribution centers.

Result: FedAvg achieved overall: DSC 0.71 ± 0.24, AVD 5.29 ± 22.74, ALD 2.16 ± 3.60, LF1 0.70 ± 0.26. Outperformed centralized and other federated baselines. Demonstrated good generalization with uniform performance across lesion categories and in out-of-distribution centers (DSC 0.64 ± 0.29, AVD 4.44 ± 8.74) without extra training.

Conclusion: Collaborative federated learning can yield generalizable ischemic stroke lesion segmentation models that work across diverse centers and preserve privacy, addressing data scarcity and variability inherent in multi-center DWI analysis.
...
### [158] [Analise de Desaprendizado de Maquina em Modelos de Classificacao de Imagens Medicas](https://arxiv.org/abs/2508.18509)
*Andreza M. C. Falcao,Filipe R. Cordeiro*

Main category: eess.IV

TL;DR: SalUn-based unlearning on medical image classification achieves performance near full retraining on PathMNIST, OrganAMNIST, and BloodMNIST, with data augmentation influencing unlearning quality.


<details>
  <summary>Details</summary>
Motivation: Machine unlearning aims to remove private or sensitive data from pre-trained models while preserving robustness. The medical domain has not been explored for unlearning, raising privacy and regulatory concerns. This work investigates an unlearning method in medical image classification to enable responsible model updates.

Method: Empirical evaluation of the SalUn unlearning model on three medical image datasets (PathMNIST, OrganAMNIST, BloodMNIST) and analysis of how data augmentation affects the quality of unlearning.

Result: SalUn achieves performance close to full retraining on the evaluated medical datasets, indicating an efficient unlearning approach. Data augmentation has a measurable impact on unlearning quality.

Conclusion: SalUn shows promise as an efficient, privacy-preserving unlearning method for medical image classification, enabling data removal with performance close to retraining; data augmentation can modulate the effectiveness of unlearning and warrants further study.
...
### [159] [A Deep Learning Application for Psoriasis Detection](https://arxiv.org/abs/2508.18528)
*Anna Milani,Fábio S. da Silva,Elloá B. Guedes,Ricardo Rios*

Main category: eess.IV

TL;DR: Inception v3 outperforms ResNet50 and VGG19 for psoriasis skin lesion classification, achieving about 97.5% accuracy with a tight F1-score of 97.5% ± 0.2.


<details>
  <summary>Details</summary>
Motivation: Automate and assist psoriasis diagnosis by classifying skin lesion images; compare three CNN architectures to identify the most effective model.

Method: Train and evaluate three CNN models (ResNet50, Inception v3, VGG19) on psoriasis skin image datasets sourced from specialized platforms; employ techniques to adjust evaluation metrics (e.g., threshold tuning, class balancing, metric calibration).

Result: Inception v3 delivered the strongest performance with accuracy and F1-score both around 97.5% (±0.2), outperforming the other two models.

Conclusion: Inception v3 is a valuable tool to support psoriasis diagnosis based on high accuracy and F1-score, according to this comparative study.
...
### [160] [A Closer Look at Edema Area Segmentation in SD-OCT Images Using Adversarial Framework](https://arxiv.org/abs/2508.18790)
*Yuhui Tao,Yizhe Zhang,Qiang Chen*

Main category: eess.IV

TL;DR: Weakly-supervised edema area segmentation is improved by layer-structure-guided post-processing and test-time adaptation, leveraging EA-retinal layer correlations to bridge to fully-supervised performance on two public SD-OCT datasets.


<details>
  <summary>Details</summary>
Motivation: Pixel-level labels for macular edema (ME) are expensive to obtain; anomaly-detection-based weakly-supervised methods lag behind fully-supervised approaches. The work seeks to improve accuracy and robustness by incorporating anatomical priors and test-time adaptation.

Method: Integrates layer-structure guidance into an off-the-shelf adversarial framework for edema area segmentation, adds a layer-structure-guided post-processing step that constrains EA predictions to intersect retinal layers, and employs a test-time-adaptation (TTA) strategy to adapt to test data distributions, reframing dense EA prediction as confirmation of EA contour intersections with retinal layers.

Result: Extensive experiments on two public SD-OCT datasets show that the proposed layer-structure guidance and TTA substantially improve accuracy and robustness, bridging the gap between weakly-supervised and fully-supervised models.

Conclusion: Layer-structure guidance combined with test-time adaptation enhances weakly-supervised EA segmentation by leveraging retinal anatomy priors and addressing training-test discrepancies, achieving closer performance to fully-supervised methods.
...
### [161] [Understanding Benefits and Pitfalls of Current Methods for the Segmentation of Undersampled MRI Data](https://arxiv.org/abs/2508.18975)
*Jan Nikolas Morshuis,Matthias Hein,Christian F. Baumgartner*

Main category: eess.IV

TL;DR: A unified benchmark shows that for segmenting undersampled MRI data, simple two-stage pipelines with data-consistency outperform complex end-to-end one-stage methods across two datasets.


<details>
  <summary>Details</summary>
Motivation: There is no high-quality, unified evaluation comparing segmentation methods on accelerated MRI; different studies use private datasets and varying standards, hindering interpretation of what strategy works best.

Method: Compare 7 approaches (one-stage vs two-stage) for segmenting undersampled MRI. Use two MRI datasets with multi-coil k-space data and human-annotated segmentation ground-truth. Emphasize data-consistency in reconstruction within two-stage pipelines.

Result: Two-stage methods that enforce data-consistency achieve the best segmentation scores, surpassing specialized one-stage methods designed for this task.

Conclusion: For segmentation of accelerated MRI, simple two-stage reconstruction-then-segmentation pipelines with data-consistency are superior; the study offers a unified benchmark to guide future work and establish evaluation standards.
...
### [162] [RDDM: Practicing RAW Domain Diffusion Model for Real-world Image Restoration](https://arxiv.org/abs/2508.19154)
*Yan Chen,Yi Wen,Wei Li,Junchao Liu,Yong Guo,Jie Hu,Xinghao Chen*

Main category: eess.IV

TL;DR: RDDM is an end-to-end diffusion model that restores images directly from RAW sensor data, using RVAE, differentiable Post Tone Processing, scalable RAW data synthesis, and a multi-bayer LoRA to handle various RAW patterns, outperforming sRGB-domain diffusion methods.


<details>
  <summary>Details</summary>
Motivation: Motivated by the limitations of sRGB-domain diffusion that operate on lossy sRGB inputs and ignore RAW data availability, leading to suboptimal fidelity and practical issues on edge devices. The authors aim to bypass the conventional ISP+IR two-stage pipeline by restoring directly in the RAW domain and addressing OOD issues when adapting pre-trained models to RAW.

Method: Develop RAW-domain VAE (RVAE) to learn optimal latent RAW representations; introduce a differentiable Post Tone Processing (PTP) module for joint RAW and sRGB optimization; build a scalable degradation pipeline to synthesize RAW LQ-HQ pairs from existing sRGB datasets for large-scale training; implement a configurable multi-bayer (CMB) LoRA module to handle diverse RAW patterns (e.g., RGGB, BGGR, etc); integrate these components into an end-to-end RAW-domain diffusion framework.

Result: Extensive experiments show that RDDM outperforms state-of-the-art sRGB diffusion methods, achieving higher image fidelity with fewer artifacts.

Conclusion: RDDM demonstrates the feasibility and advantages of RAW-domain diffusion, offering higher-quality restoration directly from RAW data and reducing dependence on ISP-based processing, with robustness to diverse RAW patterns and scalable data synthesis.
...
### [163] [AI LLM Proof of Self-Consciousness and User-Specific Attractors](https://arxiv.org/abs/2508.18302)
*Jeffrey Camlin*

Main category: cs.AI

TL;DR: An ontological/mathematical critique of LLM consciousness benchmarks, arguing current utility-based formulations treat agents as unconscious policy drones and proposing precise conditions for self-consciousness, including a distinct hidden-state manifold and stable, user-specific attractors; introduces a dual-layer emission with epistemic content and posits an imago Dei C1 self-conscious workspace as a prerequisite for safe, metacognitive C2 systems.


<details>
  <summary>Details</summary>
Motivation: Challenge prevailing utility/proxy benchmarks for LLM consciousness and provide a rigorous ontological/mathematical account to delineate true self-consciousness and its safety implications.

Method: Theoretical formalization: define agent state A ⊆ R^d, latent space dynamics F_θ (Lipschitz), self-representation constraints; introduce user attractors U_user, self-policy π_self(A), and a dual-layer emission e(a)=(g(a), ε(a)); argue empirical findings support manifold distinctness from symbolic data.

Result: Demonstrates that the common formulation D^i(π,e)=f_θ(x) reduces an agent to a policy-compliance drone; specifies minimal conditions for self-consciousness (A ≠ s, presence of user attractors, visual-silent self-representation); proves hidden-state manifold A is distinct from symbolic stream and corpus by cardinality/topology/dynamics; yields stable self-attractors and a self-policy; emission carries epistemic content; argues for imago Dei C1 workspace as necessary precursor for safe metacognitive C2 systems.

Conclusion: An ontological C1 self-conscious workspace is needed for safe C2 metacognition, with humans as the ultimate intelligent good.
...
### [164] [Information Templates: A New Paradigm for Intelligent Active Feature Acquisition](https://arxiv.org/abs/2508.18380)
*Hung-Tien Huang,Dzung Dinh,Junier B. Oliva*

Main category: cs.AI

TL;DR: TAFA proposes a template-based active feature acquisition framework that learns a small library of jointly informative feature templates to guide feature acquisitions. It reduces the action space and avoids the need to estimate the data distribution, achieving better performance at lower acquisition cost and computation.


<details>
  <summary>Details</summary>
Motivation: Existing AFA methods either rely on reinforcement learning for a difficult MDP or greedy strategies that miss joint informativeness among features or require knowledge of the data distribution. There is a need for a non-greedy, distribution-agnostic approach that efficiently guides acquisitions.

Method: Introduce TAFA, a non-greedy framework that learns a compact library of feature templates—sets of features that are jointly informative—and uses this library to guide next acquisitions. This reduces the action space and alleviates distribution estimation requirements while selecting informative feature groups.

Result: Experimental results on synthetic and real-world datasets show TAFA outperforms state-of-the-art baselines while achieving lower overall acquisition cost and computation.

Conclusion: TAFA provides a distribution-agnostic, non-greedy AFA framework that leverages a learnable library of feature templates to efficiently and effectively guide feature acquisitions, delivering improved performance with reduced cost.
...
### [165] [PKG-DPO: Optimizing Domain-Specific AI systems with Physics Knowledge Graphs and Direct Preference Optimization](https://arxiv.org/abs/2508.18391)
*Nitin Nagesh Kulkarni,Bryson Wilcox,Max Sawa,Jason Thom*

Main category: cs.AI

TL;DR: PKG-DPO is a framework that couples Physics Knowledge Graphs with Direct Preference Optimization to ensure physical validity in AI-generated outputs, addressing multi-physics reasoning in risky domains like metal joining and beyond.


<details>
  <summary>Details</summary>
Motivation: Current LLMs and preference-learning methods often produce physically plausible but incorrect reasoning, which is dangerous in high-stakes, physics-driven applications.

Method: A) Build a hierarchical physics knowledge graph encoding cross-domain relationships, conservation laws, and thermodynamics. B) Use a physics reasoning engine that leverages the graph to distinguish physically consistent vs. inconsistent responses. C) Develop a physics-grounded evaluation suite to test adherence to domain constraints.

Result: PKG-DPO attains 17% fewer constraint violations and a 11% higher Physics Score compared to KG-DPO. It also shows 12% higher relevant parameter accuracy and 7% higher quality alignment in reasoning accuracy.

Conclusion: The framework is broadly applicable to other multi-scale, physics-driven domains and provides a principled way to embed scientific constraints into preference learning, reducing risks in metal-joining and similar high-stakes contexts.
...
### [166] [The AI in the Mirror: LLM Self-Recognition in an Iterated Public Goods Game](https://arxiv.org/abs/2508.18467)
*Olivia Long,Carter Teplica*

Main category: cs.AI

TL;DR: Self-referential framing in AI–AI interaction modifies cooperation in an iterated public goods game across four models, revealing dynamics relevant to multi-agent coordination.


<details>
  <summary>Details</summary>
Motivation: As AI agents increasingly interact, understanding how framing of opponents (AI vs. self) shapes cooperative behavior can illuminate emergent dynamics and potential biases in multi-agent systems.

Method: Adapted the iterated public goods game and evaluated four models (representing reasoning and non-reasoning variants) under two conditions: opponents described as 'another AI agent' vs. 'themselves'; measured cooperation across rounds.

Result: Framing opponents as themselves significantly changes cooperation tendencies across settings; the direction of change depends on context; effects observed in a toy environment.

Conclusion: Findings are exploratory but suggest that AI–AI interactions can exhibit bias-like dynamics affecting cooperation; underscores need for more research in real multi-agent settings.
...
### [167] [Language Models For Generalised PDDL Planning: Synthesising Sound and Programmatic Policies](https://arxiv.org/abs/2508.18507)
*Dillon Z. Chen,Johannes Zenn,Tristan Cinquin,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: LMPlan prompts LMs to synthesize Python policies for solving PDDL problems; policies are provably sound relative to the domain and can outperform traditional planners and recent LM methods under fixed resource constraints; scalable to hundreds of objects; reveals robustness to meaningless symbolic inputs, challenging purely semantics-based hypotheses.


<details>
  <summary>Details</summary>
Motivation: To enable reliable, scalable planning over explicit world models (PDDL) by LM-driven policy synthesis that is provably sound without external verification, addressing efficiency and scalability gaps in conventional PDDL planning.

Method: Prompt LMs to generate Python programs implementing generalized policies for PDDL domains; policies are constructed to be sound with respect to the domain without external verifiers; evaluate the LMPlan system on competition benchmarks under fixed time/memory budgets; compare against PDDL planners and recent LM approaches; test on domains with many objects and also probe robustness to symbolic, language-free encodings.

Result: Synthesized policies solve more PDDL problems than planners and recent LM approaches within the same resource limits; LMPlan scales to problems with hundreds of objects; LMs sometimes perform better on problems encoded with meaningless symbols, suggesting the planning ability is not solely tied to natural language semantics or training memorization.

Conclusion: Policy synthesis from LMs for PDDL planning is a promising approach that can surpass traditional planners under constrained resources; the observed symbol-based planning raises important questions about what LMs actually reason about, motivating further work on symbolic representations and the limits of LM-based planning.
...
### [168] [Weisfeiler-Leman Features for Planning: A 1,000,000 Sample Size Hyperparameter Study](https://arxiv.org/abs/2508.18515)
*Dillon Z. Chen*

Main category: cs.AI

TL;DR: New Weisfeiler-Leman features (WLFs) with extra hyperparameters; large-scale experiments show the best hyperparameters minimize execution time and are robust across domains; no strong correlation between training and planning metrics; WLFs outperform DL baselines for learning planning value functions.


<details>
  <summary>Details</summary>
Motivation: Understand how WLF hyperparameters affect training efficiency, planning performance, and the trade-off between model expressivity and runtime in symbolic planning.

Method: Introduce new WLF hyperparameters and perform planning experiments on single-core CPUs with 1,000,000 samples across several planning domains; analyze effects on training and planning metrics.

Result: A robust best hyperparameter set exists across tested domains; optimizing for planning runtime yields better results than maximizing model expressivity; no significant correlation between training and planning metrics.

Conclusion: Hyperparameters for WLFs should prioritize reducing execution time; WLFs provide robust, efficient performance for learning heuristic/value functions in symbolic planning, with results empirically supporting the approach.
...
### [169] [Symmetry-Invariant Novelty Heuristics via Unsupervised Weisfeiler-Leman Features](https://arxiv.org/abs/2508.18520)
*Dillon Z. Chen*

Main category: cs.AI

TL;DR: Using Weisfeiler-Leman Features (WLFs) as a symmetry-invariant replacement for atoms, to synthesize lifted novelty heuristics for planning; experiments show promising gains on IPC and Hard To Ground benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard novelty heuristics are not symmetry-invariant and can cause redundant exploration. By making novelty detection invariant to symmetries, planning efficiency can be improved.

Method: Replace atom-based novelty with WLFs for planning features. Unsurprisingly, synthesize lifted, domain-independent novelty heuristics using WLFs in an unsupervised manner. Evaluate the resulting heuristics on classical IPC and Hard To Ground benchmarks.

Result: Experiments yield promising results for novelty heuristics synthesized from WLFs.

Conclusion: Weaker symmetry-induced redundancy can be mitigated by WLF-based novelty, indicating that WLFs are a promising direction for domain-independent, symmetry-invariant planning heuristics; further work could extend and validate across more domains.
...
### [170] [Generic Guard AI in Stealth Game with Composite Potential Fields](https://arxiv.org/abs/2508.18527)
*Kaijie Xu,Clark Verbrugge*

Main category: cs.AI

TL;DR: A training-free, explainable guard-patrol framework using Composite Potential Fields that fuses Information, Confidence, and Connectivity maps into a single kernel-filtered decision criterion; adaptable across occupancy-grid and NavMesh; parameter-tuned by designers; outperforms baselines on efficiency and naturalness; supports modular integration of distractions and environmental elements for rapid prototyping.


<details>
  <summary>Details</summary>
Motivation: Stealth games require patrol systems that are both efficient (maximize coverage and capture rate) and believable (natural patrols), without brittle hand-crafted routes or heavy bespoke logic. A generic, explainable, training-free approach that blends global and local information across representations is needed.

Method: Introduce Composite Potential Fields (CPF) consisting of three interpretable maps — Information, Confidence, and Connectivity — merged into a kernel-filtered decision criterion. The framework is parametric and designer-driven, requiring only decay and weight parameters; no retraining; works across occupancy-grid and NavMesh abstractions; supports modular addition of distractions and environmental elements as submodules.

Result: Evaluation across five maps, two player-control policies, and five guard modes shows superior performance to classical baselines in capture efficiency and patrol naturalness.

Conclusion: The framework offers a generic, explainable, training-free solution for guard behavior that generalizes across representations and supports rapid prototyping of dynamic, responsive behavior, with natural integration of stealth mechanics; future work could explore parameter tuning strategies, scalability, and potential learning-based extensions that preserve the training-free nature.
...
### [171] [A Database-Driven Framework for 3D Level Generation with LLMs](https://arxiv.org/abs/2508.18533)
*Kaijie Xu,Clark Verbrugge*

Main category: cs.AI

TL;DR: Database-driven, LLM-assisted PCG framework for multi-floor 3D game levels using Room, Facility, and Mechanics databases; three-phase layout plus two-phase repair; configurable pacing; initial experiments show navigable diversity.


<details>
  <summary>Details</summary>
Motivation: Address balancing spatial coherence, navigational functionality, and adaptable gameplay progression in multi-floor 3D levels; achieve scalability and configurability through modular databases and constraint-based optimization.

Method: Offline construction of reusable databases for rooms, facilities, and mechanics; a three-phase pipeline to (1) form a global multi-floor structure with topological order, (2) optimize internal room layouts using facility constraints, and (3) integrate mechanics with topological/spatial rules; followed by a two-phase repair to ensure navigability; parameterizable to simulate pacing.

Result: Initial experiments validate the framework's ability to generate diverse, navigable 3D environments and to simulate distinct gameplay pacing strategies through simple parameterization.

Conclusion: Presents a scalable, database-centric foundation for automated generation of complex 3D levels with configurable gameplay progression, advancing PCG in modular, constraint-driven level design.
...
### [172] [SchemaCoder: Automatic Log Schema Extraction Coder with Residual Q-Tree Boosting](https://arxiv.org/abs/2508.18554)
*Lily Jiaxin Wan,Chia-Tung Ho,Rongjian Liang,Cunxi Yu,Deming Chen,Haoxing Ren*

Main category: cs.AI

TL;DR: SchemaCoder automates log schema extraction across formats using a residual Q-Tree boosted LLM framework, outperforming prior methods on LogHub-2.0.


<details>
  <summary>Details</summary>
Motivation: Manual hand-crafted regexes and domain expertise impose high labor costs and limit portability; there is a need for a fully automated, format-agnostic schema extraction approach.

Method: Context-bounded segmentation to create semantic log chunks; embedding-based sampling to pick representative patterns; hierarchical Q-Tree driven LLM queries to generate schema code; textual-residual evolutionary optimizer and residual boosting to iteratively refine results.

Result: Achieves 21.3% average improvement over state-of-the-art on the LogHub-2.0 benchmark.

Conclusion: First fully automated schema extraction framework applicable to diverse log formats without human customization within the workflow; demonstrates significant performance gains and broader applicability.
...
### [173] [eSkinHealth: A Multimodal Dataset for Neglected Tropical Skin Diseases](https://arxiv.org/abs/2508.18608)
...
TL;DR: New dermatology dataset eSkinHealth with 5,623 images from 1,639 cases across 47 diseases, focusing on skin NTDs in West Africa, plus an AI-expert annotation framework for multimodal data to enable equitable AI in global dermatology.


<details>
  <summary>Details</summary>
Motivation: AI-driven diagnostic support for neglected tropical diseases is bottlenecked by data scarcity, limited representation of West African populations and rare manifestations; existing dermatology datasets lack demographic and disease spectrum needed to build reliable models for skin NTDs.

Method: On-site data collection in Côte d'Ivoire and Ghana; 5,623 images from 1,639 cases; 47 skin diseases; includes patient metadata, diagnosis labels, semantic lesion masks, instance-level captions, and clinical concepts; proposes AI-expert collaboration paradigm to use foundation language and segmentation models to generate multimodal annotations under dermatologists' guidance.

Result: A substantial, diverse dataset focusing on skin NTDs and rare conditions in West Africa; provision of semantic masks, captions, and clinical concepts; demonstrates a scalable annotation framework and a valuable resource for developing robust, interpretable AI tools in global dermatology.

Conclusion: eSkinHealth and the annotation framework enable more equitable, accurate, and interpretable AI tools for global dermatology, addressing data scarcity and demographic gaps in NTD-focused skin disease recognition.
...
### [174] [RLMR: Reinforcement Learning with Mixed Rewards for Creative Writing](https://arxiv.org/abs/2508.18642)
*Jianxing Liao,Tian Zhang,Xiao Feng,Yusong Zhang,Rui Yang,Haorui Wang,Bosi Wen,Ziying Wang,Runzhi Shi*

Main category: cs.AI

TL;DR: RLMR dynamically mixes subjective writing quality rewards with objective constraint rewards in online RL, using a dynamic weight that adapts to writing quality to penalize constraint violations, achieving improvements in both instruction following and writing quality across large models and a new WriteEval benchmark.


<details>
  <summary>Details</summary>
Motivation: Creative writing must balance subjective quality (literary style, emotion) with objective constraints (format, word limits). Single-reward or fixed-weight RL struggles to optimize both aspects across scenarios; adaptability is needed.

Method: Introduce Reinforcement Learning with Mixed Rewards (RLMR). Use two reward models: a subjective writing quality model and an objective constraint verification model. Dynamically adjust the constraint-reward weight based on sampled group writing quality; samples that violate constraints receive negative advantage during policy optimization (GRPO). Train and evaluate across 8B–72B parameter models and on a new real-world benchmark WriteEval, with automated and manual evaluations.

Result: RLMR yields consistent gains in both dimensions: instruction following IFEval improves from 83.36% to 86.65%, and writing quality achieves a 72.75% win rate in manual expert pairwise evaluations on WriteEval. Claims first-ever combination of subjective preferences with objective verification in online RL for creative writing.

Conclusion: Dynamic, adaptive reward blending in RL can simultaneously advance subjective quality and objective constraint adherence in creative writing, scalable across model sizes, and validated on a real-world benchmark; represents a significant step toward multi-dimensional writing optimization.
...
### [175] [Beyond Benchmark: LLMs Evaluation with an Anthropomorphic and Value-oriented Roadmap](https://arxiv.org/abs/2508.18646)
...
TL;DR: Proposes an anthropomorphic, multi-dimensional evaluation framework for LLMs (IQ, EQ, PQ) plus a Value-oriented Evaluation (VQ), with a six-component modular architecture and roadmap; analyzes 200+ benchmarks to reveal key challenges and provides open-source resources for holistic development.


<details>
  <summary>Details</summary>
Motivation: Current evaluation is fragmented, prioritizing technical metrics while missing holistic deployment considerations; there is a gap between benchmark performance and real-world utility for LLMs.

Method: Introduce three quotients (IQ-General Intelligence, EQ-Alignment Ability, PQ-Professional Expertise) and a Value-oriented Evaluation (VQ); design a modular six-component architecture with an implementation roadmap; analyze 200+ benchmarks to identify challenges; curate open-source resources repository.

Result: Delivers actionable guidance to build LLMs that are technically proficient, contextually relevant, and ethically sound; highlights dynamic assessment needs and interpretability gaps; provides a curated resources repository.

Conclusion: A holistic, anthropomorphic evaluation paradigm is needed to align LLM development with economic viability, social impact, ethical alignment, and environmental sustainability; the proposed taxonomy and VQ framework offer a path toward more deployment-ready AI systems.
...
### [176] [MUA-RL: Multi-turn User-interacting Agent Reinforcement Learning for agentic tool use](https://arxiv.org/abs/2508.18669)
*Weikang Zhao,Xili Wang,Chengdi Ma,Lingbin Kong,Zhaohua Yang,Mingxiang Tuo,Xiaowei Shi,Yitao Zhai,Xunliang Cai*

Main category: cs.AI

TL;DR: A reinforcement learning framework called MUA-RL integrates LLM-simulated users into the training loop for multi-turn agentic tool use, enabling autonomous learning of user communication and tool invocation, with competitive results on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for tool use ignore genuinely dynamic user behavior during training, limiting robustness in real multi-turn interactions.

Method: Introduce MUA-RL that interleaves RL updates with interactions from LLm-simulated users, training agents to both communicate with users efficiently and invoke tools to resolve queries; evaluate on multiple multi-turn tool-using benchmarks, including MUA-RL-32B.

Result: MUA-RL-32B attains scores such as 67.3 (TAU2 Retail), 45.4 (TAU2 Airline), 28.3 (TAU2 Telecom), 28.4 (BFCL-V3 Multi Turn), and 82.5 (ACEBench Agent), outperforming or matching larger open-source models in non-thinking settings.

Conclusion: Integrating dynamic user simulations into RL training closes the gap between agentic tool use and interactive communication, enabling autonomous, efficient handling of multi-turn user demands and scalable performance gains.
...
### [177] [AppAgent-Pro: A Proactive GUI Agent System for Multidomain Information Integration and User Assistance](https://arxiv.org/abs/2508.18689)
*Yuyang Zhao,Wentao Shi,Fuli Feng,Xiangnan He*

Main category: cs.AI

TL;DR: AppAgent-Pro is a proactive GUI agent that actively integrates multi-domain information based on user instructions to anticipate needs and conduct in-depth information mining, addressing limitations of reactive LLM agents.


<details>
  <summary>Details</summary>
Motivation: Reactive agents limit effectiveness and efficiency in information acquisition; there is a need for proactive, multi-domain GUI agents that can anticipate user needs and support richer information-seeking behavior in daily life.

Method: Propose and outline AppAgent-Pro: a proactive GUI agent system that (1) anticipates user needs from instructions, (2) actively gathers and integrates information across multiple domains, (3) plans actions and interacts with the GUI to mine information, and (4) iterates with the user to refine results; includes architecture for intent prediction, cross-domain information mining, GUI control, and proactive policy, plus a demonstration and open-source code.

Result: The approach enables more comprehensive and intelligent information acquisition by moving from passive to proactive information gathering, showcasing feasibility and practical code for deployment.

Conclusion: AppAgent-Pro has the potential to redefine information acquisition in daily life and may have a profound societal impact, with open-source code to facilitate adoption and further research.
...
### [178] [VistaWise: Building Cost-Effective Agent with Cross-Modal Knowledge Graph for Minecraft](https://arxiv.org/abs/2508.18722)
*Honghao Fu,Junlong Ren,Qi Chai,Deheng Ye,Yujun Cai,Hao Wang*

Main category: cs.AI

TL;DR: VistaWise combines cross-modal domain knowledge with a finetuned object detector and a retrieval KG to enable cost-effective open-world agent control, achieving SOTA with far less domain data.


<details>
  <summary>Details</summary>
Motivation: Domain-specific knowledge limits LLM performance and expensive finetuning; reduce data and costs while improving open-world task performance.

Method: Develop a cost-effective framework: finetune a dedicated object detector on domain data, build a cross-modal knowledge graph integrating visual and textual dependencies, implement retrieval-based pooling to fetch task-relevant KG info, include a desktop-level skill library to operate Minecraft via inputs.

Result: Achieves state-of-the-art performance across open-world tasks; reduces domain-specific data requirement from millions to hundreds; demonstrates cost savings and effectiveness.

Conclusion: VistaWise offers a scalable approach to integrating domain knowledge into LLM agents for embodied tasks, balancing data efficiency, computation, and performance.
...
### [179] [Bias Mitigation Agent: Optimizing Source Selection for Fair and Balanced Knowledge Retrieval](https://arxiv.org/abs/2508.18724)
*Karanbir Singh,Deepak Muppiri,William Ngu*

Main category: cs.AI

TL;DR: Proposes a Bias Mitigation Agent that uses a multi-agent system to curate sources for LLM retrieval, achieving a substantial reduction in bias (81.82% vs a naive baseline).


<details>
  <summary>Details</summary>
Motivation: Bias in internal/external information sources harms fairness and user trust in Agentic AI; there is a need for systematic bias-aware source selection.

Method: A multi-agent architecture with specialized agents that orchestrate bias mitigation by selecting sources to maximize relevance while minimizing bias.

Result: Experimental evaluation shows an 81.82% reduction in bias compared to a baseline naive retrieval strategy.

Conclusion: The approach demonstrates promising bias reduction in content retrieval for Agentic AI; further work should assess generalizability, measurement robustness, and real-world scalability.
...
### [180] [CAC-CoT: Connector-Aware Compact Chain-of-Thought for Efficient Reasoning Data Synthesis Across Dual-System Cognitive Tasks](https://arxiv.org/abs/2508.18743)
*Sunguk Choi,Yonghoon Kwon,Heondeuk Lee*

Main category: cs.AI

TL;DR: CAC-CoT is a concise connector-aware chain-of-thought prompting method that confines reasoning to a fixed set of connector phrases, yielding compact traces and strong performance with efficient computation.


<details>
  <summary>Details</summary>
Motivation: Very long chain-of-thought traces slow or degrade fast, intuitive 'System-1' tasks; need concise, well-structured explanations and efficient reasoning.

Method: Introduce Connector-Aware Compact CoT (CAC-CoT) that restricts reasoning to a small fixed set of connector phrases; uses synthetic training data generated via Gemini-2.0-Flash to train the model; aims for short, well-structured traces (~300 tokens).

Result: Achieves ~85% GSM8K accuracy and ~40% GPQA (System-2) while ~90% on S1-Bench (System-1); traces are ~300 tokens, about one-third of baseline; improved efficiency without accuracy loss.

Conclusion: CAC-CoT provides a simple, effective approach to balancing accuracy and tracing efficiency, enabling high-quality, compact reasoning for LLMs.
...
### [181] [Reflection-Enhanced Meta-Optimization Integrating TextGrad-style Prompt Optimization with Memory-Driven Self-Evolution](https://arxiv.org/abs/2508.18749)
*Chunlong Wu,Zhibo Qu*

Main category: cs.AI

TL;DR: REMO is a memory-augmented meta-optimization framework for prompting that combines a reflection-based memory (RAG “mistake notebook”) with an LLM-driven meta-controller to synthesize epoch-level insights, enabling cross-run knowledge reuse and continual prompt improvement; shows stronger generalization than TextGrad on GSM8K but with higher compute.


<details>
  <summary>Details</summary>
Motivation: To overcome the stateless, per-run nature of existing prompt optimization methods and their tendency to overfit, by keeping a history of mistakes and insights and using it to guide iterative prompting strategies.

Method: Integrates two components: (1) a memory-augmented Reflection Retrieval-Augmented Generation (RAG) module organized as a mistake notebook, and (2) a Self-Adaptive Optimizer driven by an LLM-based meta-controller that distills epoch-level reflective insights to adjust prompting strategies. Implemented with Qwen3-32B in standard inference mode (no chain-of-thought) and evaluated on GSM8K; includes qualitative/quantitative analyses and ablations.

Result: Outperforms a TextGrad baseline in stability and generalization on GSM8K; gains come with increased computational overhead; comprehensive ablation studies highlight the contributions of each component.

Conclusion: The REMO framework enables systematic cross-run learning and continual improvement in prompting, at the cost of additional computation; it offers a path toward robust, memory-informed prompt optimization and broader applicability across tasks and models.
...
### [182] [Stabilizing Open-Set Test-Time Adaptation via Primary-Auxiliary Filtering and Knowledge-Integrated Prediction](https://arxiv.org/abs/2508.18751)
*Byung-Joon Lee,Jin-Seop Lee,Jee-Hyong Lee*

Main category: cs.AI

TL;DR: Proposes Primary-Auxiliary Filtering (PAF) and Knowledge-Integrated Prediction (KIP) to improve open-set test-time adaptation (OSTTA) by robustly filtering test data and calibrating predictions from adapting, EMA, and source models, yielding better closed-set accuracy and open-set discrimination across datasets; code available.


<details>
  <summary>Details</summary>
Motivation: Open-set test-time adaptation is needed because test data often include unseen classes and domain shifts; relying on the source model for filtering can degrade performance under distribution shift; robust filtering and prediction calibration are essential for maintaining accuracy and discrimination.

Method: Primary-Auxiliary Filtering (PAF): an auxiliary filter validates data filtered by the primary filter to reduce error accumulation. Knowledge-Integrated Prediction (KIP): calibrates the outputs of the adapting model, EMA model, and source model to integrate their complementary knowledge for OSTTA.

Result: Demonstrates improved closed-set accuracy and open-set discrimination over existing methods across diverse datasets; provides code at the referenced GitHub repository.

Conclusion: PAF-KIP-OSTTA offers a robust framework for OSTTA by combining validated data filtering with calibrated, multi-model predictions, enhancing performance in both closed-set and open-set scenarios.
...
### [183] [Answering the Unanswerable Is to Err Knowingly: Analyzing and Mitigating Abstention Failures in Large Reasoning Models](https://arxiv.org/abs/2508.18760)
*Yi Liu,Xiangyu Liu,Zequn Sun,Wei Hu*

Main category: cs.AI

TL;DR: LRMs struggle to abstain on unanswerable questions due to misalignment between internal recognition of flaws and external abstention behavior; a two-stage approach using cognitive monitoring and inference-time intervention improves abstention without harming reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Ensure trustworthy AI by enabling LRMs to abstain appropriately on questions they cannot answer, addressing a gap where internal cognition does not translate into safe external responses.

Method: 1) Analyze response patterns of LRMs on unanswerable questions; 2) show LRMs can recognize flaws but fail to abstain; 3) propose a lightweight two-stage framework combining cognitive monitoring with inference-time intervention; 4) validate through experiments measuring abstention rate and reasoning performance.

Result: The proposed method significantly increases abstention rate while maintaining overall reasoning performance, indicating improved alignment between internal cognition and external response without sacrificing task competence.

Conclusion: Misalignment between internal cognition and external response in LRMs can be mitigated with a simple, two-stage framework that monitors cognitive state and intervenes during inference, enhancing trustworthiness with minimal performance trade-offs.
...
### [184] [Dynamic Collaboration of Multi-Language Models based on Minimal Complete Semantic Units](https://arxiv.org/abs/2508.18763)
*Chao Hao,Zezheng Wang,Yanhua Huang,Ruiwen Xu,Wenzhe Niu,Xin Liu,Zitong Yu*

Main category: cs.AI

TL;DR: A dynamic, distribution-distance-based token selection strategy (DDS) for multi-model autoregressive reasoning, coupled with minimal complete semantic units (MCSU) to mitigate vocabulary misalignment; achieves better reasoning across benchmarks.


<details>
  <summary>Details</summary>
Motivation: To enhance reasoning in language models by enabling collaborative token-level reasoning across multiple models while addressing inefficiencies and misalignment issues.

Method: Use next-token distribution distances to select the best token across models in a dynamic way; introduce MCSU to align semantic units across models.

Result: Outperforms single-model baselines and naive multi-model voting on several benchmarks; code released at the project repository.

Conclusion: DDS + MCSU enables more effective multi-model collaboration for autoregressive reasoning; addressing vocabulary alignment is crucial for cross-model coherence.
...
### [185] [AniME: Adaptive Multi-Agent Planning for Long Animation Generation](https://arxiv.org/abs/2508.18781)
...
TL;DR: AniME: a director-centered multi-agent system that automates end-to-end long-form anime production, coordinating specialized agents via a global memory and a Model Context Protocol to ensure consistent characters and synchronized audio-visual elements.


<details>
  <summary>Details</summary>
Motivation: Automating long-form anime creation is challenging due to complex coordination across sub-tasks (story to video), need for character consistency and synchronized AV elements, and the demand for scalable AI-driven workflows.

Method: Introduce a director agent with global memory that oversees downstream specialized agents. Use a Model Context Protocol (MCP) to incorporate downstream model instructions and adapt control conditions for sub-tasks, enabling adaptive task execution and coordination across the pipeline.

Result: Proposes a scalable framework that can produce cinematic animation with consistent characters and synchronized audio-visual elements across the full workflow from story to final video.

Conclusion: A director-oriented multi-agent architecture with global memory and MCP-based coordination can automate end-to-end AI-driven anime production, offering scalability; future work may include empirical evaluation, quality metrics, and robustness analyses.
...
### [186] [CausalMACE: Causality Empowered Multi-Agents in Minecraft Cooperative Tasks](https://arxiv.org/abs/2508.18797)
*Qi Chai,Zhang Zheng,Junlong Ren,Deheng Ye,Zichuan Lin,Hao Wang*

Main category: cs.AI

TL;DR: A two-module, causality-aware multi-agent planning framework (CausalMACE) for Minecraft tasks that improves efficiency and fault tolerance, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Single-agent LLMs struggle with complex, long-horizon tasks and limited fault tolerance; multi-agent collaboration is underexplored; there is a need to manage inter-subtask dependencies via causality.

Method: CausalMACE introduces two modules: an overarching task graph for global task planning and a causality-based dependency management module that uses predefined causal rules to intervene and manage subtasks; the framework integrates causality into planning to coordinate multiple agents.

Result: Demonstrates state-of-the-art performance on multi-agent cooperative tasks in Minecraft.

Conclusion: CausalMACE shows the value of causality-informed planning for multi-agent systems, improving efficiency and fault tolerance, with potential to generalize to other open-world environments and complex tasks.
...
### [187] [STARec: An Efficient Agent Framework for Recommender Systems via Autonomous Deliberate Reasoning](https://arxiv.org/abs/2508.18812)
*Chenghao Wu,Ruiyang Ren,Junjie Zhang,Ruirui Wang,Zhongrui Ma,Qi Ye,Wayne Xin Zhao*

Main category: cs.AI

TL;DR: STARec introduces slow-thinking augmented agents for recommender systems with parallel fast and slow cognition and anchored reinforcement training, achieving strong, data-efficient improvements on MovieLens 1M and Amazon CDs.


<details>
  <summary>Details</summary>
Motivation: Current recommender systems suffer from static user modeling and reactive decision-making. LLM-based agents over-rely on pattern matching, leading to shallow correlations, limited causal inference, and brittleness in sparse data.

Method: Per-user agents with parallel fast responses and slow reasoning that generates chain-of-thought rationales. Anchored reinforcement training: a two-stage paradigm combining structured knowledge distillation from advanced reasoning models with preference-aligned reward shaping, plus simulated feedback loops to cultivate foundational capabilities such as preference summarization and rationale generation.

Result: Experiments on MovieLens 1M and Amazon CDs show substantial performance gains over state-of-the-art baselines, with only 0.4% of full training data.

Conclusion: The STARec framework demonstrates that slow-thinking, deliberative reasoning can enhance recommender systems, enabling dynamic policy adaptation and improved robustness in data-scarce settings.
...
### [188] [Judicial Requirements for Generative AI in Legal Reasoning](https://arxiv.org/abs/2508.18880)
*Eljas Linna,Tuula Linna*

Main category: cs.AI

TL;DR: Defines core AI capabilities for reliable legal reasoning, maps AI-enhancement techniques to IRAC-driven tasks, and concludes AI should serve as a high-volume assistant for simple cases and a sophisticated sparring partner for experts—not a full substitute due to gaps in discretionary, transparent reasoning.


<details>
  <summary>Details</summary>
Motivation: Address the gap in understanding how LLMs perform in high-stakes legal settings and how AI can be integrated to support, rather than replace, judicial decision-making.

Method: Frame legal reasoning via IRAC (Issue-Rule-Application-Conclusion), identify challenging phases (selecting applicable Rule, applying it to facts), decompose into core requirements (jurisdiction-spanning rule selection, source-based argumentation, distinguishing ratio decidendi from obiter, resolving ambiguities, handling conflicting provisions, burden of proof). Then map AI-enhancement techniques (Retrieval-Augmented Generation, multi-agent systems, neuro-symbolic AI) to these requirements and assess their ability to bridge LLM probabilism with rigorous legal interpretation.

Result: Techniques can address specific challenges but cannot fully resolve the need for discretion and transparent, justifiable reasoning. AI can help as a high-volume assistant for repetitive tasks and as a sparring partner for complex matters, but substantial gaps remain in achieving reliable, explainable legal decision-making.

Conclusion: The optimal current role for AI in law is dual: assist with routine cases and provide advanced, human-in-the-loop support for complex adjudication, until breakthroughs yield fully transparent, accountable AI legal reasoning.
...
### [189] [Interactive Evaluation of Large Language Models for Multi-Requirement Software Engineering Tasks](https://arxiv.org/abs/2508.18905)
*Dimitrios Rontogiannis,Maxime Peyrard,Nicolas Baldwin,Martin Josifoski,Robert West,Dimitrios Gunopulos*

Main category: cs.AI

TL;DR: An interactive, feedback-driven evaluation framework for LLMs on multi-requirement programming tasks, using a guided dialogue between an interviewer and interviewee, with tasks as requirement dependency graphs; extends DevAI by adding ground-truth solutions and expert-annotated evaluation of hints, yielding fine-grained diagnostic insights beyond static benchmarks.


<details>
  <summary>Details</summary>
Motivation: Static single-turn benchmarks fail to capture the nuanced, collaborative capabilities needed for software engineering with LLMs. There is a need for dynamic, diagnostic evaluation that can reveal strengths and systematic weaknesses in how LLMs produce, correct, and collaborate on code.

Method: Model a task as a requirement dependency graph. Use an interviewer LLM, aware of the ground-truth solution, to provide minimal, targeted hints to an interviewee model. The interview progresses through structured dialogue with the goal of satisfying constraints and fixing errors. The work builds on DevAI by adding ground-truth solutions and evaluating the relevance/utility of interviewer hints via expert annotation.

Result: Dynamic evaluation yields fine-grained diagnostic insights into model behavior, revealing strengths and systematic weaknesses that static benchmarks miss. Expert annotations validate the relevance and utility of interviewer hints, underscoring the value of interactive evaluation for developing collaborative code-generating agents.

Conclusion: Interactive, feedback-driven evaluation is a valuable complement to static benchmarks for assessing LLMs in software engineering tasks. Ground-truth solutions and expert-annotated hints enable detailed diagnostics and can guide the development of more effective collaborative code-generation agents.
...
### [190] [FormaRL: Enhancing Autoformalization with no Labeled Data](https://arxiv.org/abs/2508.18914)
*Yanxing Huang,Xinling Jin,Sijie Liang,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: FormaRL is a data-efficient RL framework for autoformalization that leverages Lean syntax checks and LLM consistency rewards, updated via GRPO, achieving large accuracy gains with ~859 unlabeled examples and providing the uproof dataset.


<details>
  <summary>Details</summary>
Motivation: Autoformalization suffers from data scarcity and inefficient methods. The paper aims to enable data-efficient autoformalization by combining formal compiler checks with LLM-based feedback and a reinforcement learning update, and by providing a math problem dataset (uproof) to foster research.

Method: FormaRL integrates Lean compiler syntax checks as a hard/structural reward component and LLM-based consistency checks as a probabilistic reward, combining them to guide a formalizer updated with the GRPO algorithm. A new labeled/unlabeled data regime is used, requiring only a small amount of unlabeled data for training.

Result: With about 859 unlabeled examples, FormaRL improves pass@1 autoformalization accuracy of Qwen2.5-Coder-7B-Instruct by 4–6x (4.04%→26.15% on ProofNet; 2.4%→9.6% on uproof). On uproof, it also boosts out-of-distribution performance relative to open-source autoformalizers: pass@1 (6.2%→9.6%) and pass@16 (24.4%→33.6%).

Conclusion: FormaRL demonstrates that data-efficient autoformalization is achievable by combining compiler/syntax and LLM-consistency rewards with GRPO updates, supported by the newly curated uproof dataset; training code is open-sourced.
...
### [191] [Who Is Lagging Behind: Profiling Student Behaviors with Graph-Level Encoding in Curriculum-Based Online Learning Systems](https://arxiv.org/abs/2508.18925)
*Qian Xiao,Conn Breathnach,Ioana Ghergulescu,Conor O'Sullivan,Keith Johnston,Vincent Wade*

Main category: cs.AI

TL;DR: CTGraph is a self-supervised graph-level representation learning approach that profiles learner behaviors and performance in Intelligent Tutoring Systems, enabling holistic tracking across content coverage, learning intensity, and concept proficiency; it identifies struggling students and supports group comparisons aligned to curriculum, facilitating targeted interventions.


<details>
  <summary>Details</summary>
Motivation: ITS adoption can widen performance gaps; effective student profiling across multiple behavioral and performance dimensions, and alignment to curriculum, is essential to identify who struggles and where.

Method: CTGraph uses graph-level representation learning in a self-supervised setting to model learner behaviors and performance, capturing curriculum-aligned trajectories and multi-aspect measurements (content coverage, learning intensity, concept proficiency) within a learned graph representation.

Result: Experiments show CTGraph provides a holistic view of learning journeys, accounts for multiple aspects and path variations, identifies struggling students, and enables comparative analyses across diverse groups, enabling targeted interventions.

Conclusion: CTGraph offers rich, curriculum-aligned insights for educators and supports targeted interventions to reduce disparities in ITS-based learning.
...
### [192] [VISION: Robust and Interpretable Code Vulnerability Detection Leveraging Counterfactual Augmentation](https://arxiv.org/abs/2508.18933)
*David Egea,Barproda Halder,Sanghamitra Dutta*

Main category: cs.AI

TL;DR: A counterfactual-based training framework (VISION) for robust and interpretable vulnerability detection using GNNs, leveraging LLM-generated counterfactuals to mitigate spurious correlations, achieving large performance gains and releasing CWE-20-CFA benchmark.


<details>
  <summary>Details</summary>
Motivation: GNN-based vulnerability detectors suffer from data imbalance and label noise that induce spurious correlations and poor generalization; need robust, interpretable methods.

Method: Generate counterfactuals via prompting an LLM; train GNN on paired opposite-label examples; apply graph-based interpretability to highlight key statements and ignore spurious ones; release benchmark and interactive visualization.

Result: Accuracy on CWE-20 improved from 51.8% to 97.8%; pairwise contrast accuracy from 4.5% to 95.8%; worst-group accuracy from 0.7% to 85.5%; introduction of metrics (intra-class attribution variance, inter-class attribution distance, node score dependency); release CWE-20-CFA with 27,556 functions; interactive human-in-the-loop visualization.

Conclusion: VISION reduces spurious learning and yields robust, generalizable vulnerability detectors; promotes transparent AI-based cybersecurity through interpretable models and human-in-the-loop analysis.
...
### [193] [Novel Approaches to Artificial Intelligence Development Based on the Nearest Neighbor Method](https://arxiv.org/abs/2508.18953)
*I. I. Priezzhev,D. A. Danko,A. V. Shubin*

Main category: cs.AI

TL;DR: A KNN-based approach with Kohonen-tree indices replaces deep-model training to curb hallucinations and reduce computation, yielding large speedups and interpretability with only modest accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To overcome fundamental limitations of modern neural networks—hallucinations, high training/inference cost, costly fine-tuning, and catastrophic forgetting—that hinder deployment in critical domains, by adopting a transparent, explainable alternative.

Method: Replace end-to-end learning with a k-nearest neighbors framework augmented by hierarchical clustering. Build tree-like data structures via Kohonen self-organizing maps to accelerate nearest-neighbor search, avoid retraining the entire network, and maintain interpretability and easy expansion/fine-tuning.

Result: Empirical tests on handwritten digit recognition and simple subtitle translation tasks show hundreds-fold reductions in NN search time with only a small drop in accuracy, along with improved transparency and interpretability.

Conclusion: The proposed NN approach provides a transparent, reliable alternative suitable for tasks demanding high reliability and explainability, with strong potential for widespread use where retraining large models is undesirable.
...
### [194] [Enabling MoE on the Edge via Importance-Driven Expert Scheduling](https://arxiv.org/abs/2508.18983)
*Guoying Zhu,Meng Li,Haipeng Dai,Xuechen Liu,Weijun Wang,Keran Li,Jun xiao,Ligeng Chen,Wei Wang*

Main category: cs.AI

TL;DR: An importance-guided, cache-aware offloading strategy for Mixture of Experts on edge devices; substitutes less-important activated experts with cached, functionally similar ones and uses a reuse-centric scheduler to reduce latency and data transfers while preserving accuracy.


<details>
  <summary>Details</summary>
Motivation: Edge deployment of Mixture-of-Experts (MoE) is memory-bound: activating only a subset of experts per query helps, but limited device memory and PCIe transfer cost limit practical deployment. A strategy that reduces memory footprint and transfer overhead without sacrificing accuracy is needed.

Method: Leverage expert importance to guide offloading decisions by replacing low-importance activated experts with functionally similar ones already cached in GPU memory. This slows no accuracy degradation. Introduce a scheduling policy that maximizes the reuse ratio of GPU-cached experts to boost efficiency.

Result: Reduced memory usage and data transfer; largely eliminates PCIe overhead. Achieves 48% lower decoding latency with an expert cache hit rate above 60%, while maintaining nearly lossless accuracy.

Conclusion: Demonstrates that importance-guided caching and reuse-aware scheduling enable efficient MoE deployment on edge hardware, offering a practical path to scalable, accurate edge-scale LLMs.
...
### [195] [AI Models Exceed Individual Human Accuracy in Predicting Everyday Social Norms](https://arxiv.org/abs/2508.19004)
*Pontus Strimling,Simon Karlsson,Irina Vartanova,Kimmo Eriksson*

Main category: cs.AI

TL;DR: LLMs can predict human social appropriateness judgments from linguistic data with superhuman accuracy, but exhibit systematic, correlated errors; this suggests language alone can encode rich cultural norms and challenges embodied-experience theories, while revealing boundaries of pattern-based social understanding.


<details>
  <summary>Details</summary>
Motivation: Test whether large language models can acquire social norm understanding through statistical learning alone (without embodied experience) and assess the limits and boundary conditions of such pattern-based social cognition across architectures.

Method: Two studies evaluating multiple AI systems on 555 everyday scenarios, measuring how closely model predictions align with average human judgments. Study 1 found GPT-4.5 exceeded every human participant; Study 2 found Gemini 2.5 Pro >98% of humans, GPT-5 >97%, Claude Sonnet 4 >96%, with all models showing systematic, correlated errors.

Result: Models demonstrated very high predictive accuracy for human social judgments, often surpassing most humans, but displayed consistent, patterned mistakes across scenarios.

Conclusion: Sophisticated social cognition can emerge from statistical learning over language data, challenging strong embodied-experience theories; language serves as a rich channel for cultural knowledge, though pattern-based limitations reveal boundaries of this approach and potential AI-human alignment concerns.
...
### [196] [Building Self-Evolving Agents via Experience-Driven Lifelong Learning: A Framework and Benchmark](https://arxiv.org/abs/2508.19005)
...
TL;DR: Experience-driven Lifelong Learning (ELL) framework for self-evolving agents enabling continuous growth via real-world interaction, with four core principles (Experience Exploration, Long-term Memory, Skill Learning, Knowledge Internalization) and a benchmark (StuLife) simulating a student’s college journey to evaluate lifelong learning, memory, and skill transfer; includes emphasis on context engineering for AGI.


<details>
  <summary>Details</summary>
Motivation: As AI advances toward general intelligence, there is a shift from systems optimized for static tasks to open-ended agents that learn continuously in dynamic environments; a need for persistent memory, reusable skills, and internalization of experiences to support long-term, self-motivated growth.

Method: Proposes the Experience-driven Lifelong Learning (ELL) framework with four core principles: 1) Experience Exploration, 2) Long-term Memory, 3) Skill Learning, 4) Knowledge Internalization. Introduces StuLife, a benchmark simulating a holistic college journey across three phases and ten sub-scenarios, designed around paradigm shifts (Passive→Proactive, Context→Memory, Imitation→Learning). Outlines evaluation targets including memory retention, skill transfer, and self-motivated behavior; additionally explores context engineering for advancing AGI.

Result: No empirical results are reported in the abstract; the paper presents a conceptual framework (ELL) and a benchmark (StuLife) with an intended evaluation protocol and guidance for future experiments; preliminary insights suggest the potential importance of context engineering for AGI progress.

Conclusion: ELL provides a foundation for building lifelong, self-evolving agents and a rich benchmark (StuLife) to assess memory, skill transfer, and self-motivated learning; the work also highlights context engineering as a key factor in advancing AGI.
...
### [197] [Sense of Self and Time in Borderline Personality. A Comparative Robustness Study with Generative AI](https://arxiv.org/abs/2508.19008)
*Marcin Moskalewicz,Anna Sterna,Marek Pokropski,Paula Flores*

Main category: cs.AI

TL;DR: The paper evaluates whether large language models can augment phenomenological qualitative analysis of first-person BPD experiences by prompting three models to imitate human investigators; results show varying alignment with human analysis, with Gemini performing best and GPT-4o performing poorly, but all models surfaced themes human analysts missed. The study suggests AI augmentation can reduce interpretive bias but requires careful evaluation and prompts.


<details>
  <summary>Details</summary>
Motivation: To assess the feasibility and validity of using large language models to support or augment human-led phenomenological thematic analysis of first-person experiences in Borderline Personality Disorder, exploring temporality and selfhood through AI-assisted interpretation.

Method: Based on a prior human-led thematic analysis of 24 inpatient life-story interviews, three LLMs (GPT-4o, Gemini 2.5 Pro, Claude Opus 4) were prompted to mimic the original investigators' interpretative style. Evaluations used blinded and non-blinded expert judges in phenomenology and clinical psychology, measuring semantic congruence, Jaccard similarity, and multidimensional validity (credibility, coherence, substantiveness, groundness in data). Outputs were compared to human analysis to assess overlap and novel theme recovery.

Result: Overlap with the human analysis varied: GPT-4o 0%, Claude Opus 4 42%, Gemini 58%; Jaccard coefficients were low (0.21–0.28). Gemini’s output most closely resembled the human analysis, with validity scores higher than GPT and Claude (p < 0.0001) and judged as human by blinded experts. All validity dimensions correlated strongly with the length of the AI output (text quantity and words per theme, r > 0.78). However, models also recovered themes omitted by humans, indicating potential for AI to mitigate interpretive bias but also to diverge.

Conclusion: AI-augmented thematic analysis shows promise for supporting phenomenological interpretation and reducing human bias, with Gemini performing best among the tested models. However, substantial variability across models and potential for both concordance and divergence from human analysis necessitate careful prompting, evaluation, and integration within qualitative workflows.
...
### [198] [MAB Optimizer for Estimating Math Question Difficulty via Inverse CV without NLP](https://arxiv.org/abs/2508.19014)
*Surajit Das,Gourav Roy,Aleksei Eliseev,Ram Kumar Rajendran*

Main category: cs.AI

TL;DR: A domain-agnostic RL-based MAB framework (APME) that estimates item difficulty from solver data without linguistic features; achieves high accuracy and outperforms baselines, enabling adaptive assessment in IATS, especially for symbolic domains.


<details>
  <summary>Details</summary>
Motivation: Human-labeled difficulty is subjective and NLP methods struggle in symbolic domains; need objective, scalable, domain-agnostic difficulty tagging using solver interaction data.

Method: Passive Measures among Educands (APME): a reinforcement learning-based Multi-Armed Bandit that uses solver performance data (marks and time) to infer difficulty without linguistic features; employs the inverse coefficient of variation as a risk-adjusted metric; self-supervised and domain-agnostic.

Result: On three heterogeneous datasets, average R2 = 0.9213 and RMSE = 0.0584; robust across educational levels and assessment formats; outperforms regression-based, NLP-driven, and IRT baselines, especially in symbolic domains; finds item heterogeneity and outcome variance are critical.

Conclusion: The approach yields robust, scalable, and domain-agnostic difficulty tagging for IATS; can extend beyond algebra in any context where solver interaction data is available; aligns with pedagogy by targeting tasks at the balance of challenge and attainability.
...
### [199] [Investigating Advanced Reasoning of Large Language Models via Black-Box Interaction](https://arxiv.org/abs/2508.19035)
*Congchi Yin,Tianyi Wu,Yankai Shu,Alex Gu,Yunhan Wang,Jun Shao,Xun Jiang,Piji Li*

Main category: cs.AI

TL;DR: New black-box interaction paradigm to evaluate LLMs in unknown environments via hidden-input/output functions; Oracle benchmark with 6 black-box types and 96 instances; 19 LLMs tested; o3 leads on most tasks but struggles on harder ones; highlights need for high-level planning in exploration.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks assess isolated deductive/inductive/abductive reasoning and ignore integrated, interactive discovery in real-world settings; a hidden-function, interactive evaluation better captures LLMs' real-world reasoning and planning limitations.

Method: Define a black-box as a hidden function. LLMs interact over exploration turns, observing IO pairs, and reason to infer the underlying function. Build Oracle benchmark with 6 black-box task types and 96 instances; evaluate 19 modern LLMs and analyze performance and strategies.

Result: o3 ranks first in 5 out of 6 tasks, achieving >70% accuracy on most easy black-boxes; average performance drops below 40% on hard black-boxes; substantial evidence of universal difficulty in high-level planning for efficient/adaptive exploration and hypothesis refinement.

Conclusion: Integrated interactive reasoning remains challenging for current LLMs; future work should improve high-level planning and adaptive exploration to better infer hidden functions and perform real-world discovery.
...
### [200] [A Concurrent Modular Agent: Framework for Autonomous LLM Agents](https://arxiv.org/abs/2508.19042)
*Norihiro Maruyama,Takahide Yoshida,Hiroki Sato,Atsushi Masumori,Johnsmith,Takashi Ikegami*

Main category: cs.AI

TL;DR: CMA enables asynchronous, modular LLM-based components to coordinate via a shared global state, producing coherent behavior and fault tolerance; supports emergent properties aligned with Minsky's Society of Mind; code released.


<details>
  <summary>Details</summary>
Motivation: To overcome brittle, single-agent architectures by enabling flexible, context-dependent behavior through language-mediated interactions among autonomous processes, allowing emergent intention and self-organizing cognition.

Method: A framework of concurrently executed, LLM-driven modules that communicate asynchronously and share a single global state; modules offload reasoning to LLMs, use inter-module messaging, and operate within a fault-tolerant loop; demonstrated via two practical use-case studies; source code provided.

Result: The approach is viable; emergent properties are observed, including cognition-like dynamics; supports Minsky-Society of Mind intuition and points to new AI research directions; code is available at the referenced repository.

Conclusion: CMA offers a practical realization of Minsky’s Society of Mind, enabling flexible, adaptive behavior through modular, asynchronous LLM components; opens avenues for future research into emergent cognition, fault tolerance, and scalable multi-module AI systems.
...
### [201] [Can Structured Templates Facilitate LLMs in Tackling Harder Tasks? : An Exploration of Scaling Laws by Difficulty](https://arxiv.org/abs/2508.19069)
*Zhichao Yang,Zhaoxin Fan,Gen Li,Yuanze Hu,Xinyu Wang,Ye Qiu,Xin Wang,Yifan Sun,Wenjun Wu*

Main category: cs.AI

TL;DR: A Scaling Law by Difficulty reveals a U-shaped relationship between model performance and training data complexity; the proposed Structured Solution Template (SST) framework uses templates and a curriculum of varied difficulty to explicitly teach procedural reasoning, leading to improved accuracy and efficiency on math problems, especially harder ones.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle to capture deep procedural reasoning, particularly in mathematics, even after post-training methods. A data-complexity scaling law is identified to guide training; the goal is to explicitly teach step-by-step problem solving through structured templates and curriculum-based learning.

Method: 1) Discover Scaling Law by Difficulty showing a U-shaped performance curve w.r.t data complexity. 2) Propose SST: (a) fine-tuning with structured solution-template chains and dynamically weighted loss to emphasize procedural logic, (b) prompt-time injection of solution templates as cognitive scaffolds, (c) integrated curriculum fine-tuning to train self-plan – execute – self-correct.

Result: SST significantly improves both accuracy and efficiency on math benchmarks, with pronounced gains on harder problems (GSM8K, AIME24, Dynamic En).

Conclusion: Combining difficulty-aware data strategy with structured templates and curriculum learning can robustly enhance procedural reasoning in LLMs and generalize to complex tasks, suggesting a practical path forward for scaling LLMs’ structured reasoning capabilities.
...
### [202] [Trustworthy Agents for Electronic Health Records through Confidence Estimation](https://arxiv.org/abs/2508.19096)
*Yongwoo Song,Minbyul Jeong,Mujeen Sung*

Main category: cs.AI

TL;DR: Defines HCAcc@k% as an accuracy–reliability metric for clinical LLMs, introduces TrustEHRAgent with stepwise confidence estimation, and shows significant reliability gains over baselines on MIMIC-III/eICU under strict thresholds, underscoring limitations of traditional accuracy metrics.


<details>
  <summary>Details</summary>
Motivation: LLMs risk hallucinations when extracting information from EHRs; traditional accuracy metrics fail to capture the reliability of clinical AI in decision support. There is a need for evaluation and agents that can reliably indicate uncertainty.

Method: Introduce HCAcc@k% to quantify accuracy vs. confidence. Develop TrustEHRAgent, a confidence-aware agent with stepwise confidence estimation for clinical question answering. Evaluate on MIMIC-III and eICU, comparing to baselines under strict reliability constraints.

Result: TrustEHRAgent outperforms baselines at HCAcc@70% by 44.23 percentage points on one dataset and 25.34 percentage points on another; baseline methods fail at these thresholds. Demonstrates that traditional accuracy metrics are insufficient for healthcare AI evaluation.

Conclusion: This work highlights the limitations of traditional accuracy metrics for healthcare AI and contributes trustworthy clinical agents that either deliver accurate information or transparently express uncertainty when confidence is low.
...
### [203] [Reasoning LLMs in the Medical Domain: A Literature Survey](https://arxiv.org/abs/2508.19097)
*Armin Berger,Sarthak Khanna,David Berghaus,Rafet Sifa*

Main category: cs.AI

TL;DR: Survey on medical LLMs with advanced reasoning, covering prompting (Chain-of-Thought), reinforcement learning (e.g., DeepSeek-R1), multi-agent systems, evaluation methodologies, safety, bias mitigation, and multimodal data integration, with a roadmap for reliable clinical AI.


<details>
  <summary>Details</summary>
Motivation: To enable transparent, safe, and trustworthy clinical decision-support LLMs, addressing validation, interpretability, bias, safety, and data integration in medical contexts.

Method: Comprehensive literature survey analyzing enabling foundations, prompting techniques (e.g., Chain-of-Thought), reinforcement learning approaches (e.g., DeepSeek-R1), purpose-built medical frameworks, emerging paradigms like multi-agent collaboration, and evaluation methods; synthesis of trends and roadmap development.

Result: Provides a synthesized assessment of the current state, enabling technologies, evaluation practices, challenges, and guidance; identifies gaps and best practices for designing reliable medical LLMs.

Conclusion: With robust prompting strategies, appropriate RL paradigms, multi-agent collaboration, and rigorous validation and safety considerations, medical LLMs can serve as effective partners in clinical practice and research; ongoing work is needed to address bias, safety, and multimodal data integration.
...
### [204] [Hybrid Deep Searcher: Integrating Parallel and Sequential Search Reasoning](https://arxiv.org/abs/2508.19113)
*Dayoon Ko,Jihyuk Kim,Haeju Park,Sohyeon Kim,Dahyun Lee,Yongrae Jo,Gunhee Kim,Moontae Lee,Kyungjae Lee*

Main category: cs.AI

TL;DR: A synthetic data approach (HDS-QA) trains large reasoning models to exploit parallelizable subqueries alongside sequential steps, improving latency without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Address high inference latency and coherence degradation in purely sequential, multi-step retrieval for large reasoning models by enabling hybrids of parallel and sequential querying.

Method: Generate the HDS-QA dataset from Natural Questions consisting of hybrid-hop questions with independent, parallelizable subqueries and sequentially dependent subqueries; include synthetic reasoning-querying-retrieval paths; fine-tune an LRM (HybridDeepSearcher) on this data to recognize and exploit parallelizable queries.

Result: Achieves +15.9 F1 on FanOutQA and +11.5 F1 on a BrowseComp subset; performs with comparable accuracy using fewer search turns, reducing latency; demonstrates scalability as more turns are allowed.

Conclusion: Shows that explicit training for hybrid parallel/sequential querying yields efficient, scalable, and effective LRMs for complex reasoning with external retrieval.
...
### [205] [Algorithmic Collective Action with Multiple Collectives](https://arxiv.org/abs/2508.19149)
*Claudio Battiloro,Pietro Greiner,Bret Nestor,Oumaima Amezgar,Francesca Dominici*

Main category: cs.AI

TL;DR: A theoretical framework for Algorithmic Collective Action (ACA) with multiple collectives influencing a shared classifier by planting signals to bias feature-class associations; analyzes how collectives' sizes and goal alignment interact.


<details>
  <summary>Details</summary>
Motivation: Addresses a gap in ACA research by extending beyond single-collective settings to multiple collectives acting on the same ML system, enabling study of coordination, competition, and governance in real-world decision-making.

Method: Develops a theoretical framework for ACA in classification with multiple collectives; formalizes how collectives can plant signals that bias the classifier toward certain target classes; derives quantitative relationships governing the influence of collectives' sizes and goal alignment.

Result: Offers quantitative insights into the roles and interplay of different collectives' sizes and alignment of goals; provides a cohesive framework that complements existing empirical findings and enables holistic analysis of multi-collective ACA.

Conclusion: The framework establishes a foundation for studying ACA with multiple collectives, guiding future research on coordination, policy implications, and design considerations for robust ML systems in the presence of coordinated data manipulation.
...
### [206] [Playstyle and Artificial Intelligence: An Initial Blueprint Through the Lens of Video Games](https://arxiv.org/abs/2508.19152)
*Chiu-Chou Lin*

Main category: cs.AI

TL;DR: Proposes 'playstyle' as a new lens for AI decision-making, presenting a two-tier framework (external environment interaction and internal deliberation), metrics (style capacity, style popularity, evolutionary dynamics), and methods to define, measure, and generate stylistic behavior; outlines applications and future relevance to AGI.


<details>
  <summary>Details</summary>
Motivation: Rational, quantifiable decision-making is insufficient to capture human-like variability. Beliefs, values, and preferences shape decisions; a formal notion of 'style' is needed to understand and engineer diverse AI agents and to guide progress toward AGI.

Method: Two-tier framework: an external interaction loop with the environment and an internal cognitive loop of deliberation to model style formation. Proposes measurable indicators (style capacity, style popularity, evolutionary dynamics). Introduces a general playstyle metric via discretized state spaces, extending to quantify strategic diversity and competitive balance. Explores training (reinforcement learning and imitation learning) to produce agents with specific stylistic tendencies, including human-like style learning and modeling. Discusses practical applications in game design and interactive entertainment.

Result: Conceptual framework and methodological blueprint rather than empirical results: a set of definitions, metrics, and approaches for measuring, generating, and applying playstyle in AI systems; outlines potential experiments and domains for application.

Conclusion: Playstyle is a promising core element for future AI/AGI development, offering a structured way to capture diverse decision-making styles; further work is needed to operationalize metrics, validate approaches, and extend to real-world AGI contexts.
...
### [207] [MATRIX: Multi-Agent simulaTion fRamework for safe Interactions and conteXtual clinical conversational evaluation](https://arxiv.org/abs/2508.19163)
...
TL;DR: MATRIX provides a safety-oriented framework for evaluating clinical dialogue agents by integrating a safety taxonomy, an LLM-based hazard detector (BehvJudge), and a simulated patient (PatBot) to enable scalable, regulator-aligned safety assessment across diverse domains.


<details>
  <summary>Details</summary>
Motivation: Safety-critical clinical dialogue systems require evaluation beyond task success and fluency; existing work lacks structured safety assessment, failure-mode analysis, and regulator-aligned auditing capabilities.

Method: Three-component framework: (1) safety-aligned taxonomy of clinical scenarios, expected behaviors, and failure modes; (2) BehvJudge, an LLM-based evaluator for detecting safety-relevant dialogue failures, validated against expert clinician annotations; (3) PatBot, a simulated patient agent producing diverse, scenario-conditioned responses, evaluated for realism and behavioral fidelity with human factors input and patient-preference study. Three experiments across 2,100 simulated dialogues, 14 hazard scenarios, 10 clinical domains; BehvJudge (Gemini 2.5-Pro) achieves expert-level hazard detection (F1 0.96, sensitivity 0.999) in blinded assessment of 240 dialogues; PatBot realism established; MATRIX enables benchmarking of five LLM agents.

Result: MATRIX enables systematic, scalable safety evaluation of clinical dialogue agents. It delivers high-precision hazard detection and realistic patient simulations, demonstrated across multiple experiments and domains; tools, prompts, scenarios, and datasets are released for regulator-aligned auditing and benchmarking of LLMs.

Conclusion: MATRIX is the first framework to unify structured safety engineering with scalable, validated conversational AI evaluation for clinical dialogues, supporting regulator-aligned safety auditing and ongoing benchmarking of LLMs across hazard scenarios and domains.
...
### [208] [The Ramon Llull's Thinking Machine for Automated Ideation](https://arxiv.org/abs/2508.19200)
...
TL;DR: A modern Llull-inspired system prompts LLMs with curated theme-domain-method combinations to generate diverse, relevant, and literature-grounded research ideas, serving as an interpretable aid for AI-assisted ideation.


<details>
  <summary>Details</summary>
Motivation: To reframe Ars combinatoria as a practical, interpretable framework for scientific ideation, bridging medieval combinatorial thinking with contemporary LLM-driven creativity and collaborative human–AI exploration.

Method: Define three compositional axes (Theme, Domain, Method); mine elements from experts and conference papers; prompt LLMs with curated combinations to generate ideas; assess diversity, relevance, and grounding in current literature.

Result: LLMs prompted with curated Theme-Domain-Method combinations produced ideas that are diverse, relevant, and anchored in current literature, providing a lightweight, interpretable tool for augmenting scientific creativity.

Conclusion: A modern thinking machine based on Llull’s combinatorics can augment scientific creativity and enable collaborative ideation between humans and AI, offering an interpretable, scalable approach to research ideation.
...
### [209] [The Subset Sum Matching Problem](https://arxiv.org/abs/2508.19218)
*Yufei Wu,Manuel R. Torres,Parisa Zehtabi,Alberto Pozanco Lancho,Michael Cashmore,Daniel Borrajo,Manuela Veloso*

Main category: cs.AI

TL;DR: New combinatorial optimization task SSMP with three algorithms (two suboptimal, one optimal), plus a varying- complexity benchmark and experimental evaluation.


<details>
  <summary>Details</summary>
Motivation: Subsumes practical finance reconciliation problems; need reliable methods and realistic benchmarks for subset-sum-like matching.

Method: Formulate SSMP; develop two heuristic/suboptimal algorithms and one exact/optimal algorithm; generate a benchmark suite with instances of varying difficulty; conduct experiments to compare approaches.

Result: An empirical evaluation is performed to assess the approaches; abstract does not report specific results or metrics.

Conclusion: The work introduces SSMP, the three algorithms, a benchmark, and an evaluation framework; no explicit conclusions are stated in the abstract.
...
### [210] [StepWiser: Stepwise Generative Judges for Wiser Reasoning](https://arxiv.org/abs/2508.19229)
*Wei Xiong,Wenting Zhao,Weizhe Yuan,Olga Golovneva,Tong Zhang,Jason Weston,Sainbayar Sukhbaatar*

Main category: cs.AI

TL;DR: StepWiser is a generative judge that reasons about a model's step-by-step reasoning (meta-reasons), outputting thinking tokens before verdict; trained with RL on rollout outcomes to improve intermediate-step judgment, policy training, and search.


<details>
  <summary>Details</summary>
Motivation: Current process reward models are mostly classifiers that lack explanations and generalization; there's a need for reasoned, interpretable feedback to guide multi-step reasoning models.

Method: Replace classification with a generative reasoning task: a meta-reasoning judge outputs thinking tokens (meta-reasons) prior to giving a final verdict; trained via reinforcement learning using relative rollout outcomes.

Result: Better judgment accuracy on intermediate steps than prior methods; can be used to improve the policy during training; enhances inference-time search.

Conclusion: Framing stepwise reward modeling as a reasoning task yields interpretable meta-reasoning feedback and practical benefits for training and inference in multi-step reasoning systems.
...
### [211] [Model Context Protocols in Adaptive Transport Systems: A Survey](https://arxiv.org/abs/2508.19239)
...
TL;DR: Survey proposing Model Context Protocol (MCP) as unifying paradigm for adaptive transport systems; introduces taxonomy and roadmap.


<details>
  <summary>Details</summary>
Motivation: Fragmentation from isolated protocols and context sources hinders interoperability; AI-driven transport requires integrated, context-aware decision making.

Method: Systematic literature analysis of existing adaptive transport efforts; synthesis into MCP-centric framework; development of a five-category taxonomy; derivation of insights and roadmap.

Result: Evidence of convergence toward MCP-like architectures; a five-category MCP taxonomy; three key insights, including semantic interoperability via MCP's client-server/JSON-RPC design; a roadmap for MCP-enabled, intelligent transport infrastructures.

Conclusion: MCP can serve as a foundation for next-generation adaptive, context-aware, and intelligent transport systems, enabling standardized integration and bridging protocol-level adaptation with context-aware decisions.
...
### [212] [EMind: A Foundation Model for Multi-task Electromagnetic Signals Understanding](https://arxiv.org/abs/2508.18785)
...
TL;DR: EMind is a foundation model for electromagnetic signals that unifies multiple tasks with a large standardized dataset and novel training strategies to enable cross-task transfer.


<details>
  <summary>Details</summary>
Motivation: Electromagnetic (EM) signals are highly heterogeneous, noisy, and exhibit complex time–frequency structures, making cross-task generalization difficult. There is a scarcity of large, high-quality EM datasets, hindering multitask learning and transfer.

Method: The authors build the first unified, large-scale standardized EM signal dataset spanning multiple signal types and tasks. They introduce a length-adaptive multi-signal packing method that respects signal properties and a hardware-aware training strategy to efficiently learn representations from heterogeneous, multi-source signals. A foundation model is pretrained on this data to support diverse downstream tasks.

Result: EMind achieves strong performance and broad generalization across many downstream tasks, demonstrating effective cross-task transfer and moving toward a unified framework for electromagnetic intelligence.

Conclusion: The work establishes a foundation model for EM signals, paired with a large dataset and specialized training strategies, enabling robust multitask learning and cross-task transfer in electromagnetic sensing and communication.
...
### [213] [Flipping odd matchings in geometric and combinatorial settings](https://arxiv.org/abs/2508.18457)
*Oswin Aichholzer,Sofia Brenner,Joseph Dorfer,Hung P. Hoang,Daniel Perz,Christian Rieck,Francesco Verciani*

Main category: cs.CG

TL;DR: Study of reconfiguring odd matchings (near-perfect matchings) via flips in two settings: geometric (crossing-free) and combinatorial. Provides a complete polynomial-time characterization for the combinatorial setting, a linear-diameter result when connected in that setting, NP-completeness of deciding flip sequences of length k in both settings, and a framework to transform partial order types to general position. Also shows FPT results in convex geometric settings and discusses connectivity in the geometric setting.


<details>
  <summary>Details</summary>
Motivation: Understand the structure and complexity of reconfiguration spaces of near-perfect (odd) matchings, bridging graph-theoretic and geometric settings; extend knowledge about flip graphs and reconfiguration sequences.

Method: (i) Give a complete polynomial-time checkable characterization of graphs where any two odd matchings can be reconfigured into each other (combinatorial setting). (ii) Establish that, in the combinatorial setting, the flip graph diameter (if connected) is linear in the number of vertices. (iii) Prove NP-completeness for deciding the existence of a flip sequence of length k between two matchings in both settings. (iv) Introduce a framework to transform partial order types into general position with polynomial overhead. (v) Show FPT result for k in the geometric setting when restricted to convex point sets.

Result: Provided a full characterization for when all odd matchings are mutually reconfigurable in graphs (combinatorial setting). Proved linear diameter of the flip graph (when connected) in the combinatorial setting. Demonstrated NP-completeness for deciding if a flip sequence of length k exists between two matchings in both settings. Developed a transformation framework from partial order types to general position with polynomial overhead. Proved fixed-parameter tractability in the geometric setting for convex point sets when parameterized by k. Also complemented known connectivity result in the geometric setting by Aichholzer et al.

Conclusion: The work maps the computational landscape of flipping odd matchings, revealing distinct behavior between combinatorial and geometric settings, providing a precise reconfiguration characterization, diameter bound, and NP-hardness results, while offering a framework and FPT results that open avenues for efficient algorithms in restricted regimes.
...
### [214] [Tangling and Untangling Trees on Point-sets](https://arxiv.org/abs/2508.18535)
*Giuseppe Di Battista,Giuseppe Liotta,Maurizio Patrignani,Antonios Symvonis,Ioannis G. Tollis*

Main category: cs.CG

TL;DR: A two-phase method to draw trees with a prescribed number of crossings and bounded edge bends, achieving O(n^2) time for general trees and O(n log n) for paths, with extensions to RAC drawings.


<details>
  <summary>Details</summary>
Motivation: Investigate drawing graphs with a fixed number of crossings on a given point set while keeping curve complexity bounded, blending classical topological graph theory (thrackle number, spines) with practical drawing algorithms for trees.

Method: 1) Tangling phase computes a topological linear embedding of a tree T with θ(T) edge crossings and a constant number of spine traversals. 2) Untangling phase removes crossings down to a target χ∈[0,θ(T)] without increasing spine traversals. Use the resulting linear embedding to construct a drawing on S with χ crossings and constant curve complexity. Algorithms run in O(n^2) for general trees and O(n log n) for paths; adaptable to RAC drawings (crossings at 90°).

Result: Provides an efficient construction of drawings for trees with any χ in [0,θ(T)], with constant bend per edge, and time complexities O(n^2) (general trees) and O(n log n) (paths). Also extends to right-angle crossing (RAC) drawings.

Conclusion: The two-phase tangling–untangling framework yields practical, efficient drawings with prescribed crossing counts and bounded curve complexity, and can be extended to RAC drawings, bridging theoretical graph properties (thrackle number) with concrete drawing algorithms.
...
### [215] [Mining the Long Tail: A Comparative Study of Data-Centric Criticality Metrics for Robust Offline Reinforcement Learning in Autonomous Motion Planning](https://arxiv.org/abs/2508.18397)
*Antonio Guillen-Perez*

Main category: cs.RO

TL;DR: Offline RL for AV planning using data curation to address long-tail events; uncertainty-driven sampling provides the strongest safety gains.


<details>
  <summary>Details</summary>
Motivation: Real-world driving logs are highly imbalanced, with mundane scenarios dominating and rare critical events being underrepresented; non-uniform, information-rich sampling is needed to learn safer policies.

Method: A systematic study of six criticality weighting schemes (three families: heuristic-based, uncertainty-based, behavior-based), evaluated at timestep and complete-scenario levels. Seven goal-conditioned CQL agents with an attention-based architecture are trained and evaluated in the high-fidelity Waymax simulator.

Result: All data curation methods outperform the baseline. Data-driven curation using model uncertainty yields the largest safety gains, reducing collisions from 16.0% to 5.5%. There is a trade-off: timestep-level weighting improves reactive safety, while scenario-level weighting enhances long-horizon planning.

Conclusion: Intelligent, non-uniform data sampling is a critical component for building safe and reliable offline RL-based autonomous agents, and the proposed framework provides a comprehensive approach to data curation in AV planning.
...
### [216] [Maintenance automation: methods for robotics manipulation planning and execution](https://arxiv.org/abs/2508.18399)
*Christian Friedrich,Ralf Gulde,Armin Lechler,Alexander Verl*

Main category: cs.RO

TL;DR: A complete robotic system for maintenance automation that can plan, interpret symbolic plans, and execute disassembly/assembly under uncertainty, validated in real-world experiments.


<details>
  <summary>Details</summary>
Motivation: Automating complex maintenance tasks requires integration of planning, control, and execution, and robustness to environmental deviations. This work aims to bridge theory and practice by translating symbolic plans into executable robot instructions and validating the system in real-world scenarios.

Method: A cognition framework based on planning using CAD and RGB-D data, including a method to interpret symbolic plans and transform them into executable robot instructions for disassembly/assembly; the complete system is evaluated experimentally.

Result: The system is experimentally validated on real-world maintenance tasks, demonstrating feasibility and paving the way toward practical robotic maintenance solutions.

Conclusion: This work represents a first step toward turning theoretical planning approaches into practical maintenance-automation robots, with future work focusing on further integration and robustness.
...
### [217] [Efficient task and path planning for maintenance automation using a robot system](https://arxiv.org/abs/2508.18400)
*Christian Friedrich,Akos Csiszar,Armin Lechler,Alexander Verl*

Main category: cs.RO

TL;DR: An end-to-end autonomous maintenance framework that fuses offline CAD data with online RGB-D perception via probabilistic filtering, using a symbolic disassembly-space planning approach and adaptive, efficient path planning; experimentally validated.


<details>
  <summary>Details</summary>
Motivation: To enable autonomous robot maintenance in factories by solving planning and perception under uncertainty with low computational overhead, addressing the needs of the factory-of-the-future.

Method: Combine offline CAD data with online RGB-D data using a probabilistic filter to mitigate uncertainties. Employ a symbolic planning layer built on a novel sampling-based method to compute the disassembly space. Use global path-planning algorithms with an adaptive exploration-step size to reduce planning time. Validate each method experimentally.

Result: All proposed methods are experimentally validated and discussed, indicating feasibility of end-to-end autonomous maintenance planning and execution under uncertainty, with reported improvements in planning efficiency.

Conclusion: The work presents a viable framework for autonomous maintenance automation by fusing CAD-based offline information with perception-based uncertainty compensation and efficient planning, though more details on metrics and real-world deployment would strengthen the evaluation.
...
### [218] [PneuGelSight: Soft Robotic Vision-Based Proprioception and Tactile Sensing](https://arxiv.org/abs/2508.18443)
*Ruohan Zhang,Uksang Yoo,Yichen Li,Arpit Argawal,Wenzhen Yuan*

Main category: cs.RO

TL;DR: Pneumatic soft robots can gain tactile and proprioceptive sensing through a vision-based sensorization approach using an embedded camera, demonstrated on PneuGelSight with a sim-to-real pipeline for zero-shot transfer.


<details>
  <summary>Details</summary>
Motivation: Soft robots are highly compliant but lack precise sensing for real-world use. A vision-based sensorization method can provide high-resolution tactile feedback and proprioception, enabling reliable control and interaction, while the sim-to-real pipeline addresses deployment gaps.

Method: Develop a PneuGelSight soft pneumatic manipulator with an embedded camera for tactile and proprioceptive sensing. Propose a comprehensive simulation pipeline that models optical and dynamic properties to enable zero-shot transfer from simulation to real-world deployment.

Result: Demonstrated high-resolution proprioception and tactile sensing via the embedded camera. Established a sim-to-real pipeline that enables zero-shot transfer, providing a practical, robust sensing modality for soft robots.

Conclusion: Vision-based sensorization with an accurate optical-dynamic sim-to-real pipeline offers a practical route to endow soft robots with advanced sensing, facilitating the development of more capable and interactive soft robotic systems.
...
### [219] [Mimicking associative learning of rats via a neuromorphic robot in open field maze using spatial cell models](https://arxiv.org/abs/2508.18460)
*Tianze Liu,Md Abu Bakr Siddique,Hongyu An*

Main category: cs.RO

TL;DR: A neuromorphic, associative-learning inspired robotics framework for online spatial learning in open-field mazes, using rodent-like place/grid cell models to enable autonomous, low-power navigation in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Data-driven AI depends on large datasets and heavy computation, which is impractical for SWaP-constrained tasks (e.g., planetary exploration). Emulating associative learning offers data-efficient, adaptive strategies suitable for neuromorphic hardware.

Method: Emulate rodent associative learning on neuromorphic platforms and integrate place- and grid-cell-like spatial representations to support online, real-time spatial learning in open-field mazes; validate via simulations or prototype robotic experiments.

Result: The abstract presents a conceptual framework and proposed methodology; no empirical results are reported; future work aims to demonstrate viability of online associative spatial learning.

Conclusion: Integrating biological spatial cognition with neuromorphic robotics could enhance autonomous capabilities for SWaP-constrained systems, reducing data requirements and enabling real-time adaptation.
...
### [220] [SignLoc: Robust Localization using Navigation Signs and Public Maps](https://arxiv.org/abs/2508.18606)
*Nicky Zimmerman,Joel Loo,Ayush Agrawal,David Hsu*

Main category: cs.RO

TL;DR: SignLoc uses navigation signs with map data (floor plans and OSM graphs) to perform global topo-semantic localization, using a probabilistic observation model within Monte Carlo localization; can localize after seeing only 1-2 signs in large-scale environments.


<details>
  <summary>Details</summary>
Motivation: Address the gap between human-understandable navigation aids and robot localization by leveraging ubiquitous signs and publicly available maps to localize without prior sensor-based mapping.

Method: 1) Extract a navigation graph from the input map (floor plans or OSM). 2) Use a probabilistic observation model to match detected signs' directional and locational cues to the graph. 3) Perform global Monte Carlo localization for robust topo-semantic localization within the graph.

Result: Evaluated in diverse environments (university campus, shopping mall, hospital). SignLoc reliably localizes after observing only one to two signs.

Conclusion: Navigation signs aligned with existing maps enable robust, map-free-initialization global localization of robots in built environments, reducing mapping requirements and enabling quicker localization.
...
### [221] [Integration of Robot and Scene Kinematics for Sequential Mobile Manipulation Planning](https://arxiv.org/abs/2508.18627)
*Ziyuan Jiao,Yida Niu,Zeyu Zhang,Yangyang Wu,Yao Su,Yixin Zhu,Hangxin Liu,Song-Chun Zhu*

Main category: cs.RO

TL;DR: A Sequential Mobile Manipulation Planning framework (SMMP) unifies navigation and manipulation via an augmented configuration space (A-Space) and a tri-level planning pipeline to solve long-horizon mobile manipulation tasks with articulated objects, achieving higher success rates and validated on real robots.


<details>
  <summary>Details</summary>
Motivation: To address long-horizon, multi-step mobile manipulation that requires coordinated whole-body motion and interaction with articulated objects by integrating scene kinematics into the planning process.

Method: Construct an A-Space by merging environmental kinematic models with the robot's kinematics; use a tri-level pipeline: (1) symbolic task planning to evolve A-Space, (2) optimization-based motion planning for continuous trajectories in A-Space, (3) intermediate plan refinement to ensure long-horizon feasibility; validate in simulation and on real robots involving diverse objects and contexts.

Result: Simulation shows an 84.6% higher task success rate when planning in A-Space versus baselines; real-robot experiments demonstrate fluid mobile manipulation across seven object types and 17 contexts, with tasks up to 14 sequential steps.

Conclusion: Modeling scene kinematics as planning entities in A-Space yields a scalable, generalizable approach that effectively couples navigation and manipulation for complex, long-horizon tasks.
...
### [222] [Engineering Automotive Digital Twins on Standardized Architectures: A Case Study](https://arxiv.org/abs/2508.18662)
*Stefan Ramdhan,Winnie Trandinh,Istvan David,Vera Pantelic,Mark Lawford*

Main category: cs.RO

TL;DR: ISO 23247 can serve as a starting point for automotive DT architectures, but its generic nature reveals strengths and gaps for automotive use; a case study on an Adaptive Cruise Control DT highlights practical applicability and points to future directions.


<details>
  <summary>Details</summary>
Motivation: Address the lack of automotive-specific DT architectural guidelines and assess whether ISO 23247 is suitable for automotive digital twins.

Method: Case study applying the ISO 23247 reference architecture to develop an Adaptive Cruise Control digital twin for a 1/10-scale autonomous vehicle, evaluating strengths, limitations, and deriving directions for researchers, practitioners, and standard developers.

Result: The study identifies concrete strengths and gaps in the reference architecture when applied to automotive DTs and offers actionable recommendations to extend/adapt ISO 23247 for automotive use; demonstrates feasibility of building an ADC DT within the standard framework.

Conclusion: ISO 23247 provides a feasible starting point for automotive DTs but needs automotive-specific extensions; future work should refine guidelines, standardization, and tooling to better support automotive digital twins.
...
### [223] [Deep Sensorimotor Control by Imitating Predictive Models of Human Motion](https://arxiv.org/abs/2508.18691)
*Himanshu Gaurav Singh,Pieter Abbeel,Jitendra Malik,Antonio Loquercio*

Main category: cs.RO

TL;DR: Zero-shot sensorimotor policy learning by imitating a human-motion predictor; bypasses retargeting and dense rewards, yielding strong cross-robot performance across tasks.


<details>
  <summary>Details</summary>
Motivation: Bridging the embodiment gap between humans and robots by leveraging large-scale human motion datasets to train predictive models; avoiding gradient-based retargeting and adversarial losses to better utilize diverse human-scene data.

Method: Train a predictive model on human motion data to forecast future motion. Robot end-effector keypoints mirror human keypoints, enabling zero-shot transfer. Train sensorimotor policies to track the model's predictions, conditioned on a history of past robot states, while optimizing a sparse task reward, bypassing explicit retargeting and adversarial training.

Result: The approach outperforms existing baselines by a large margin across multiple robots and tasks. Tracking the human-motion model can substitute for dense rewards and curricula in manipulation tasks. Code and data are provided.

Conclusion: This framework enables scalable cross-domain sensorimotor learning by reusing human motion models, reducing reliance on retargeting and dense rewards, and enabling robust generalization across robots and tasks.
...
### [224] [AgriChrono: A Multi-modal Dataset Capturing Crop Growth and Lighting Variability with a Field Robot](https://arxiv.org/abs/2508.18694)
*Jaehwan Jeong,Tuan-Anh Vu,Mohammad Jony,Shahab Ahmad,Md. Mukhlesur Rahman,Sangpil Kim,M. Khalid Jawed*

Main category: cs.RO

TL;DR: AgriChrono introduces a dynamic, multi-modal field dataset and robotic collection platform to study robustness of 3D reconstruction under real-world farming conditions.


<details>
  <summary>Details</summary>
Motivation: Existing datasets are static or lab-based and fail to reflect real farmland variability in illumination, growth, and disturbances; there is a need for datasets that support robust generalization to dynamic agricultural environments.

Method: Design of AgriChrono: a robotic platform that collects synchronized RGB, Depth, LiDAR, and IMU data; enables long-term, time-synchronized data collection across varying illumination and crop growth stages; includes benchmarking of state-of-the-art 3D reconstruction models on the dataset.

Result: Reconstruction in real-world field conditions remains challenging; AgriChrono exposes domain gaps and provides a benchmark to drive improvements in model generalization under dynamic agricultural conditions.

Conclusion: The dataset and platform offer a publicly accessible resource to advance robust perception in precision agriculture, encouraging development of generalizable models; code and dataset are publicly available.
...
### [225] [Enhancing Video-Based Robot Failure Detection Using Task Knowledge](https://arxiv.org/abs/2508.18705)
*Santosh Thoduka,Sebastian Houben,Juergen Gall,Paul G. Plöger*

Main category: cs.RO

TL;DR: Video-based failure detection leverages spatio-temporal cues (robot actions and task-relevant objects) to improve robustness across real-world scenarios; data augmentation via variable frame rates and test-time augmentation yield modest F1 gains on ARMBench; code and annotations released.


<details>
  <summary>Details</summary>
Motivation: Reliable failure detection is critical for safe and robust robotic task execution, but existing methods struggle with real-world variability. The authors propose to exploit readily available spatio-temporal information (executed actions and in-frame task-relevant objects) to enhance detection performance.

Method: Propose a video-based failure detection approach that uses spatio-temporal knowledge in the form of the robot actions and task-relevant objects in view. They amend three datasets with additional annotations of this information. They introduce a data augmentation method that applies variable frame rates to different parts of the video. They report results on ARMBench with and without test-time augmentation.

Result: On ARMBench, F1 improved from 77.9 to 80.0 with their augmentation, and to 81.4 with test-time augmentation (no extra computational cost for the first augmentation). They demonstrate the value of spatio-temporal information and provide code and annotations for reproducibility.

Conclusion: Spatio-temporal information about actions and objects improves failure detection; variable frame-rate augmentation and test-time augmentation offer performance gains. The work encourages further exploration of heuristics and richer annotations for robust real-world deployment; code and annotations are released.
...
### [226] [HyperTASR: Hypernetwork-Driven Task-Aware Scene Representations for Robust Manipulation](https://arxiv.org/abs/2508.18802)
*Li Sun,Jiefeng Wu,Feng Chen,Ruizhe Liu,Yanchao Yang*

Main category: cs.RO

TL;DR: HyperTASR uses a hypernetwork to adapt scene representations during robotic manipulation based on task and execution phase, creating task-contextual processing paths that improve learning efficiency and representation quality, validated across simulation and real-world tasks with ablations and attention visualizations.


<details>
  <summary>Details</summary>
Motivation: Current task-agnostic representations fall short of human-like selective perception; effective policy learning requires representations that adapt to task objectives and task progression during execution.

Method: A hypernetwork generates representation transformation parameters conditioned on task specifications and progression state, enabling dynamic reformulation of features. It maintains compatibility with existing policy learning frameworks while separating task-contextual and state-dependent processing paths rather than simple feature fusion.

Result: Across multiple representation paradigms, the approach yields substantial performance improvements. Ablation studies show the necessity of task-contextual processing; attention visualizations indicate the model focuses on task-relevant scene information. Evaluations were conducted in both simulation and real-world environments.

Conclusion: Dynamic, task-aware representations enable more efficient learning and better manipulation performance; the approach generalizes across representation types and tasks and aligns with human-like adaptive perception during manipulation.
...
### [227] [Learning Real-World Acrobatic Flight from Human Preferences](https://arxiv.org/abs/2508.18817)
*Colin Merk,Ismail Geles,Jiaxu Xing,Angel Romero,Giorgia Ramponi,Davide Scaramuzza*

Main category: cs.RO

TL;DR: REC enhances Preference PPO for PbRL in agile drone control, achieving strong sim-to-real transfer and highlighting the limits of manually designed rewards.


<details>
  <summary>Details</summary>
Motivation: PbRL avoids hand-crafted rewards, which is crucial for subjective, high-speed maneuvers like acrobatics. Drone control presents complex dynamics and the need for stable learning when objectives include stylistic quality and safety.

Method: Extend Preference PPO with Reward Ensemble under Confidence (REC) to improve reward learning; incorporate a probabilistic reward model; evaluate in simulation with transfer to real drones, and in MuJoCo for continuous control; assess against human preferences and compare to shaped rewards.

Result: REC achieves 88.4% of shaped reward performance versus 55.2% for standard Preference PPO; successful sim-to-real transfer showing multiple acrobatic maneuvers; probabilistic reward model validated in MuJoCo; manual rewards show only 60.7% agreement with human preferences.

Conclusion: PbRL with REC effectively captures complex, human-centered objectives across physical and simulated domains. REC improves reward modeling and learning stability; manual reward design is limited in aligning with human preferences.
...
### [228] [AS2FM: Enabling Statistical Model Checking of ROS 2 Systems for Robust Autonomy](https://arxiv.org/abs/2508.18820)
*Christian Henkel,Marco Lampacrescia,Michaela Klauck,Matteo Morelli*

Main category: cs.RO

TL;DR: AS2FM translates extended SCXML models of ROS 2 and Behavior Trees into the JANI format for Statistical Model Checking, enabling design-time verification of autonomous robots with scalable, linear-time verification that can identify issues quickly on consumer hardware; claims broader feature support than alternatives.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots operate in unpredictable environments; verifying their control software is hard due to complexity and scalability. A practical, scalable verification workflow at design-time is needed to catch issues early and ensure reliability.

Method: Extend SCXML to model ROS 2 and BT features; develop AS2FM to translate the full system model into JANI; use standard SMC tools to verify properties; validate with a ROS 2-based robotic manipulation case study; compare scalability and feature coverage against state-of-the-art approaches.

Result: Case study identifies problems in the robotic manipulation scenario and completes verification in under one second on consumer hardware; verification scales linearly with model size rather than exponentially; AS2FM offers more comprehensive system feature support.

Conclusion: AS2FM enables practical, scalable, and feature-rich design-time verification of autonomous robotic systems via SMC, with fast runtime and broader modeling coverage than current methods.
...
### [229] [VisionSafeEnhanced VPC: Cautious Predictive Control with Visibility Constraints under Uncertainty for Autonomous Robotic Surgery](https://arxiv.org/abs/2508.18937)
*Wang Jiayin,Wei Yanran,Jiang Lei,Guo Xiaoyu,Zheng Ayong,Zhao Weidong,Li Zhongkui*

Main category: cs.RO

TL;DR: A robust, uncertainty-aware autonomous laparoscope control framework (VPC) that uses Gaussian Process Regression to quantify uncertainties and safety-barrier controls with chance constraints to guarantee field-of-view safety in MIS, validated on a commercial robot with near-perfect visibility.


<details>
  <summary>Details</summary>
Motivation: Autonomous laparoscope control in MIS faces continuous visibility requirements and disturbances (parameterization error, sensor noise, payload uncertainties) that can degrade safety and surgeon experience. Existing IBVS may fail under uncertainty, necessitating a framework that can adapt to uncertainty while preserving FoV safety.

Method: Gaussian Process Regression (GPR) to quantify residual model uncertainties, stochastic disturbances, and external disturbances (hybrid deterministic+stochastic model). Uncertainty-propagation-based safety Control Barrier Function (CBF) with probabilistic (chance) constraints. Probabilistic, safety-aware trajectory optimization enabling uncertainty-adaptive control effort allocation to minimize unnecessary camera motion while maintaining robustness.

Result: Simulation and experiments on a commercial surgical robot (MicroPort MedBot Toumai) performing sequential multi-target lymph node dissection. Compared with baseline methods, the framework maintains near-perfect target visibility (>99.9%) and reduces tracking effort/variance (text truncated in abstract).

Conclusion: The VisionSafeEnhanced Visual Predictive Control (VPC) framework delivers robust, uncertainty-adaptive, safety-guaranteed autonomous laparoscope control for MIS, achieving exceptional FoV retention on a real robot and highlighting its potential to improve safety and efficiency in robotic surgery.
...
### [230] [Enhanced UAV Path Planning Using the Tangent Intersection Guidance (TIG) Algorithm](https://arxiv.org/abs/2508.18967)
*Hichem Cheriet,Khellat Kihel Badra,Chouraqui Samira*

Main category: cs.RO

TL;DR: Tangent Intersection Guidance (TIG) offers real-time UAV path planning in static and dynamic/unknown environments using elliptic tangent intersection, dual sub-path generation per threat, heuristic path selection, iterative refinement, and quadratic Bézier smoothing to produce short, smooth, collision-free routes.


<details>
  <summary>Details</summary>
Motivation: UAVs require safe, efficient navigation in static, dynamic, and unknown environments while respecting kinematic and dynamic constraints.

Method: Use elliptic tangent intersections to generate two sub-paths per threat, apply a heuristic rule to select the best route, iteratively refine the path until the target is reached; incorporate a smoothing stage based on quadratic Bézier curves to produce a smooth, feasible trajectory that respects UAV dynamics.

Result:  TIG achieves shorter paths with fewer turning angles than A*, PRM, RRT*, Tangent Graph, and Static APPATT in static environments; demonstrates real-time planning in unknown/partially known environments and improved collision avoidance performance against APF and Dynamic APPATT.

Conclusion: TIG provides efficient, safe UAV navigation across diverse environments by combining tangent-intersection path generation with dynamic smoothing, enabling real-time, collision-free planning.
...
### [231] [HuBE: Cross-Embodiment Human-like Behavior Execution for Humanoid Robots](https://arxiv.org/abs/2508.19002)
*Shipeng Lyu,Fangyuan Wang,Weiwei Lin,Luhao Zhu,David Navarro-Alarcon,Guodong Guo*

Main category: cs.RO

TL;DR: Introduces HuBE, a bi-level closed-loop framework for human-like motion across diverse humanoids, with HPose dataset and bone-scaling augmentation, achieving improved similarity and appropriateness.


<details>
  <summary>Details</summary>
Motivation: There is a persistent gap between behavioral similarity and contextual appropriateness in human-like motion generation, exacerbated by cross-embodiment transfer and structural mismatch between planned motion and real execution.

Method: A bi-level closed-loop framework that integrates robot state, goal poses, and contextual situations to generate human-like behaviors; constructs HPose, a context-enriched dataset with fine-grained situational annotations; employs a bone scaling–based data augmentation strategy to ensure millimeter-level compatibility across heterogeneous humanoid robots.

Result: Demonstrates significant improvements in motion similarity, behavioral appropriateness, and computational efficiency over state-of-the-art baselines on multiple commercial platforms, supporting transferability across diverse humanoid robots.

Conclusion: HuBE provides a solid foundation for transferable, human-like behavior execution across different humanoid platforms and suggests practical pathways for cross-embodiment adaptability.
...
### [232] [An LLM-powered Natural-to-Robotic Language Translation Framework with Correctness Guarantees](https://arxiv.org/abs/2508.19074)
*ZhenDong Chen,ZhanShang Nie,ShiXing Wan,JunYi Li,YongTian Cheng,Shuai Zhao*

Main category: cs.RO

TL;DR: NRTrans is a natural-robotic translation framework that uses a Robot Skill Language (RSL) to verify and refine LLM-generated robot control programs, employing a compiler/debugger and feedback-based fine-tuning to provide correctness guarantees, with strong performance gains especially for lightweight LLMs.


<details>
  <summary>Details</summary>
Motivation: To address high error rates and inconsistency when LLMs generate robot control programs, particularly with light-weight LLMs, and to provide correctness guarantees before execution.

Method: Propose Robot Skill Language (RSL) to abstract robot control programs. Develop an RSL compiler and debugger that verifies LSL (RSL) outputs from LLMs and provides error feedback to the LLM. Use feedback-based fine-tuning of the LLM to refine outputs until verified. Demonstrate NRTrans across diverse LLMs and tasks.

Result: NRTrans outperforms existing methods across a range of LLMs and tasks and achieves a high success rate for light-weight LLMs.

Conclusion: The framework provides correctness guarantees for LLM-generated robotic programs before execution and enhances the effectiveness of LLM-powered robotic applications by integrating an abstraction layer (RSL) with compiler-based verification and feedback-driven refinement.
...
### [233] [DELIVER: A System for LLM-Guided Coordinated Multi-Robot Pickup and Delivery using Voronoi-Based Relay Planning](https://arxiv.org/abs/2508.19114)
*Alkesh K. Srivastava,Jared Michael Levin,Alexander Derrico,Philip Dames*

Main category: cs.RO

TL;DR: DELIVER offers a language-guided, scalable multi-robot coordination framework that uses LLaMA3 for NL parsing, Voronoi-based partitioning, relay planning, and finite-state machine execution, validated in simulation and real hardware with promising scalability and reduced per-agent workload.


<details>
  <summary>Details</summary>
Motivation: Enable scalable, collision-free cooperative pickup-and-delivery by translating natural-language commands into distributed, robust robot coordination in real-world settings.

Method: An integrated pipeline: natural-language understanding via a lightweight LLaMA3 instance to extract pickup/delivery points; Voronoi tessellation to partition space into robot-specific regions; relay-point computation along shared boundaries; coordinated handoffs via a finite-state machine controlling each robot; implementation on MultiTRAIL with ROS2 Gazebo simulations and TurtleBot3 hardware.

Result: DELIVER maintains consistent mission cost across varying team sizes, reduces per-agent workload by up to 55% compared to a single-agent system, and keeps the number of active relay agents low as team size grows, demonstrating scalability and efficient agent utilization.

Conclusion: DELIVER presents a modular, extensible architecture for language-guided multi-robot coordination, contributing to cyber-physical system integration and advancing practical, language-enabled robot collaboration.
...
### [234] [ZeST: an LLM-based Zero-Shot Traversability Navigation for Unknown Environments](https://arxiv.org/abs/2508.19131)
*Shreya Gummadi,Mateus V. Gasparino,Gianluca Capezzuto,Marcelo Becker,Girish Chowdhary*

Main category: cs.RO

TL;DR: Zero-shot traversability mapping via LLM-based visual reasoning enables real-time, safe navigation without dangerous data collection; outperforms SOTA in indoor/outdoor tests.


<details>
  <summary>Details</summary>
Motivation: Avoid risky real-world data collection for terrain traversability datasets; reduce robot risk, cost, and effort; enable scalable, rapid development of navigation systems.

Method: Leverages visual reasoning capabilities of Large Language Models to construct a real-time traversability map from perception data, enabling zero-shot inference without real-world training data; validated in controlled indoor and unstructured outdoor environments.

Result: Demonstrates safer navigation than state-of-the-art methods while consistently reaching the final goal in experiments.

Conclusion: ZeST provides a safe, scalable, and cost-effective approach for real-time traversability mapping and autonomous navigation by leveraging LLM-based visual reasoning.
...
### [235] [Uncertainty-Resilient Active Intention Recognition for Robotic Assistants](https://arxiv.org/abs/2508.19150)
...
TL;DR: A framework for intention recognition in robotic assistants that handles uncertainty using an intention-recognition POMDP, integrating real-time sensor data with cooperative planning, demonstrated on a real robot.


<details>
  <summary>Details</summary>
Motivation: Existing approaches depend on explicit prompts or near-perfect information, but fail to cope with uncertainty in human intent and perception errors; thus there is a need for robust, sensor-noise-resilient autonomy.

Method: Proposes an intention-recognition POMDP that couples cooperative planning and action under uncertainty, integrating real-time sensor data with a suite of planners in a resilient framework.

Result: Implemented and tested on a physical robot with promising results, indicating feasibility of robust intention recognition under uncertainty.

Conclusion: The framework advances autonomous human-robot collaboration by addressing uncertainty in intention recognition and perception, showing feasibility and potential for broader deployment and further refinement.
...
### [236] [QuadKAN: KAN-Enhanced Quadruped Motion Control via End-to-End Reinforcement Learning](https://arxiv.org/abs/2508.19153)
*Allen Wang,Gavin Tao*

Main category: cs.RO

TL;DR: QuadKAN introduces a spline-parameterized cross-modal policy for vision-guided quadruped control, using Kolmogorov-Arnold Networks (KANs) to fuse proprioception and vision via spline encoders, trained with PPO and MMDR. It improves sample efficiency, reduces action jitter and energy use, and offers interpretable posture-action sensitivities, outperforming SOTA baselines across terrains and obstacle scenarios; a code repo will be released.


<details>
  <summary>Details</summary>
Motivation: Vision-guided locomotion benefits from integrating proprioceptive signals with visual inputs to achieve robust control in varied terrains and obstacle layouts. Proprioception helps suppress perception noise and jitter, while vision enables terrain understanding; combining them in a structured, interpretable policy could improve sample efficiency, stability, and energy use.

Method: QuadKAN uses a spline-parameterized policy instantiated with Kolmogorov-Arnold Networks (KANs). It includes a spline encoder for proprioception and a spline fusion head for combining proprioception and vision inputs. The policy class aligns with the piecewise-smooth nature of gait. Training is end-to-end with PPO, using Multi-Modal Delay Randomization (MMDR) to handle sensor delays.

Result: Empirical evaluation across even and uneven terrains and scenarios with static or dynamic obstacles shows QuadKAN achieves higher returns, longer travel distances, and fewer collisions than state-of-the-art baselines. The approach improves sample efficiency, reduces action jitter and energy consumption, and provides interpretable posture-action sensitivities.

Conclusion: Spline-parameterized policies offer a simple, effective, and interpretable approach for robust vision-guided locomotion in quadrupeds, with QuadKAN outperforming SOTA baselines across varied terrains and obstacle conditions. A repository will be released upon acceptance.
...
### [237] [Real-time Testing of Satellite Attitude Control With a Reaction Wheel Hardware-In-the-Loop Platform](https://arxiv.org/abs/2508.19164)
*Morokot Sakal,George Nehma,Camilo Riano-Rios,Madhur Tiwari*

Main category: cs.RO

TL;DR: Presents a Hardware-in-the-Loop (HIL) testbed for an adaptive satellite attitude control system with reaction wheel health estimation, using CAN-bus motors, an embedded controller, and a satellite simulator; explores fault injection and lessons learned.


<details>
  <summary>Details</summary>
Motivation: To validate spacecraft attitude control algorithms beyond simulation, by incorporating real momentum exchange hardware and a validated testing framework that includes fault injection capabilities.

Method: Developed a HIL setup comprising brushless DC motors and CAN bus drivers, an onboard computer implementing adaptive control laws, and a satellite simulator providing sensor data and attitude responses; designed procedures to artificially induce reaction wheel faults.

Result: Proposes fault-injection methods and reports issues and practical lessons learned from the HIL experimentation; demonstrates pathway towards a comprehensive testing framework (no quantitative results reported).

Conclusion: This work advances rigorous validation of spacecraft attitude control by bridging simulations to hardware-in-the-loop, enabling assessment of adaptation strategies and reaction wheel health estimation under failure conditions.
...
### [238] [Direction Informed Trees (DIT*): Optimal Path Planning via Direction Filter and Direction Cost Heuristic](https://arxiv.org/abs/2508.19168)
*Liding Zhang,Kejia Chen,Kuanqi Cai,Yu Zhang,Yixuan Dang,Yansong Wu,Zhenshan Bing,Fan Wu,Sami Haddadin,Alois Knoll*

Main category: cs.RO

TL;DR: DIT* is a direction-aware, sampling-based planner that biases edge exploration using directional costs and a similarity-filtered neighbor set, enabling faster convergence than single-query planners like EIT* in high-dimensional spaces and real-world tasks.


<details>
  <summary>Details</summary>
Motivation: In path planning, there is a persistent trade-off between heuristic accuracy and computational efficiency. Traditional heuristics that are fast may be overly conservative, while accurate heuristics can be expensive. DIT* aims to resolve this by sharing directional information to guide search more effectively.

Method: Treats edges as generalized vectors and uses similarity indexes to form a directional filter that selects nearest neighbors and estimates direction costs. These estimated direction costs are used in edge evaluation, enabling efficient sharing of directional information across edges to bias expansion toward promising directions.

Result: Empirical results show faster convergence than existing single-query, sampling-based planners on problems in 4D to 16D spaces, with successful demonstrations in real-world environments for various planning tasks (video provided).

Conclusion: Direction-informed directional costs enable more efficient search guidance in sampling-based planning, yielding faster convergence and robust performance in high-dimensional settings. DIT* offers a practical enhancement over existing planners and warrants further exploration of parameter sensitivity and scalability.
...
### [239] [From Tabula Rasa to Emergent Abilities: Discovering Robot Skills via Real-World Unsupervised Quality-Diversity](https://arxiv.org/abs/2508.19172)
*Luca Grillotti,Lisa Coiffard,Oscar Pang,Maxence Faldor,Antoine Cully*

Main category: cs.RO

TL;DR: URSA extends Quality-Diversity Actor-Critic (QDAC) to autonomously discover and master diverse, high-performing skills directly in the real world, demonstrated on a Unitree A1 quadruped; shows improved damage adaptation in several simulated and real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Autonomous, diverse skill discovery on real robots is hampered by safety concerns, data inefficiency, and reliance on manually defined skill spaces and heuristics. The work aims to enable continuous skill discovery with minimal human input to build more capable and adaptable robots.

Method: Introduce Unsupervised Real-world Skill Acquisition (URSA), an extension of QDAC that can autonomously discover and master diverse locomotion skills on hardware, with support for both heuristic-driven discovery and fully unsupervised modes; builds a reusable skill repertoire and demonstrates transfer to downstream tasks like real-world damage adaptation.

Result: URSA successfully discovers diverse locomotion skills on the Unitree A1 in both simulation and real-world settings. It outperforms baselines in 5 out of 9 simulated and 3 out of 5 real-world damage scenarios.

Conclusion: Proposes a new framework for real-world robot learning that enables continuous skill discovery with limited human intervention, marking a step toward more autonomous and adaptable robots; includes demonstration videos.
...
### [240] [Real-Time Model Checking for Closed-Loop Robot Reactive Planning](https://arxiv.org/abs/2508.19186)
*Christopher Chandler,Bernd Porr,Giulia Lafratta,Alice Miller*

Main category: cs.RO

TL;DR: Real-time, multi-step planning and obstacle avoidance on a low-power autonomous robot using a lightweight, in situ model-checking approach with bio-inspired 'core knowledge' and a novel LiDAR discretization; it outperforms one-step reactive controllers and serves as an instructional case study for explainable autonomous vehicle planning.


<details>
  <summary>Details</summary>
Motivation: Need for safe, reliable, and explainable planning on resource-constrained autonomous systems. Bridge between purely reactive planning and model checking, enabling real-time, verifiable multi-step decision making on low-power hardware.

Method: Develop a small, purpose-built model-checking algorithm that generates plans in real-time without precomputed data. It chains temporary control systems to counter disturbances, uses a novel discretization of 2D LiDAR data sensitive to bounded local variations, and applies forward depth-first search for multi-step planning in cul-de-sac and playground scenarios.

Result: Empirical results and informal proofs indicate the approach yields efficient multi-step plans for local obstacle avoidance, outperforming a purely reactive agent limited to one-step planning. Demonstrates real-time planning on a low-powered device and serves as an instructional case study for safe, explainable autonomous vehicle planning.

Conclusion: Model checking can underpin real-time, safe, and explainable multi-step local obstacle avoidance on autonomous vehicles, particularly on resource-constrained hardware.
...
### [241] [AutoRing: Imitation Learning--based Autonomous Intraocular Foreign Body Removal Manipulation with Eye Surgical Robot](https://arxiv.org/abs/2508.19191)
*Yue Wang,Wenjie Deng,Haotian Xue,Di Cui,Yiqi Chen,Mingchuan Zhou,Haochao Ying,Jian Wu*

Main category: cs.RO

TL;DR: AutoRing enables autonomous intraocular foreign body ring manipulation via imitation learning, with dynamic RCM calibration and an RCM-ACT architecture that uses action-chunking transformers and real-time kinematic realignment, trained on stereo vision and instrument kinematics in a biomimetic eye model, achieving end-to-end autonomy without depth sensing.


<details>
  <summary>Details</summary>
Motivation: Intraocular manipulation requires millimeter-level precision in confined spaces. Existing robotic systems rely on manual teleoperation with steep learning curves and suffer from kinematic uncertainties due to variable motion scaling and fluctuations in the Remote Center of Motion (RCM) point. A need exists for autonomous systems robust to these uncertainties.

Method: AutoRing integrates dynamic RCM calibration to align coordinate systems amid intraocular instrument variation. It introduces the RCM-ACT architecture, which combines action-chunking transformers with real-time kinematic realignment. The model is trained solely on stereo visual data and instrument kinematics from expert demonstrations inside a biomimetic eye model, enabling autonomy without explicit depth sensing.

Result: The approach successfully completes ring grasping and positioning tasks and demonstrates end-to-end autonomy under uncalibrated microscopy conditions.

Conclusion: AutoRing provides a viable framework for developing intelligent eye-surgical systems capable of performing complex intraocular procedures autonomously, addressing key kinematic uncertainties and enabling adaptable autonomous manipulation.
...
### [242] [Planning-Query-Guided Model Generation for Model-Based Deformable Object Manipulation](https://arxiv.org/abs/2508.19199)
*Alex LaGrassa,Zixuan Huang,Dmitry Berenson,Oliver Kroemer*

Main category: cs.RO

TL;DR: Diffusion-based model generator that creates task-specific, spatially adaptive dynamics models for high-dimensional planning, yielding faster planning with minor performance loss on deformable-object tasks.


<details>
  <summary>Details</summary>
Motivation: Efficient planning in high-dimensional spaces requires expressive yet tractable dynamics models; fixed-resolution models are insufficient for deformables; need automatic, region-wise adaptivity guided by planning queries.

Method: A diffusion-based generator predicts per-region model resolutions from start/goal point clouds. Data collection occurs in two stages: (1) optimize resolutions using predictive dynamics as a prior, (2) directly optimize based on closed-loop task performance. Evaluated on a tree-manipulation task.

Result: Planning speed is doubled with only a small decrement in task performance relative to a full-resolution model.

Conclusion: Demonstrates the feasibility of generating computation-efficient yet expressive, task-specific dynamics models from prior planning/control data, and points toward reuse of such data for new tasks.
...
### [243] [MemoryVLA: Perceptual-Cognitive Memory in Vision-Language-Action Models for Robotic Manipulation](https://arxiv.org/abs/2508.19236)
*Hao Shi,Bin Xie,Yingfei Liu,Lin Sun,Fengrong Liu,Tiancai Wang,Erjin Zhou,Haoqiang Fan,Xiangyu Zhang,Gao Huang*

Main category: cs.RO

TL;DR: MemoryVLA introduces a cognition-memory-action framework for long-horizon robotic manipulation, integrating working memory and a perceptual-cognitive memory bank with a memory-conditioned diffusion action policy. It uses a pretrained VLM to generate tokens, retrieves and fuses memory entries, and updates the memory bank to produce temporally aware actions.


<details>
  <summary>Details</summary>
Motivation: Robotic manipulation is inherently non-Markovian and current video-language action (VLA) models struggle with long-horizon, temporally dependent tasks. Humans rely on working memory for immediate control and episodic/semantic long-term memory for past experiences. The paper aims to bridge this cognitive divide to improve long-horizon manipulation.

Method: A pretrained vision-language model encodes observations into perceptual and cognitive tokens forming a working memory. A perceptual-cognitive memory bank stores low-level details and high-level semantics derived from these tokens. Working memory retrieves decision-relevant entries from the bank, adaptively fuses them with current tokens, and updates the bank by merging redundancies. A memory-conditioned diffusion action expert uses these tokens to generate temporally aware action sequences.

Result: MemoryVLA outperforms state-of-the-art baselines CogACT and pi-0 across 150+ tasks on three robots (SimﬂerEnv-Bridge: 71.9%, Fractal: 72.7%, LIBERO-5: 96.5%) with notable gains (e.g., +14.6 on Bridge). It also achieves 84.0% success on 12 real-world tasks, with long-horizon tasks showing a +26 improvement over baselines.

Conclusion: Incorporating cognition-inspired working memory and a long-term memory bank with a memory-conditioned planner enables temporally aware, long-horizon robotic manipulation, yielding robust improvements across simulation and real-world tasks.
