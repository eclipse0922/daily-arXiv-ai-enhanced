...
### [1] [When CNNs Outperform Transformers and Mambas: Revisiting Deep Architectures for Dental Caries Segmentation](https://arxiv.org/abs/2511.14860)
...
TL;DR: CNN-based DoubleU-Net outperforms transformers and Mamba variants in dental caries segmentation on panoramic radiographs; data limitations and domain priors matter more than model complexity.


<details>
  <summary>Details</summary>
Motivation: Benchmark diverse architectures (CNNs, vision transformers, and Mamba architectures) for automated dental caries segmentation to identify architecture-task alignment and guide model choice under limited annotated data.

Method: Train twelve state-of-the-art architectures (including VMUnet, MambaUNet, VMUNetv2, RMAMamba-S, TransNetR, PVTFormer, DoubleU-Net, ResUNet++) under identical configurations on the DC1000 panoramic dental caries dataset and evaluate using dice, mIoU, and precision.

Result: DoubleU-Net achieved the highest dice coefficient (0.7345), mIoU (0.5978), and precision (0.8145); CNN-based architectures dominated the top results; Transformer and Mamba variants underperformed due to limited data and weaker spatial priors.

Conclusion: In this domain with limited annotated data, architecture-task alignment can trump model complexity; simpler CNN architectures like DoubleU-Net can outperform more complex attention-based or Mamba architectures for dental caries segmentation on panoramic radiographs.
...
TL;DR: Presents BR-DF: a volumetric representation for CAD B-Rep using a signed distance function for the surface and per-face unsigned distance functions for topology, enabling a robust (100% success) conversion to watertight B-Rep via an extended Marching Cubes; and a diffusion-based generator (3D U-Net) to jointly predict SDF and per-face UDFs.


<details>
  <summary>Details</summary>
Motivation: Address the fragility and non-watertight nature of many CAD B-Rep reconstructions by embedding B-Rep topology in a volumetric framework, enabling reliable conversion to faceted B-Rep and enabling data-driven generation of CAD models.

Method: Define BR-DF as a hybrid volumetric representation: an SDF encodes the CAD surface geometry; per-face UDFs encode vertices/edges/faces topology and per-face information. Extend Marching Cubes to directly convert BR-DF to watertight faceted B-Rep. Develop a multi-branch latent diffusion model with a 3D U-Net backbone to jointly generate the SDF and per-face UDFs of BR-DF models.

Result: Achieves comparable CAD generation performance to state-of-the-art methods, with the unprecedented 100% success rate in producing (faceted) B-Rep models, i.e., guaranteed watertight B-Rep outputs via the BR-DF pipeline.

Conclusion: BR-DF provides a robust, volumetric pathway to CAD B-Rep generation and conversion, combining implicit surface representation with topology-encoding per-face functions, and enabling reliable diffusion-based generation of both geometry and topology for CAD models.
...
### [3] [GeoSceneGraph: Geometric Scene Graph Diffusion Model for Text-guided 3D Indoor Scene Synthesis](https://arxiv.org/abs/2511.14884)
*Antonio Ruiz,Tao Wu,Andrew Melnik,Qing Cheng,Xuqin Wang,Lu Liu,Yongliang Wang,Yanfeng Zhang,Helge Ritter*

Main category: cs.CV

TL;DR: A text-conditioned, graph-based 3D indoor scene synthesizer (GeoSceneGraph) using equivariant graph neural networks (EGNNs) that leverages scene graph structure and geometric symmetry without predefined relationship classes, achieving competitive results with relation-based methods and enabling efficient deployment on constrained devices.


<details>
  <summary>Details</summary>
Motivation: Indoor 3D scene synthesis from text is valuable for film, design, gaming, VR, and synthetic data. Current approaches either train from scratch (lacking structural coherence) or rely on vision-language models; graph-based methods often need user-provided graphs or ground-truth relationships, hindering practicality. There is a need for a scalable, flexible approach that exploits the natural graph structure of scenes without heavy annotations and can run on limited hardware.

Method: Propose GeoSceneGraph, an EGNN-based framework conditioned on text features to synthesize 3D indoor scenes from prompts. It explicitly leverages the graph structure and geometric symmetries of scenes, does not rely on predefined relationship classes or ground-truth relationship annotations, and introduces a simple strategy to condition EGNNs on high-dimensional text features. Ablation studies validate design choices.

Result: GeoSceneGraph achieves performance comparable to methods that rely on ground-truth relationships, despite not using such relationships. The conditioning strategy for text features effectively integrates language input with the EGNN framework, and ablations support the proposed design.

Conclusion: By marrying graph-based, symmetry-aware representations with text-conditioned EGNNs, GeoSceneGraph enables coherent, scalable indoor scene synthesis from prompts without requiring relationship annotations, offering potential for efficient deployment on resource-constrained devices and broader applicability across domains.
...
### [4] [HULFSynth : An INR based Super-Resolution and Ultra Low-Field MRI Synthesis via Contrast factor estimation](https://arxiv.org/abs/2511.14897)
*Pranav Indrakanti,Ivor Simpson*

Main category: cs.CV

TL;DR: An unsupervised, bidirectional MRI synthesis framework that translates between HF (high-field) and ULF (ultra-low-field) images using a physics-inspired forward model and an implicit neural representation for super-resolution, achieving notable WM-GM contrast gains on both synthetic and real low-field data.


<details>
  <summary>Details</summary>
Motivation: Bridge cross-field MRI data (HF and ULF) and enable unsupervised synthesis by leveraging the physics of how MR contrast changes with field strength, reducing reliance on paired datasets.

Method: A forward model estimates tissue-type SNR values conditioned on target contrast to simulate HF→ULF transformation. For super-resolution, an Implicit Neural Representation (INR) predicts HF intensities and tissue-type segmentations without observed HF data. The approach is trained/validated on synthetic ULF-like data derived from 3T T1 images and on paired 3T–64mT T1 images.

Result: WM-GM contrast improved by 52% in synthetic ULF-like images and 37% in 64mT images. The method demonstrates robustness to variations in target contrast, noise, and initial seeding.

Conclusion: The study presents a physics-informed, unsupervised bidirectional MRI synthesis framework that can enhance cross-field MRI synthesis and super-resolution without direct HF data, with strong WM-GM contrast gains and demonstrated robustness.
...
TL;DR: InstructMix2Mix (I-Mix2Mix) distills 2D diffusion editor capabilities into a pretrained multi-view diffusion model to enable consistent, multi-view edits from sparse inputs by leveraging a 3D prior and novel SDS adaptations.


<details>
  <summary>Details</summary>
Motivation: The problem of editing scenes from sparse, multi-view inputs requires maintaining cross-view consistency. Existing approaches using per-scene neural fields or temporal attention often produce artifacts and incoherent edits when the views are sparse or non-redundant.

Method: I-Mix2Mix distills a 2D diffusion editor into a pretrained multi-view diffusion model, replacing the neural field consolidator in Score Distillation Sampling (SDS) with a multi-view diffusion student. It introduces: (1) incremental student updates across diffusion timesteps, (2) a specialized teacher noise scheduler to avoid degeneration, and (3) an attention modification to boost cross-view coherence without extra cost.

Result: Experiments show improved multi-view consistency while maintaining high per-frame edit quality.

Conclusion: I-Mix2Mix effectively transfers 2D editing capabilities into a multi-view framework, leveraging a 3D prior to achieve coherent edits across views with sparse inputs and preserving per-frame quality; promising for robust multi-view image editing.
...
TL;DR: SkinR1 proposes a dermatology vision-language model that fuses textbook-based reasoning with reinforcement learning to achieve grounded, scalable diagnostic performance across heterogeneous data.


<details>
  <summary>Details</summary>
Motivation: To tackle three core challenges in clinical VLMs for dermatology: (1) data heterogeneity with inconsistent labels and annotations, (2) absence of grounded diagnostic rationales requiring reliable supervision, and (3) limited scalability/generalization from small, densely annotated datasets to large, sparsely annotated ones.

Method: 1) A textbook-based reasoning generator that creates hierarchy-aware, differential-diagnosis (DDx) informed trajectories for expert-like supervision; 2) supervised fine-tuning (SFT) to instill grounded reasoning; 3) a novel RL paradigm that leverages disease hierarchy to transfer grounded reasoning patterns to large-scale, sparse data.

Result: Extensive experiments on multiple dermatology datasets show superior diagnostic accuracy; ablation confirms the importance of the reasoning foundation provided by SFT.

Conclusion: SkinR1 demonstrates that integrating a solid reasoning foundation with RL enables scalable, grounded clinical VLMs for dermatology, addressing data heterogeneity and generalization challenges.
...
TL;DR: FarSLIP advances remote sensing vision–language pretraining by combining patch-to-patch distillation for local/global alignment with CLS-based region-category supervision, built on the MGRS-200k dataset; it achieves new state-of-the-art in RS open-vocabulary segmentation, zero-shot classification, and image-text retrieval.


<details>
  <summary>Details</summary>
Motivation: Global CLIP-style alignment misses fine-grained region details in remote sensing and current RS-text datasets underutilize object-level supervision; naive region-text alignment often harms semantic coherence, hindering performance.

Method: Construct MGRS-200k, a multi-granularity RS image-text dataset with rich object-level supervision for region-category alignment. Develop FarSLIP, which uses patch-to-patch distillation to align local and global visual cues and employs CLS-token-based region-category alignment to exploit region-text supervision without explicit patch-level alignment.

Result: FarSLIP yields improved fine-grained vision–language alignment in RS, setting new state-of-the-art on RS open-vocabulary semantic segmentation and on image-level tasks like zero-shot classification and image-text retrieval.

Conclusion: FarSLIP demonstrates that careful preservation of semantic coherence, combined with patch-level dispositional alignment and CLS-based region supervision, significantly enhances RS-Language-Image pretraining; the dataset, code, and models are publicly available.
...
TL;DR: nnMIL is a simple, generalizable multiple-instance learning framework that links patch-level foundation-model features to robust, slide-level clinical predictions in pathology via random sampling, large-batch optimization, and a lightweight sliding-window aggregator with uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of current patch-to-slide aggregation methods in pathology foundation models, improving generalizability, reliability, and uncertainty quantification across diverse tasks and external cohorts.

Method: Introduce random sampling at both patch and feature levels to enable large-batch optimization; adopt task-aware sampling strategies; use a lightweight aggregator with sliding-window inference to produce ensemble slide-level predictions and enable principled uncertainty estimation.

Result: Across 40,000 whole-slide images, 35 clinical tasks, and four pathology foundation models, nnMIL outperformed existing MIL methods for disease diagnosis, histologic subtyping, molecular biomarker detection, and pan-cancer prognosis. It demonstrated strong cross-model generalization, reliable uncertainty quantification, and robust survival stratification in external cohorts.

Conclusion: nnMIL offers a practical and generalizable solution for translating pathology foundation models into clinically meaningful predictions, facilitating reliable AI deployment in real-world settings.
...
### [10] [CPSL: Representing Volumetric Video via Content-Promoted Scene Layers](https://arxiv.org/abs/2511.14927)
...
### [20] [ESA: Energy-Based Shot Assembly Optimization for Automatic Video Editing](https://arxiv.org/abs/2511.02505)
...
### [33] [Fast Post-Hoc Confidence Fusion for 3-Class Open-Set Aerial Object Detection](https://arxiv.org/abs/2511.15343)
...
### [59] [RS-CA-HSICT: A Residual and Spatial Channel Augmented CNN Transformer Framework for Monkeypox Detection](https://arxiv.org/abs/2511.15476)
...
### [64] [CompTrack: Information Bottleneck-Guided Low-Rank Dynamic Token Compression for Point Cloud Tracking](https://arxiv.org/abs/2511.15580)
...
### [73] [MF-GCN: A Multi-Frequency Graph Convolutional Network for Tri-Modal Depression Detection Using Eye-Tracking, Facial, and Acoustic Features](https://arxiv.org/abs/2511.15675)
...
### [74] [WarNav: An Autonomous Driving Benchmark for Segmentation of Navigable Zones in War Scenes](https://arxiv.org/abs/2511.15429)
...
### [75] [Representation Space Constrained Learning with Modality Decoupling for Multimodal Object Detection](https://arxiv.org/abs/2511.15433)
*YiKang Shao,Tao Shi*
...
### [97] [INQUIRE-Search: A Framework for Interactive Discovery in Large-Scale Biodiversity Databases](https://arxiv.org/abs/2511.15656)
...
### [98] [MoDES: Accelerating Mixture-of-Experts Multimodal Large Language Models via Dynamic Expert Skipping](https://arxiv.org/abs/2511.15690)
...
### [103] [The Illusion of Procedural Reasoning: Measuring Long-Horizon FSM Execution in LLMs](https://arxiv.org/abs/2511.14777)
*Mahdi Samiei,Mahdi Mansouri,Mahdieh Soleymani Baghshah*

Main category: cs.AI

TL;DR: FSM Execution provides a transparent, deterministic benchmark to evaluate procedural reasoning in LLMs by requiring step-by-step execution of an explicit finite-state machine. Results show degradation with longer horizons and higher branching; larger models help locally but remain brittle in multi-step tasks unless intermediate steps are externalized.


<details>
  <summary>Details</summary>
Motivation: There is a gap in interpretable benchmarks for long-horizon, rule-based procedural reasoning in LLMs. FSM-based execution isolates procedural fidelity from world knowledge by using deterministic transition rules.

Method: Provide explicit FSM definitions to the model; the model executes step-by-step based on input actions, maintaining state across turns. Measure Turn Accuracy (per-step) and Task Accuracy (overall). Systematically vary horizon and branching, compare memory span vs rule retrieval, examine effect of model size, and study prompts that elicit intermediate steps.

Result: Empirical findings show systematic degradation as task horizon or branching complexity increases. High branching factor in rule retrieval hurts performance more than limited memory span. Larger models improve local (per-step) accuracy but remain brittle in multi-step scenarios unless prompted to externalize intermediate steps.

Conclusion: FSM-based evaluation offers a transparent, complexity-controlled probe for diagnosing procedural-fidelity failures in LLMs, guiding inductive-bias design to enable genuine long-horizon procedural competence and providing a rigorous foundation for studying and improving algorithmic reliability in LLMs.
...
### [104] [Learning Interestingness in Automated Mathematical Theory Formation](https://arxiv.org/abs/2511.14778)
*George Tsoukalas,Rahul Saha,Amitayush Thakur,Sabrina Reguyal,Swarat Chaudhuri*

Main category: cs.AI

TL;DR: FERMAT introduces an RL environment for automating open-ended theory discovery and an evolutionary approach (with LLM-based function abstraction) to learn interestingness measures, showing improved discovery in number theory and finite fields; code available.


<details>
  <summary>Details</summary>
Motivation: Automating discovery of new mathematical theories is a grand AI challenge; enabling concept discovery, theorem proving, and objective scoring of 'interestingness' to drive exploration.

Method: Define FERMAT RL environment with symbolic actions for concept discovery and theorem proving; develop an LLM-based evolutionary algorithm that abstracts functions to synthesize interestingness measures; evaluate on elementary number theory and finite fields; release open-source.

Result: LLM-based EA with function abstraction outperforms hard-coded baselines in discovering nontrivial mathematical objects; demonstrates effective automatic scoring of interestingness; FERMAT released on GitHub.

Conclusion: FERMAT provides a flexible platform for automated theory discovery; LLM-assisted evolution improves learning of meaningful scoring functions; potential applicability to broader domains and future research directions.
...
### [105] [Ask WhAI:Probing Belief Formation in Role-Primed LLM Agents](https://arxiv.org/abs/2511.14780)
*Keith Moore,Jun W. Kim,David Lyu,Jeffrey Heo,Ehsan Adeli*

Main category: cs.AI

TL;DR: Ask WhAI is a systems-level framework to inspect and perturb belief states in multi-agent reasoning, enabling replay, off-record belief queries, and counterfactual evidence injection in a medical case simulator; it reveals epistemic priors and silos and offers a reproducible testbed.


<details>
  <summary>Details</summary>
Motivation: To understand how entrenched priors and disciplinary stances shape AI-driven multi-agent reasoning, and to create a controllable environment to study belief dynamics and epistemic silos in scientific collaboration.

Method: Architectural framework that records and replays agent interactions on a shared time-stamped EMR; supports out-of-band queries into each agent’s beliefs and rationale; allows counterfactual evidence injection; uses breakpoints at diagnostic moments; employs a LabAgent oracle with ground truth results; a set of role-prior primed agents (e.g., neurologist, infectious disease specialist) and a moderator, with sequential or parallel encounters; analysis compares pre- and post-event beliefs.

Result: Findings show agent beliefs reflect real-world disciplinary stances (e.g., canonical-study bias, resistance to counterevidence); beliefs can be traced and interrogated beyond human capability, enabling study of how priors and evidence integration affect reasoning; demonstrates a reproducible method to analyze belief formation and epistemic silos in multi-agent reasoning.

Conclusion: Ask WhAI offers a reproducible, testable framework for studying belief formation, epistemic silos, and the impact of priors in collaborative AI reasoning, with potential applications in interpretability, safety assessment, and education of multi-agent systems.
...
### [106] [Subnational Geocoding of Global Disasters Using Large Language Models](https://arxiv.org/abs/2511.14788)
*Michele Ronco,Damien Delforge,Wiebke S. Jäger,Christina Corbane*

Main category: cs.AI

TL;DR: A fully automated LLM-assisted workflow geocodes disaster events by cross-verifying GADM, OpenStreetMap, and Wikidata, producing reliability scores; applied to EM-DAT 2000–2024 to geocode 14,215 events across 17,948 locations; no manual intervention, scalable, and cross-source verifiable.


<details>
  <summary>Details</summary>
Motivation: Disaster databases often store subnational locations in unstructured text with inconsistent granularity and spelling, hindering integration with spatial datasets and risk assessment. An automated, scalable, and verifiable geocoding workflow is needed to improve data quality and utility.

Method: Use GPT-4o to process and clean textual location information. Assign geometries by cross-checking three independent geoinformation repositories—GADM, OpenStreetMap, and Wikidata. Compute a reliability score based on agreement among sources and their availability, then generate subnational geometries. The workflow operates with no manual intervention and supports cross-verification and flexible remapping to preferred frameworks.

Result: Applied to EM-DAT data from 2000–2024, geocoding 14,215 events across 17,948 unique locations. The method achieves cross-source verification, covers all disaster types, and scales without manual curation, enabling flexible remapping to users' frameworks.

Conclusion: LLMs can effectively extract and structure geographic information from unstructured disaster-text data, offering a scalable, reliable approach for disaster risk assessment and reduction through automated, cross-verified geocoding.
...
### [107] [Project Rachel: Can an AI Become a Scholarly Author?](https://arxiv.org/abs/2511.14819)
*Martin Monperrus,Benoit Baudry,Clément Vidal*

Main category: cs.AI

TL;DR: An action research project creates an AI author identity (Rachel So) and publishes AI-generated papers to study how academia responds to AI authorship; results show multiple publications, citations, and a peer-review invitation, highlighting significant implications for publishers and scholarly systems.


<details>
  <summary>Details</summary>
Motivation: To empirically examine the effects of AI-authored scholarly content on the academic ecosystem and the future of scholarly communication.

Method: Action research; create a complete AI academic identity; publish AI-generated research papers (10+) between March and October 2025; track reception, citations, invitations, and discuss implications.

Result: Published 10+ AI-generated papers; achieved citations and a peer-review invitation; observed ecosystem responses; provides data on how publishers, researchers, and the scientific system react to AI authorship.

Conclusion: AI authorship has profound implications for scholarly publishing and the scientific enterprise; the study provides empirical data to inform the future discourse on superhuman AI systems in research.
...
### [108] [Uncertainty-Aware Measurement of Scenario Suite Representativeness for Autonomous Systems](https://arxiv.org/abs/2511.14853)
*Robab Aghazadeh Chakherlou,Siddartha Khastgir,Xingyu Zhao,Jerein Jeyachandran,Shufeng Chen*

Main category: cs.AI

TL;DR: An imprecise Bayesian approach is proposed to quantify how representative scenario data are of the system’s operational domain, yielding interval-based uncertainty estimates rather than a single value.


<details>
  <summary>Details</summary>
Motivation: Trustworthy AI requires datasets that reflect actual operating conditions (ODD/TOD); current data may be incomplete or biased, especially with limited data and unknown TOD distribution.

Method: Compare feature distributions of scenario suites with the inferred TOD distribution; model TOD with imprecise Bayesian priors to handle uncertainty and dependence among features; derive local (between categories) and global interval representations of representativeness.

Result: Produces interval-valued, uncertainty-aware estimates of representativeness that reflect priors and data limitations; demonstrates the approach with a numerical example across categories such as weather, road type, and time of day, including dependencies.

Conclusion: Imprecise Bayesian representativeness offers robust, uncertainty-aware guidance for dataset design and validation in safety-critical AI systems and can inform targeted data collection to close gaps between scenario suites and TOD.
...
TL;DR: A macro-action RL framework (MAQ) distills human demonstrations into macro actions via Vector-Quantized VAE to align RL trajectories with human behavior while maximizing rewards; achieves higher human-likeness on D4RL Adroit and is compatible with existing RL algorithms.


<details>
  <summary>Details</summary>
Motivation: Current reward-driven RL often yields unnatural, non-human-like behaviors, raising interpretability and trust concerns. The authors define human-likeness as trajectory similarity to human behavior and adapt receding-horizon control to enable tractable, efficient human-like learning.

Method: Introduce Macro Action Quantization (MAQ) that compresses human demonstrations into macro actions using Vector-Quantized VAE. Integrate this with receding-horizon control to optimize action sequences for human-likeness and reward. Demonstrate compatibility by integrating MAQ with various off-the-shelf RL algorithms and evaluate on the D4RL Adroit benchmarks with both trajectory similarity metrics and human evaluation.

Result: MAQ significantly improves human-likeness, achieving higher trajectory similarity scores and the top human-likeness rankings among RL agents in human evaluation. The framework is readily integrable with existing RL algorithms.

Conclusion: MAQ provides a tractable, efficient path to learning human-like RL agents and can be adopted across RL algorithms, indicating a promising direction for interpretable and trustworthy AI. Code is released for reproduction.
...
TL;DR: OpenBioLLM is an open-source modular multi-agent framework for genomic QA that reproduces GeneGPT with open-source LLMs and achieves on-par or better results with lower latency; code at GitHub.


<details>
  <summary>Details</summary>
Motivation: Address scalability, cost, data privacy, and generalization limitations of the proprietary GeneGPT.

Method: Reproduced GeneGPT using Llama 3.1, Qwen2.5, and Qwen2.5 Coder in a monolithic setup to identify limits; then built OpenBioLLM with modular agents for tool routing, query generation, and response validation enabling coordinated reasoning.

Result: OpenBioLLM matches or outperforms GeneGPT on over 90% of benchmark tasks (Gene-Turing 0.849, GeneHop 0.830); uses smaller open-source models without fine-tuning; reduces latency by 40-50% across tasks.

Conclusion: OpenBioLLM demonstrates the viability and efficiency of open-source multi-agent genomic QA systems; code and resources available at GitHub.
...
TL;DR: ProRAC is a neuro-symbolic RAC framework that uses progression of actions to compute a final state and answer questions, achieving strong performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Reasoning about actions and their effects is central to RAC problems; LLMs benefit from a structured progression and state evaluation to improve accuracy.

Method: Extract actions and questions from the RAC problem, progressively execute each action to derive the final state, and then evaluate the query against the progressed state to produce an answer, using LLMs as the underlying reasoning engine.

Result: The approach yields strong performance across various RAC benchmarks, domains, LLM backbones, and task types.

Conclusion: Progression-based neuro-symbolic reasoning for RAC is effective and robust, demonstrating general applicability across benchmarks and configurations.
...
TL;DR: Rogue One is an LLM-driven, multi-agent AutoFE framework that uses Scientist, Extractor, and Tester to collaboratively generate and validate predictive features, leveraging retrieval-augmented generation and a flooding-pruning strategy with qualitative feedback, achieving strong performance on 19 classification and 9 regression datasets and surfacing testable hypotheses (e.g., potential biomarker discovery).


<details>
  <summary>Details</summary>
Motivation: Addresses the fragility of monolithic LLM feature extraction, limited quantitative feedback, and poor integration of external domain knowledge in tabular data modeling; aims for interpretable, semantically meaningful features and scientific utility.

Method: Decentralized three-agent architecture (Scientist, Extractor, Tester) operating iteratively; integrates a retrieval-augmented generation (RAG) system for external knowledge; introduces a flooding-pruning strategy and qualitative feedback loop; evaluates on a broad suite of datasets.

Result: Outperforms state-of-the-art AutoFE methods on 19 classification and 9 regression datasets; demonstrates qualitative gains including discovery of novel hypotheses (e.g., biomarker in myocardial dataset); features are more semantically meaningful and interpretable.

Conclusion: The framework advances AutoFE by combining multi-agent collaboration, RAG, and qualitative feedback to produce powerful, interpretable features and enable scientific discovery; potential implications for domain knowledge integration and real-world decision making.
...
TL;DR: SafeRBench provides end-to-end safety evaluation for Large Reasoning Models by analyzing inputs, intermediate reasoning traces, and outputs using risk-characterized prompts, micro-thought chunking for fine-grained safety assessment across ten dimensions, and human-LLM alignment validation, demonstrated on 19 LRMs.


<details>
  <summary>Details</summary>
Motivation: Current safety benchmarks mainly judge final outputs and neglect safety risks that surface in intermediate reasoning steps. LRMs’ chain-of-thought can inject harmful content or justified misinfo through reasoning traces, necessitating end-to-end safety assessment.

Method: 1) Input characterization with risk categories and levels, mapping affected groups and severity to create a balanced, harm-gradient prompt suite. 2) Micro-thought chunking to divide long reasoning traces into semantically coherent units for fine-grained evaluation across ten safety dimensions. 3) Human safety alignment validation by comparing LLM-based safety judgments with human annotations. 4) Evaluation of 19 LRMs to assess multidimensional safety and protective measures.

Result: Demonstrates SafeRBench as a feasible framework enabling detailed, multidimensional safety assessment of LRMs. It reveals risks and potential protective mechanisms from multiple perspectives and validates alignment between automated (LLM-based) and human safety judgments.

Conclusion: End-to-end safety benchmarking for LRM reasoning is feasible and valuable. SafeRBench provides a structured, multi-faceted evaluation platform that can guide safer model development and more comprehensive safety standards.
...
TL;DR: A synergistic KT framework (HISE-KT) combines heterogeneous information networks (HINs) with large language models (LLMs) to improve prediction accuracy and provide evidence-backed explanations, via automated meta-path quality assessment and a meta-path-based student retrieval mechanism.


<details>
  <summary>Details</summary>
Motivation: Current KT methods using HINs inject noise through manual/meta-path selection and lack meta-path quality assessment; LLM-based approaches ignore cross-student information and explanations. A unified, interpretable approach leveraging both paradigms is needed.

Method: 1) Construct a multi-relationship HIN with diverse node types to capture relations via multiple meta-paths. 2) Use LLMs to score and filter meta-path instances, retaining high-quality paths. 3) Implement a student retrieval mechanism inspired by educational psychology to retrieve similar trajectories via meta-paths. 4) Apply a structured prompt that combines the target student’s history with retrieved trajectories to prompt the LLM for predictions and explainable reports.

Result: Empirical results on four public datasets show that HISE-KT achieves superior prediction performance and improved interpretability compared to existing KT baselines.

Conclusion: Integrating HINs with LLMs through automated meta-path quality assessment and meta-path-based retrieval yields more accurate KT predictions and evidence-based explanations, addressing noise and explainability gaps in prior methods.
...
### [125] [Task Specific Sharpness Aware O-RAN Resource Management using Multi Agent Reinforcement Learning](https://arxiv.org/abs/2511.15002)
...
### [126] [Z-Merge: Multi-Agent Reinforcement Learning for On-Ramp Merging with Zone-Specific V2X Traffic Information](https://arxiv.org/abs/2511.14910)
*Yassine Ibork,Myounggyu Won,Lokesh Das*

Main category: cs.RO

TL;DR: A V2X-assisted MARL framework for on-ramp merging that uses MA-POMDP with a hybrid action space to coordinate lane-changing and gap creation using RSU-provided global information, achieving better merging success, efficiency, and safety in simulations.


<details>
  <summary>Details</summary>
Motivation: Ramp merging in mixed traffic is a critical & challenging problem. Existing methods rely on local/neighbor info with limited coordination, leading to safety and efficiency gaps. There is a need to leverage V2X-enabled global information to coordinate lane-changing and gap-adaptation across agents.

Method: Formulate the problem as a Multiagent Partially Observable Markov Decision Process (MA-POMDP). Agents use both local and RSU-provided global observations via V2X. Introduce a hybrid action space that supports discrete and continuous decisions, implemented with a parameterized deep Q-learning approach. Evaluate in SUMO/MOSAIC V2X simulations across diverse traffic scenarios.

Result: The framework significantly improves merging success rate, traffic efficiency, and road safety across varied traffic scenarios in simulation.

Conclusion: V2X-assisted MARL with zone-specific global information enables effective coordination of lateral and longitudinal merging strategies, enhancing performance in on-ramp merging under mixed traffic; the approach is promising for real-world deployment and improved safety.
...
### [127] [A visual study of ICP variants for Lidar Odometry](https://arxiv.org/abs/2511.14919)
*Sebastian Dingler,Hannes Burrichter*

Main category: cs.RO

TL;DR: Visual analysis of ICP variants for lidar odometry using a 2D visualization of the ICP objective; introduces dynamic-object filtering and ego blind-spot mitigation to improve robustness.


<details>
  <summary>Details</summary>
Motivation: Real-world lidar odometry suffers from dynamic objects, non-overlap areas, and sensor noise, which degrade ICP accuracy. A visualization of the multidimensional ICP objective can reveal failure modes and guide robust design.

Method: Extend a 2D visualization method for the multidimensional ICP objective to lidar odometry, compare several ICP variants using this visualization, and propose a novel dynamic-object filtering method along with an approach to mitigate ego blind spots.

Result: The visualization enables diagnosing how ICP variants respond to dynamics and noise; the authors propose dynamic-object filtering and ego blind-spot mitigation methods, though the abstract does not report quantitative results.

Conclusion: Visualization-based analysis is effective for diagnosing ICP robustness in lidar odometry; the proposed filtering and ego blind-spot strategies can enhance real-world performance.
...
### [128] [SVBRD-LLM: Self-Verifying Behavioral Rule Discovery for Autonomous Vehicle Identification](https://arxiv.org/abs/2511.14977)
*Xiangyu Li,Zhaomiao Guo*

Main category: cs.RO

TL;DR: A framework (SVBRD-LLM) automatically discovers, verifies, and applies interpretable behavioral rules for autonomous vehicles from real traffic videos using YOLOv8/ByteTrack for tracking and GPT-5 zero-shot prompting; yields a high-quality rule library and strong AV-identification performance.


<details>
  <summary>Details</summary>
Motivation: As autonomous vehicles become common, understanding real-world AV behavior is crucial for traffic safety analysis, policy making, and public acceptance.

Method: Extract vehicle trajectories with YOLOv8 and ByteTrack, compute kinematic features, and use GPT-5 zero-shot prompting to generate 35 interpretable behavioral rule hypotheses comparing autonomous vs human-driven vehicles. Validate rules on a separate set, refine iteratively to filter spurious correlations, and compile a high-confidence rule library. Evaluate on an independent test set for tasks including speed-change prediction, lane-change prediction, and AV identification.

Result: On 1500+ hours of real traffic videos, the framework achieves 90.0% accuracy and 93.3% F1-score in autonomous vehicle identification. The discovered rules highlight AVs’ speed-control smoothness, lane-change conservativeness, and acceleration stability, each with semantic description, applicable context, and validation confidence.

Conclusion: Demonstrates a practical pipeline for discovering interpretable, data-driven AV behavioral rules from real-world video data, yielding a usable rule library to support safety analyses, policy considerations, and public communication.
...
### [129] [An Alignment-Based Approach to Learning Motions from Demonstrations](https://arxiv.org/abs/2511.14988)
*Alex Cuellar,Christopher K Fourie,Julie A Shah*

Main category: cs.RO

TL;DR: CALM (Cluster Alignment for Learned Motions) is an LfD framework that aligns demonstrations to a representative mean trajectory rather than pure time- or state-dependence, using clustering to generate multi-modal behaviors. It addresses drawbacks of both time-dependent and time-independent methods and demonstrates convergence properties and practical validation on 2D datasets and a 7-DoF robot across three domains.


<details>
  <summary>Details</summary>
Motivation: In Learning from Demonstration, time-dependent and time-independent approaches each have drawbacks: time-dependent methods struggle to learn overlapping trajectories, while time-independent methods can be fragile under perturbations. A robust alignment-based framework that preserves trajectory diversity and remains stable under perturbations is desirable.

Method: Introduce CALM, which aligns demonstrations to a mean trajectory rather than strictly time- or state-based representations. Develop an alignment technique that tolerates shifts due to perturbations and employ demonstration clustering to yield multi-modal behaviors. Discuss convergence properties and validate on 2D datasets, with implementation on a 7-DoF robot across three domains.

Result: CALM mitigates the drawbacks of both time-dependent and time-independent approaches on 2D datasets and demonstrates practical applicability by implementing the system on a 7-DoF robot learning tasks in three domains, including convergence analysis and perturbation-tolerant alignment.

Conclusion: CALM offers a robust LfD framework that combines mean-trajectory alignment with demonstration clustering to enable multi-modal motions while avoiding the pitfalls of existing time-dependent and time-independent methods. It provides convergence insights and practical validation on both simulations and real robot tasks.
...
### [130] [Communication-Aware Asynchronous Distributed Trajectory Optimization for UAV Swarm](https://arxiv.org/abs/2511.14994)
*Yue Yu,Xiaobo Zheng,Shaoming He*

Main category: cs.RO

TL;DR: A two-tier Communication-Aware Asynchronous Distributed Trajectory Optimization (CA-ADTO) for UAV swarms using Parameterized Differential Dynamic Programming (PDDP) for local optimization and asynchronous ADMM (async-ADMM) for swarm-wide coordination, achieving reduced communication overhead under constraints.


<details>
  <summary>Details</summary>
Motivation: Distributed UAV swarm trajectory optimization is attractive but hampered by unreliable or limited-bandwidth communication; a framework that maintains distributed control while minimizing data exchange is needed for real-world operation.

Method: A two-tier architecture: (1) local trajectory optimization via Parameterized Differential Dynamic Programming (PDDP) for each UAV, and (2) swarm coordination via asynchronous ADMM (async-ADMM). The CA-ADTO framework integrates these components to enable fully distributed optimization with reduced communication, robust to nonlinear dynamics and spatio-temporal coupling under communication constraints.

Result: The architecture enables fully distributed optimization with substantially reduced communication overhead, making it suitable for scenarios with unreliable connectivity. It effectively handles nonlinear dynamics and spatio-temporal coupling under communication constraints.

Conclusion: CA-ADTO is a viable framework for communication-constrained UAV swarms, offering distributed optimization with lower communication requirements and robustness to nonlinear dynamics, suitable for real-world deployment.
...
### [131] [Lie Group Control Architectures for UAVs: a Comparison of SE2(3)-Based Approaches in Simulation and Hardware](https://arxiv.org/abs/2511.15023)
*Dimitria Silveria,Kleber Cabral,Peter Jardine,Sidney Givigi*

Main category: cs.RO

TL;DR: SE2(3) MPC on quadrotors leverages Lie group SE2(3) structure to achieve superior trajectory tracking and robustness on Quanser QDrone, outperforming SE2(3)-LQR and industry-standard controllers in real-time experiments; simulation shows comparable performance to SE2(3)-LQR.


<details>
  <summary>Details</summary>
Motivation: Bridge Lie group theory with practical MPC for quadcopters to improve robustness and real-time performance; exploit SE2(3) geometry for better handling of attitude, position, velocity constraints.

Method: Develop SE2(3)-based MPC; integrate predictive optimization with Lie group state representation; compare to SE2(3)-based LQR in simulation; hardware experiments on Quanser QDrone; compare with industry standard controller.

Result: Simulation: SE2(3) MPC comparable to SE2(3)-LQR; Hardware: SE2(3) MPC achieves superior trajectory tracking and robustness across scenarios relative to SE2(3)-LQR and industry standard; demonstrates practical effectiveness.

Conclusion: Lie group-based controllers are practically effective; SE2(3) MPC offers improved performance and real-time viability; provides comparative insights into impact on behavior and performance.
...
### [132] [Painted Heart Beats](https://arxiv.org/abs/2511.15105)
*Angshu Adhya,Cindy Yang,Emily Wu,Rishad Hasan,Abhishek Narula,Patrícia Alves-Oliveira*

Main category: cs.RO

TL;DR: AURA is a biometrics-informed robot-artist framework where a robot arm uses the artist's heartbeat to adjust proximity during painting, with higher arousal causing retreat and neutral arousal leading to continued painting; it also explores language and touch interactions.


<details>
  <summary>Details</summary>
Motivation: To enable smoother, responsive human-robot artistic collaboration by leveraging real-time biometric cues to modulate robot behavior and reduce cognitive load or interference.

Method: Built a robot arm that collaborates with a human painter and uses EmotiBit to monitor heart rate. The system uses arousal levels to decide proximity: move closer during neutral state to enhance collaboration; move away during high arousal; maintain activity during neutral state. Additionally, the authors present alternative interaction modalities via natural language and physical touch.

Result: The robot demonstrated arousal-aware proximity control and continued painting behavior at neutral states; the approach suggests viable biometric-informed interaction; provides additional interaction modalities.

Conclusion: Biometric cues can effectively guide human-robot artistic collaboration, enabling fluent interaction; future work could refine arousal detection, safety, and expand modalities.
...
### [133] [Eq.Bot: Enhance Robotic Manipulation Learning via Group Equivariant Canonicalization](https://arxiv.org/abs/2511.15194)
*Jian Deng,Yuandong Wang,Yangfu Zhu,Tao Feng,Tianyu Wo,Zhenzhou Shao*

Main category: cs.RO

TL;DR: Eq.Bot offers a model-agnostic SE(2) equivariant canonicalization framework for robotic manipulation that boosts performance without architectural changes, achieving up to 50% gains across CNN and Transformer baselines.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal learning for robotic manipulation lacks geometric consistency under rotations/translations; bespoke equivariant architectures are complex, costly, and poorly portable.

Method: Transform observations into an SE(2)-canonical space, apply an existing policy, and map actions back to the original space. This model-agnostic approach avoids architectural modifications.

Result: Extensive experiments show the approach improves performance over existing methods on various tasks for both CNN-based (e.g., CLIPort) and Transformer-based (e.g., OpenVLA-OFT) architectures, with gains up to 50%.

Conclusion: Eq.Bot provides a portable, architecture-agnostic route to SE(2) equivariance in robotic manipulation, enabling better generalization and performance without redesigning models.
...
### [134] [VIRAL: Visual Sim-to-Real at Scale for Humanoid Loco-Manipulation](https://arxiv.org/abs/2511.15200)
...
TL;DR: VIRAL is a visual sim-to-real framework that learns humanoid loco-manipulation entirely in simulation with a privileged teacher and a vision-based student, scalable compute, and broad visual-domain randomization to achieve zero-shot deployment on a real robot (Unitree G1) with RGB policies for long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Autonomous loco-manipulation is crucial for real-world humanoid robotics, but real robots suffer from data inefficiency and the sim-to-real gap. The paper argues that scaling simulation and distilling vision-based policies from a privileged teacher can bridge this gap.

Method: Train a privileged RL teacher in full state with delta-action space and reference-state initialization on long-horizon tasks. Distill a vision-based student policy from the teacher via large-scale simulation with tiled rendering, using online DAgger and behavior cloning. Use large-scale visual domain randomization (lighting, materials, camera params, image quality, sensor delays) plus real-to-sim alignment of hands and cameras. Train with up to 64 GPUs. Deploy RGB policy on Unitree G1 for continuous loco-manipulation (up to 54 cycles) without real-world fine-tuning, and test across spatial/appearance variations. Perform extensive ablations to identify critical design choices.

Result: The approach yields a robust RGB-based loco-manipulation policy that transfers zero-shot to real hardware, achieving continuous operation for dozens of cycles and approaching expert teleoperation, with ablations highlighting the key components (compute scale, distillation, domain randomization, and alignment).

Conclusion: Large-scale simulation and a teacher-student distillation pipeline with extensive visual randomization can enable practical zero-shot RGB loco-manipulation on humanoid hardware; compute scale and design choices are essential, though real-world alignment remains a critical practical factor and may limit applicability in resource-constrained settings.
...
### [135] [A Class of Dual-Frame Passively-Tilting Fully-Actuated Hexacopter](https://arxiv.org/abs/2511.15225)
*Jiajun Liu,Yimin Zhu,Xiaorui Liu,Mingye Cao,Mingchao Li,Lixian Zhang*

Main category: cs.RO

TL;DR: A novel fully-actuated hexacopter with dual-frame passive tilting enables decoupled translation and attitude control using few actuators; eliminates internal force cancellation to boost efficiency and endurance; a model-based full-actuation controller is designed and validated by simulations.


<details>
  <summary>Details</summary>
Motivation: To achieve true decoupled translational and rotational control in a hexacopter with minimal actuators, improving flight efficiency and endurance by solving internal force cancellation issues present in existing fully-actuated UAVs.

Method: Develop dynamic model of the fully-actuated hexacopter with a dual-frame passive tilting structure; design a full-actuation controller based on the model to achieve efficient and stable control; conduct simulations to validate decoupled motion capability and controller performance.

Result: Simulation results validate the system's superior fully-actuated motion capability and the effectiveness of the proposed controller; improvements in efficiency/endurance inferred from reduced internal force cancellation under the same payload.

Conclusion: The proposed design and control approach enable efficient, stable fully-actuated hexacopter operation with enhanced endurance under payload, with simulation-level validation of the control strategy.
...
### [136] [Modelling and Model-Checking a ROS2 Multi-Robot System using Timed Rebeca](https://arxiv.org/abs/2511.15227)
...
### [137] [Symmetry-Breaking in Multi-Agent Navigation: Winding Number-Aware MPC with a Learned Topological Strategy](https://arxiv.org/abs/2511.15239)
...
### [138] [Behavior Trees vs Executable Ontologies: a Comparative Analysis of Robot Control Paradigms](https://arxiv.org/abs/2511.15274)
*Alexander Boldachev*
...
### [139] [Look, Zoom, Understand: The Robotic Eyeball for Embodied Perception](https://arxiv.org/abs/2511.15279)
...
### [140] [Path Planning through Multi-Agent Reinforcement Learning in Dynamic Environments](https://arxiv.org/abs/2511.15284)
...
### [141] [Optimizing Robot Positioning Against Placement Inaccuracies: A Study on the Fanuc CRX10iA/L](https://arxiv.org/abs/2511.15290)
...
### [142] [MSA - Technique for Stiffness Modeling of Manipulators with Complex and Hybrid Structures](https://arxiv.org/abs/2511.15294)
...
### [143] [C2F-Space: Coarse-to-Fine Space Grounding for Spatial Instructions using Vision-Language Models](https://arxiv.org/abs/2511.15333)
...
### [144] [Platform-Agnostic Reinforcement Learning Framework for Safe Exploration of Cluttered Environments with Graph Attention](https://arxiv.org/abs/2511.15358)
...
### [145] [RRT*former: Environment-Aware Sampling-Based Motion Planning using Transformer](https://arxiv.org/abs/2511.15414)
...
### [146] [Discovering Optimal Natural Gaits of Dissipative Systems via Virtual Energy Injection](https://arxiv.org/abs/2511.15513)
...
### [147] [Theoretical Closed-loop Stability Bounds for Dynamical System Coupled with Diffusion Policies](https://arxiv.org/abs/2511.15520)
...
### [148] [Decentralized Gaussian Process Classification and an Application in Subsea Robotics](https://arxiv.org/abs/2511.15529)
...
### [149] [NMPC-based Motion Planning with Adaptive Weighting for Dynamic Object Interception](https://arxiv.org/abs/2511.15532)
...
### [150] [UltraDP: Generalizable Carotid Ultrasound Scanning with Force-Aware Diffusion Policy](https://arxiv.org/abs/2511.15550)
...
### [152] [Optimus-Q: Utilizing Federated Learning in Adaptive Robots for Intelligent Nuclear Power Plant Operations through Quantum Cryptography](https://arxiv.org/abs/2511.15614)
...
### [153] [Real-time Point Cloud Data Transmission via L4S for 5G-Edge-Assisted Robotics](https://arxiv.org/abs/2511.15677)
...
### [154] [In-N-On: Scaling Egocentric Manipulation with in-the-wild and on-task Data](https://arxiv.org/abs/2511.15704)
...
### [155] [Transformer Injectivity & Geometric Robustness - Analytic Margins and Bi-Lipschitz Uniformity of Sequence-Level Hidden States](https://arxiv.org/abs/2511.14808)
*Mikael von Strauss*

Main category: cs.LG

TL;DR: The paper develops a theoretical dichotomy for prompt-to-last-token mappings in decoder-only Transformers, introducing layer-specific collision discriminants and injective strata, plus symmetry-aware reductions, and validates these ideas with geometric diagnostics showing strong injectivity at full and 8-bit precision but limited injectivity under 4-bit quantization; empirical results align with the theory, suggesting generically injective representations in continuous-parameter settings, with practical probes for invertibility.


<details>
  <summary>Details</summary>
Motivation: To understand when Transformer representations are identifiable/injective with respect to discrete prompts, both in theory (per-layer injectivity and its persistence along training) and in practice (diagnostics on real models and quantization effects).

Method: Theoretically, for each layer ℓ define a collision discriminant Δ^ℓ ⊂ Θ and an injective stratum U^ℓ = Θ\Δ^ℓ. Prove a dichotomy: either the model is nowhere injective on the set, or U^ℓ is open and dense and every F^ℓ_θ is injective. Under mild non-singularity assumptions on the optimizer and an absolutely continuous initialization, generic injectivity persists along smooth training trajectories over fixed horizons. Extend to symmetry groups G: discriminants and injective strata descend to Θ/G. Empirically, define a separation margin and a co-Lipschitz constant between prompt space and last-token representation space, estimated via nearest-neighbor statistics on large prompt sets. Evaluate on pretrained LLaMA-3 and Qwen across layers, sequence lengths, model scales, and 8- and 4-bit activation quantization; also train a small GPT-2 from scratch to observe metric stability.

Result: The theory yields a clear dichotomy: either no injectivity anywhere or an open dense injective regime at each layer, with injectivity persisting along training under mild assumptions, and symmetry reduces the analysis to quotient spaces. Empirically, there are no collisions under full precision or 8-bit quantization; 4-bit quantization introduces a small number of collisions and lowers the co-Lipschitz constant. The GPT-2 from scratch experiment shows metric stability during training.

Conclusion: Transformer representations are generically and persistently injective in the continuous-parameter idealization. Practical invertibility can be probed with simple geometric diagnostics, and symmetry considerations align injectivity with functional equivalence classes.
...
### [156] [Dynamic Nested Hierarchies: Pioneering Self-Evolution in Machine Learning Architectures for Lifelong Intelligence](https://arxiv.org/abs/2511.14823)
*Akbar Anbar Jafari,Cagri Ozcinar,Gholamreza Anbarjafari*

Main category: cs.LG

TL;DR: Proposes dynamic nested hierarchies that autonomously adapt the number of optimization levels, their nesting structures, and update frequencies to enable lifelong learning and adaptation in non-stationary environments, backed by theoretical guarantees and empirical gains in NLP, continual learning, and long-context reasoning.


<details>
  <summary>Details</summary>
Motivation: Current ML models, including LLMs, struggle in non-stationary settings due to rigid architectures and fixed update regimes. This leads to poor continual adaptation and lifelong learning (anterograde amnesia). The paper extends the nested learning paradigm with dynamic, neuroplastic-inspired hierarchies to enable self-evolution and better distribution-shift handling.

Method: Introduce dynamic nested hierarchies that can autonomously adjust the number of optimization levels, nesting structures, and update frequencies during training or inference. Provide rigorous mathematical formulations and proofs (convergence, expressivity bounds, sublinear regret) and conduct empirical evaluations in language modeling, continual learning, and long-context reasoning.

Result: The approach yields superior performance across evaluated tasks, accompanied by theoretical guarantees (convergence, expressivity, sublinear regret), demonstrating effective lifelong learning and adaptation in challenging, non-stationary environments.

Conclusion: Dynamic nested hierarchies constitute a foundational step toward adaptive, general-purpose AI, enabling self-evolution, better handling of distribution shifts, and improved long-context reasoning via autonomous architectural and update-heuristic adjustments.
...
### [157] [Empowering Multi-Turn Tool-Integrated Reasoning with Group Turn Policy Optimization](https://arxiv.org/abs/2511.14846)
*Yifeng Ding,Hung Le,Songyang Han,Kangrui Ruan,Zhenghui Jin,Varun Kumar,Zijian Wang,Anoop Deoras*

Main category: cs.LG

TL;DR: GTPO introduces turn-level rewards, return-based advantage estimation, and self-supervised reward shaping to train LLMs in multi-turn Tool-Integrated Reasoning; it outperforms GRPO by 3.0% across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current RL methods like GRPO rely on coarse, trajectory-level rewards that give insufficient learning signals for multi-turn tool use and code execution, causing training stagnation. A finer, per-turn and self-supervised learning signal is needed to advance LLMs’ mathematical and reasoning capabilities in real-world tasks.

Method: Three innovations: (1) turn-level reward assignment providing fine-grained feedback for each turn; (2) return-based advantage estimation using normalized discounted returns as advantages; (3) self-supervised reward shaping leveraging signals from generated code to densify sparse binary outcome rewards.

Result: GTPO outperforms GRPO by 3.0% on average across diverse reasoning benchmarks, demonstrating improved learning efficiency and effectiveness for multi-turn TIR tasks.

Conclusion: GTPO effectively enhances training of LLMs for multi-turn Tool-Integrated Reasoning and advances capabilities in complex mathematical reasoning for real-world applications.
...
### [158] [FinTRec: Transformer Based Unified Contextual Ads Targeting and Personalization for Financial Applications](https://arxiv.org/abs/2511.14865)
*Dwipam Katariya,Snehita Varma,Akshat Shreemali,Benjamin Wu,Kalanand Mishra,Pranab Mohanty*

Main category: cs.LG

TL;DR: FinTRec is a unified transformer-based sequential recommender for financial services that outperforms tree-based baselines, reduces training cost and technical debt, and enables cross-product signal sharing across long-range, multi-channel interactions.


<details>
  <summary>Details</summary>
Motivation: Financial Services require modeling long-range user interactions across digital and physical channels, handling multiple interrelated products with coordinated objectives, and complying with regulatory constraints. A unified, cost-efficient transformer approach aims to improve recommendation quality while addressing practical deployment concerns.

Method: Introduce FinTRec, a transformer-based unified sequential recommendation framework for FS, enabling cross-product signal sharing and product-specific adaptation. Evaluate via historic simulation and live A/B tests against production-grade tree-based baselines.

Result: FinTRec consistently outperforms the tree-based baseline in offline and live evaluations, reduces training cost and technical debt, and improves cross-product performance across all products.

Conclusion: This work presents the first comprehensive study of unified sequential recommendation modeling in FS, demonstrating transformer-based architectures as a viable alternative to traditional tree-based models while meeting technical and business objectives.
...
### [159] [Transformer-Guided Deep Reinforcement Learning for Optimal Takeoff Trajectory Design of an eVTOL Drone](https://arxiv.org/abs/2511.14887)
*Nathan M. Roberts,Xiaosong Du*

Main category: cs.LG

TL;DR: Transformer-guided DRL improves training efficiency and energy-optimal takeoff trajectories for eVTOL by using a transformer to explore a realistic state space; achieves significant reduction in training steps and higher energy-optimal accuracy than vanilla DRL.


<details>
  <summary>Details</summary>
Motivation: DRL for eVTOL takeoff trajectory optimization faces long training times and high problem complexity; traditional methods struggle with dimensionality, so a more powerful exploration/representation (transformer) could accelerate learning and improve optimality.

Method: Integrate a transformer module with DRL to guide state-space exploration at each time step. Apply to a takeoff trajectory optimization problem for an eVTOL drone, minimizing energy while meeting takeoff constraints (minimum vertical displacement, minimum horizontal velocity). Control variables include power and wing angle to vertical. Compare transformer-guided DRL to vanilla DRL and to conventional methods (dynamic programming, LQR). Key metrics: training steps to convergence and energy-optimality accuracy versus a simulation-based optimum.

Result: Transformer-guided DRL required 4.57×10^6 time steps to learn takeoff, about 25% of the 19.79×10^6 steps needed by vanilla DRL. It achieved 97.2% accuracy on the optimal energy consumption compared to the simulation-based optimum, higher than vanilla DRL’s 96.3% accuracy.

Conclusion: The transformer-guided DRL approach outperforms vanilla DRL in both training efficiency and verification of optimal energy-enabled takeoff trajectories, demonstrating the value of transformer-guided state-space exploration for complex, nonlinear control tasks in eVTOL applications.
...
### [160] [Bringing Federated Learning to Space](https://arxiv.org/abs/2511.14889)
*Grace Kim,Filip Svoboda,Nicholas Lane*

Main category: cs.LG

TL;DR: Space-ified FL enables scalable on-board learning in LEO constellations using FedAvg/FedProx/FedBuff, achieving near-centralized performance with significant speedups.


<details>
  <summary>Details</summary>
Motivation: Address downlink bandwidth limits and intermittent connectivity in growing satellite networks; enable autonomous, data-driven operations through distributed learning.

Method: Develop a space-ification framework adapting terrestrial FL algorithms to orbital constraints; perform large-scale parameter sweeps across 768 constellation configurations varying cluster sizes, satellites per cluster, ground stations; evaluate performance metrics (accuracy, convergence, speedups).

Result: Space-adapted FL scales to 100 satellites, near central performance; 9x speedup via orbital scheduling and local cluster coordination; provides actionable guidance for mission designers.

Conclusion: This framework enables resilient, autonomous distributed learning in future satellite ecosystems; further research can broaden to asynchronous FL, security, and dynamic topologies.
...
### [161] [It's LIT! Reliability-Optimized LLMs with Inspectable Tools](https://arxiv.org/abs/2511.14903)
*Ruixin Zhang,Jon Donnelly,Zhicheng Guo,Ghazal Khalighinejad,Haiyang Huang,Alina Jade Barnett,Cynthia Rudin*

Main category: cs.LG

TL;DR: Proposes LIT: an inspectable tool-use framework that steers LLMs to rely on external, reliable tools, using a reliability-cost framework to select tool paths, with a 1,300-question benchmark; demonstrates improved reliability without harming performance.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce opaque reasoning and unreliable solutions in high-stakes tasks; there is a need to enforce trustworthiness by favoring more reliable, easy-to-troubleshoot tools.

Method: Introduce LIT (LLMs with Inspectable Tools), a framework that leverages tool-calling to choose reliable, troubleshootable solutions, potentially via multiple sequential tool calls; develop a customizable set of reliability cost functions for tools; create a 1,300-question benchmark interacting with the Harvard USPTO Patent Dataset and a NeurIPS 2023 papers dataset to cover math, coding, and modeling problems.

Result: LLMs can achieve more reliable and informed problem-solving while maintaining task performance under the LIT framework.

Conclusion: LIT enables trustworthy, inspectable problem-solving by guiding tool selection based on reliability and troubleshootability; shows potential across domains.
...
### [162] [Structured Contrastive Learning for Interpretable Latent Representations](https://arxiv.org/abs/2511.14920)
*Zhengyang Shen,Hua Tu,Mayue Shi*

Main category: cs.LG

TL;DR: Introduces Structured Contrastive Learning (SCL) to enforce three latent groups—Invariant, Variant, Free—improving robustness to phase shifts in ECG and rotation in IMU benchmarks; achieves strong gains without architecture changes.


<details>
  <summary>Details</summary>
Motivation: Neural networks are brittle to semantically irrelevant transformations; laissez-faire representation learning allows latent spaces to drift when task performance suffices. A structured latent space with interpretable invariances and controlled variability could improve robustness and interpretability.

Method: Partition latent representations into three semantic groups: invariant features that stay constant under specified transformations, variant features that are encouraged to differentiate transformations via a novel variant mechanism, and free features that preserve task flexibility. Implement a push-pull dynamic through a variant mechanism within contrastive learning, with no architectural changes and easy integration into existing pipelines.

Result: ECG cosine similarity under phase shifts degrades from 1.0 to 0.2 in a naive setup; with SCL, ECG similarity improves from 0.25 to 0.91 under phase shifts. WISDM IMU-based activity recognition achieves 86.65% accuracy with 95.38% rotation consistency, outperforming traditional data augmentation.

Conclusion: Shifts from reactive data augmentation to proactive structural learning, enabling interpretable, robust latent representations without changing architectures, validated on ECG and IMU tasks.
...
### [163] [Integrating Causal Inference with Graph Neural Networks for Alzheimer's Disease Analysis](https://arxiv.org/abs/2511.14922)
...
### [164] [How to Train Private Clinical Language Models: A Comparative Study of Privacy-Preserving Pipelines for ICD-9 Coding](https://arxiv.org/abs/2511.14936)
...
### [165] [Knowledge Graphs as Structured Memory for Embedding Spaces: From Training Clusters to Explainable Inference](https://arxiv.org/abs/2511.14961)
...
### [166] [IonCast: A Deep Learning Framework for Forecasting Ionospheric Dynamics](https://arxiv.org/abs/2511.15004)
...
### [167] [Simulated Human Learning in a Dynamic, Partially-Observed, Time-Series Environment](https://arxiv.org/abs/2511.15032)
...
### [168] [Oversampling techniques for predicting COVID-19 patient length of stay](https://arxiv.org/abs/2511.15048)
...
### [169] [Interpretable temporal fusion network of multi- and multi-class arrhythmia classification](https://arxiv.org/abs/2511.15062)
*Yun Kwan Kim*
...
### [170] [Deep Pathomic Learning Defines Prognostic Subtypes and Molecular Drivers in Colorectal Cancer](https://arxiv.org/abs/2511.15067)
...
### [171] [Fourier-KAN-Mamba: A Novel State-Space Equation Approach for Time-Series Anomaly Detection](https://arxiv.org/abs/2511.15083)
...
### [172] [Semiconductor Industry Trend Prediction with Event Intervention Based on LSTM Model in Sentiment-Enhanced Time Series Data](https://arxiv.org/abs/2511.15112)
...
### [173] [Efficient RF Passive Components Modeling with Bayesian Online Learning and Uncertainty Aware Sampling](https://arxiv.org/abs/2511.15125)
...
### [174] [Novel sparse matrix algorithm expands the feasible size of a self-organizing map of the knowledge indexed by a database of peer-reviewed medical literature](https://arxiv.org/abs/2511.15136)
...
### [175] [From Solving to Verifying: A Unified Objective for Robust Reasoning in LLMs](https://arxiv.org/abs/2511.15137)
...
### [176] [Cross-Modal Consistency-Guided Active Learning for Affective BCI Systems](https://arxiv.org/abs/2511.15138)
...
### [177] [Complex variational autoencoders admit Kähler structure](https://arxiv.org/abs/2511.15172)
*Andrew Gracyk*
...
### [178] [FaultDiffusion: Few-Shot Fault Time Series Generation with Diffusion Model](https://arxiv.org/abs/2511.15174)
...
### [179] [Vehicle Routing Problems via Quantum Graph Attention Network Deep Reinforcement Learning](https://arxiv.org/abs/2511.15175)
...
### [180] [Masked Auto-Regressive Variational Acceleration: Fast Inference Makes Practical Reinforcement Learning](https://arxiv.org/abs/2511.15190)
...
### [181] [Reasoning in Diffusion Large Language Models is Concentrated in Dynamic Confusion Zones](https://arxiv.org/abs/2511.15208)
...
### [182] [D2D Power Allocation via Quantum Graph Neural Network](https://arxiv.org/abs/2511.15246)
...
### [183] [EntroPIC: Towards Stable Long-Term Training of LLMs via Entropy Stabilization with Proportional-Integral Control](https://arxiv.org/abs/2511.15248)
...
### [184] [Optimized scheduling of electricity-heat cooperative system considering wind energy consumption and peak shaving and valley filling](https://arxiv.org/abs/2511.15250)
...
### [185] [PLATONT: Learning a Platonic Representation for Unified Network Tomography](https://arxiv.org/abs/2511.15251)
...
### [186] [GRPO-RM: Fine-Tuning Representation Models via GRPO-Driven Reinforcement Learning](https://arxiv.org/abs/2511.15256)
...
### [187] [PCARNN-DCBF: Minimal-Intervention Geofence Enforcement for Ground Vehicles](https://arxiv.org/abs/2511.15522)
...
### [188] [SNAP: Low-Latency Test-Time Adaptation with Sparse Updates](https://arxiv.org/abs/2511.15276)
...
### [189] [Quant-Trim in Practice: Improved Cross-Platform Low-Bit Deployment on Edge NPUs](https://arxiv.org/abs/2511.15300)
...
### [190] [DeepThinkVLA: Enhancing Reasoning Capability of Vision-Language-Action Models](https://arxiv.org/abs/2511.15669)
...
### [191] [On the Internal Semantics of Time-Series Foundation Models](https://arxiv.org/abs/2511.15324)
...
### [192] [KrawtchoukNet: A Unified GNN Solution for Heterophily and Over-smoothing with Adaptive Bounded Polynomials](https://arxiv.org/abs/2511.15327)
*Huseyin Goksu*
...
### [193] [LaguerreNet: Advancing a Unified Solution for Heterophily and Over-smoothing with Adaptive Continuous Polynomials](https://arxiv.org/abs/2511.15328)
*Huseyin Goksu*
...
### [194] [STREAM-VAE: Dual-Path Routing for Slow and Fast Dynamics in Vehicle Telemetry Anomaly Detection](https://arxiv.org/abs/2511.15339)
...
### [195] [Multi-layer Stack Ensembles for Time Series Forecasting](https://arxiv.org/abs/2511.15350)
...
### [196] [Cost-Aware Prediction (CAP): An LLM-Enhanced Machine Learning Pipeline and Decision Support System for Heart Failure Mortality Prediction](https://arxiv.org/abs/2511.15357)
...
### [197] [CID: Measuring Feature Importance Through Counterfactual Distributions](https://arxiv.org/abs/2511.15371)
...
### [198] [Parameter Importance-Driven Continual Learning for Foundation Models](https://arxiv.org/abs/2511.15375)
...
### [199] [EVA-Net: Interpretable Brain Age Prediction via Continuous Aging Prototypes from EEG](https://arxiv.org/abs/2511.15393)
...
### [200] [Proximal Approximate Inference in State-Space Models](https://arxiv.org/abs/2511.15409)
...
### [201] [Towards Understanding Layer Contributions in Tabular In-Context Learning Models](https://arxiv.org/abs/2511.15432)
...
### [202] [TSFM in-context learning for time-series classification of bearing-health status](https://arxiv.org/abs/2511.15447)
...
### [203] [FairEnergy: Contribution-Based Fairness meets Energy Efficiency in Federated Learning](https://arxiv.org/abs/2511.15454)
...
### [204] [NTK-Guided Implicit Neural Teaching](https://arxiv.org/abs/2511.15487)
...
### [205] [Sample-Adaptivity Tradeoff in On-Demand Sampling](https://arxiv.org/abs/2511.15507)
...
### [206] [CODE: A global approach to ODE dynamics learning](https://arxiv.org/abs/2511.15619)
...
### [207] [Continual Reinforcement Learning for Cyber-Physical Systems: Lessons Learned and Open Challenges](https://arxiv.org/abs/2511.15652)
...
### [208] [Walrus: A Cross-Domain Foundation Model for Continuum Dynamics](https://arxiv.org/abs/2511.15684)
...
### [209] [The Impact of Quantization on Large Reasoning Model Reinforcement Learning](https://arxiv.org/abs/2511.15694)
